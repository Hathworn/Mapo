...
__global__ void MatrixMulSh(float *Md, float *Nd, float *Pd, const int WIDTH) {
    // Shared arrays to hold tiles of the input matrices
    ...
    float Pvalue = 0; // Accumulate result for each thread

    for (int m = 0; m < WIDTH / TILE_WIDTH; m++) {
        // Load tiles into shared memory
        Mds[threadIdx.y][threadIdx.x] = Md[row * WIDTH + (m * TILE_WIDTH + threadIdx.x)];
        Nds[threadIdx.y][threadIdx.x] = Nd[(m * TILE_WIDTH + threadIdx.y) * WIDTH + col];
        __syncthreads(); // Synchronize to ensure all data is available
