...
__global__ void tonemap( float* d_x, float* d_y, float* d_log_Y, float* d_cdf_norm, float* d_r_new, float* d_g_new, float* d_b_new, float  min_log_Y, float  max_log_Y, float  log_Y_range, int    num_bins, int    num_pixels_y, int    num_pixels_x )
{
    int nx = num_pixels_x;
    int ny = num_pixels_y;
    int image_index_1d = (ny * blockIdx.y + threadIdx.y) * nx + (blockIdx.x * blockDim.x + threadIdx.x);

    // Ensure indices are within the image dimensions
    if (threadIdx.x < nx && threadIdx.y < ny)
    {
        float x = d_x[image_index_1d];
        float y = d_y[image_index_1d];
        float log_Y = d_log_Y[image_index_1d];
        
        // Calculate bin index with simplified logic
        int bin_index = int(fminf(num_bins - 1, (num_bins * (log_Y - min_log_Y)) / log_Y_range));
