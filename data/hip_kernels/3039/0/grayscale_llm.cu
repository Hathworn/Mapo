#include "hip/hip_runtime.h"
#include "includes.h"
/*----------------------------------------------------------------

*

* Multiprocesadores: Cuda

* Fecha: 11-Nov-2019

* Autor: A01206747 Mariana Perez
Autor: A01205559 Roberto Nuñez

* Image = 1080 x 1920
Speedup =  33.93700 ms / 0.00250 ms  = 13.5748

*--------------------------------------------------------------*/

__global__ void grayscale(unsigned char *src, unsigned char *dest, int width, int height, int nChannels) {
    // Calculate global thread coordinates
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    int x = blockIdx.x * blockDim.x + threadIdx.x;

    // Check if within image bounds
    if(y < height && x < width) {
        int pos = (y * width + x) * nChannels;

        // Use float for calculations, apply constant grayscale transformation
        unsigned char r = src[pos];
        unsigned char g = src[pos + 1];
        unsigned char b = src[pos + 2];
        float gray = 0.2126f * r + 0.7152f * g + 0.0722f * b;

        // Store the result back to the destination
        dest[pos] = dest[pos + 1] = dest[pos + 2] = (unsigned char)gray;
    }
}