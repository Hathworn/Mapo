...
__device__ void vanGenuchten(double *C, double *K, double *Ksat, double *theta, double *h, double n, double m, int size) {
    ...
    for (int i = 0; i < size; i++) {
        ...
        double powSe_m = pow(Se, 1.0/m);
        K[i] = Ksat[i] * sqrt(Se) * pow(1.0 - pow(1.0 - powSe_m, m), 2);

        // Compute the specific moisture storage derivative of eqn (21).
        if (h_ < 0) {
            C[i] = 100 * alpha * n * (1.0/n - 1.0) * pow(alpha * fabs(h_), n - 1)
                   * (theta_R - theta_S) * pow(pow(alpha * fabs(h_), n) + 1, 1.0/n - 2.0);
        } else {
            C[i] = 0.0;
        }
    }
