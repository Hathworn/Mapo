...
__global__ void Find3DMinMax(int *d_Result, float *d_Data1, float *d_Data2, float *d_Data3, int width, int pitch, int height) {
    ...
    for (int y = 0; y < 32 + 2; y++) {
        ...
        // Optimize data cache loading with only boundary checks, remove redundant checks
        if (tx >= (WARP_SIZE - 1)) {
        ...
        if (y > 1) {
            if (tx < memWid) {
                // Use fewer minmax calculations by reusing intermediate results
                ...
                ymin1[tx] = min1;
                ymin2[tx] = fminf(min1, fminf(data2[memPos0], fminf(data2[memPos1], data2[memPos2])));
                ymin3[tx] = min3;
                ymax1[tx] = max1;
                ymax2[tx] = fmaxf(max1, fmaxf(data2[memPos0], fmaxf(data2[memPos1], data2[memPos2])));
                ymax3[tx] = max3;
            }
        }
        __syncthreads();
        ...
        if (y > 1) {
            if (tx < MINMAX_W) {
                if (xWritePos <= xEndClamped) {
                    // Minimize computation by pre-computing repeated expressions
                    float minv = fminf(fminf(ymin2[tx], ymin2[tx + 2]), fminf(ymin1[tx + 1], ymin3[tx + 1]));
                    minv = fminf(minv, fminf(data2[memPos0 + 1], data2[memPos2 + 1]));
                    minv = fminf(minv, d_Threshold[1]);

                    float maxv = fmaxf(fmaxf(ymax2[tx], ymax2[tx + 2]), fmaxf(ymax1[tx + 1], ymax3[tx + 1]));
