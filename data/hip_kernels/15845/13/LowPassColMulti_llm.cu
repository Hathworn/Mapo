...
__global__ void LowPassColMulti(float *d_Result, float *d_Data, int width, int pitch, int height)
{
    ...
    if (colStart < width) {
        ...
        // Use loop unrolling for loading data
    ...
    if (colStart < width) {
        ...
        // Reduce memory access and optimize computation
        for (int y = miny + ty; y <= maxy; y += blockDim.y) {
            float sum = 0.0f;
            sum += (sdata[0*CONVCOL_W] + sdata[8*CONVCOL_W]) * kernel[0];
            sum += (sdata[1*CONVCOL_W] + sdata[7*CONVCOL_W]) * kernel[1];
            sum += (sdata[2*CONVCOL_W] + sdata[6*CONVCOL_W]) * kernel[2];
            sum += (sdata[3*CONVCOL_W] + sdata[5*CONVCOL_W]) * kernel[3];
            sum += sdata[4*CONVCOL_W] * kernel[4];
            d_Result[gPos] = sum;
            sdata += sStep;
            gPos += gStep;
        }
