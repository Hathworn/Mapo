...
__global__ void ComputeOrientations(float *g_Data, int *d_Ptrs, float *d_Orient, int maxPts, int w, int h)
{
    ...
    for (int y = 0; y < 15; y++) {
        ...
        int xi = xp + x;
        int yi = yp + y;
        xi = min(max(0, xi), w - 1); // Clamp within bounds
        yi = min(max(0, yi), h - 1); // Clamp within bounds
        if (x >= 0 && x < 15)
            data[memPos] = g_Data[yi * w + xi];
    }
    __syncthreads();

    // Compute gradients and bin contributions
    for (int y = 1; y < 14; y++) {
        int memPos = 16 * y + x;
        if (x >= 1 && x < 14) {
            float dy = data[memPos + 16] - data[memPos - 16];
            float dx = data[memPos + 1] - data[memPos - 1];
            int bin = 16.0f * atan2f(dy, dx) / 3.141592653589793f + 16.5f;
            bin = bin == 32 ? 0 : bin; // Wrap bins
