...
__global__ void ComputeOrientations(float *g_Data, int *d_Ptrs, float *d_Orient, int maxPts, int w, int h)
{
    ...
    // Load data around keypoint into shared memory
    for (int y = 0; y < 15; y++) {
        int memPos = 16 * y + x;
    ...
    for (int y = 1; y < 14; y++) {
        ...
        if (x >= 1 && x < 14) {
            ...
            int bin = __float2int_rn(16.0f * atan2f(dy, dx) / 3.1416f + 16.5f) & 31;
            float grad = hypotf(dx, dy);
    ...
    float v = hist[tx];
    if (tx == 0) {
        hist[32] = v * 6 + 4 * (hist[1] + hist[31]) + (hist[2] + hist[30]);
    } else if (tx == 1) {
        hist[33] = v * 6 + 4 * (hist[2] + hist[0]) + (hist[3] + hist[31]);
    } else if (tx >= 2 && tx <= 29) {
        hist[tx + 32] = v * 6 + 4 * (hist[tx + 1] + hist[tx - 1]) + (hist[tx + 2] + hist[tx - 2]);
    } else if (tx == 30) {
        hist[62] = v * 6 + 4 * (hist[31] + hist[29]) + (hist[0] + hist[28]);
    } else if (tx == 31) {
        hist[63] = v * 6 + 4 * (hist[0] + hist[30]) + (hist[1] + hist[29]);
    }
    __syncthreads();

    // Find peak orientations
    if (tx == 0) {
