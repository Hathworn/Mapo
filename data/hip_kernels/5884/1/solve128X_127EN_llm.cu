...
__global__ void solve128X_127EN(uint32_t *gRege, uint8_t *gRHash, uint32_t *gproof)
{
    ...
    for (i = 0; i < SolveEN; i++)
    {
        ...
        else if (m == CLEN - 1)
        {
            ...
            // Sort path
#pragma unroll
            for (int outer = 0; outer < CLEN - 1; ++outer)
            {
                for (int inner = 0; inner < CLEN - outer - 1; ++inner)
                {
                    if (path[block_tid][inner] > path[block_tid][inner + 1])
                    {
                        tmp = path[block_tid][inner];
                        path[block_tid][inner] = path[block_tid][inner + 1];
                        path[block_tid][inner + 1] = tmp;
                    }
                }
            }
