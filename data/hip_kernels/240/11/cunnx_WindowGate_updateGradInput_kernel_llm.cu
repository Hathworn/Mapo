...
__global__ void cunnx_WindowGate_updateGradInput_kernel(
    ...
    const float* output, const float* gradOutput, int inputSize,
    int outputSize, int outputWindowSize, float c, float d, float e, float lr)
{
    ...
    for (unsigned int i = tx; i < outputWindowSize; i += blockDim.x) {
        acc += gradOutput_k[i] * output_k[i] * ((float)(i + 1) - centroid);
    }
    buffer[tx] = acc;
    ...
    for (int i = tx; i < inputSize; i += blockDim.x) {
        ...
        acc -= logf(input + 1e-7f) * target;  // NLL
        gradInput_k[i] = -target / (input + 1e-7f);  // grad w.r.t. NLL
    }
    buffer[tx] = acc;
