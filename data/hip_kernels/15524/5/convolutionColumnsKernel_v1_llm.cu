...
__global__ void convolutionColumnsKernel_v1(float *d_Dst, float *d_Src, int imageW, int imageH, int pitch, int filter_Rad, int Halo_steps) {
    ...
    const int sharedIdxOffset = threadIdx.x * (COLUMNS_RESULT_STEPS + 2 * Halo_steps) * COLUMNS_BLOCKDIM_Y + threadIdx.y;
    const int pitchOffset = COLUMNS_BLOCKDIM_Y * pitch;

    // Upper halo
    #pragma unroll
    for (int i = 0; i < Halo_steps; i++) {
        const int index = sharedIdxOffset + i * COLUMNS_BLOCKDIM_Y;
        s_Data[index] = (baseY >= -i * COLUMNS_BLOCKDIM_Y) ? d_Src[i * pitchOffset] : 0;
    }
    ...
    for (int i = Halo_steps; i < Halo_steps + COLUMNS_RESULT_STEPS + Halo_steps; i++) {
        const int index = sharedIdxOffset + i * COLUMNS_BLOCKDIM_Y;
        s_Data[index] = (baseY + i * COLUMNS_BLOCKDIM_Y < imageH) ? d_Src[i * pitchOffset] : 0;
    }
    ...
    for (int i = Halo_steps; i < Halo_steps + COLUMNS_RESULT_STEPS; i++) {
        ...
        if (baseY + i * COLUMNS_BLOCKDIM_Y < imageH) {
            ...
            for (int j = -filter_Rad; j <= filter_Rad; j++) {
                sum += c_Kernel[filter_Rad - j] * s_Data[sharedIdxOffset + i * COLUMNS_BLOCKDIM_Y + j];
            }
            d_Dst[i * pitchOffset] = sum;
        }
    }
}