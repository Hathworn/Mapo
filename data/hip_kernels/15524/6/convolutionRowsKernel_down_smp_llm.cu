...
__global__ void convolutionRowsKernel_down_smp(float *d_Dst, float *d_Src, int imageW, int n_imageW, int imageH, int filter_Rad, int Halo_steps)
{
    ...
    if (baseY < imageH)
    {
        ...
        {
            s_Data[(threadIdx.y * (ROWS_RESULT_STEPS * 2 + Halo_steps * 2) * ROWS_BLOCKDIM_X) + threadIdx.x + i * ROWS_BLOCKDIM_X] = (baseX + i * ROWS_BLOCKDIM_X >= 0) ? d_Src[i * ROWS_BLOCKDIM_X] : 0;
        }
        ...
        {
            s_Data[(threadIdx.y * (ROWS_RESULT_STEPS * 2 + Halo_steps * 2) * ROWS_BLOCKDIM_X) + threadIdx.x + i * ROWS_BLOCKDIM_X] = (baseX + i * ROWS_BLOCKDIM_X < imageW) ? d_Src[i * ROWS_BLOCKDIM_X] : 0;
        }

        // Compute and store results
        __syncthreads();
        
        // Combining the conditions to avoid redundant checks
        #pragma unroll
        for (int i = 0; i < ROWS_RESULT_STEPS; ++i)
        {
            if (baseX1 + i * ROWS_BLOCKDIM_X < n_imageW)
            {
                ...
                {
                    sum += c_Kernel[filter_Rad - j] * s_Data[(threadIdx.y * (ROWS_RESULT_STEPS * 2 + Halo_steps * 2) * ROWS_BLOCKDIM_X) + (Halo_steps + 2 * i) * ROWS_BLOCKDIM_X + threadIdx.x * 2 + j];
                }
