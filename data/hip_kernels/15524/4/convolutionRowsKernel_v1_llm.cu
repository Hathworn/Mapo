...
__global__ void convolutionRowsKernel_v1(float *d_Dst, float *d_Src, int imageW, int filter_Rad, int Halo_steps)
{
    ...
    {
        s_Data[threadIdx.x + i * ROWS_BLOCKDIM_X] = (baseX + i * ROWS_BLOCKDIM_X >= 0) ? d_Src[i * ROWS_BLOCKDIM_X] : 0;
    }
    ...
    {
        s_Data[threadIdx.x + i * ROWS_BLOCKDIM_X] = (baseX + i * ROWS_BLOCKDIM_X < imageW) ? d_Src[i * ROWS_BLOCKDIM_X] : 0;
    }
    ...
    for (int i = Halo_steps; i < Halo_steps + ROWS_RESULT_STEPS; i++)
    {
        ...
        if (baseX + i * ROWS_BLOCKDIM_X < imageW)
        {
            ...
            {
                sum += c_Kernel[filter_Rad - j] * s_Data[threadIdx.x + i * ROWS_BLOCKDIM_X + j];
            }
