...
void bambooLogKernelBegin(int staticKernelIndex) {
	...
	if( staticKernelIndex == fiStaticKernelIndex && dynamicKernelIndex == fiDynamicKernelIndex ){
		...
		hipMemcpyToSymbol(HIP_SYMBOL(deviceFiFlag), &fiFlag, sizeof(bool), 0, hipMemcpyHostToDevice);
		hipMemcpyToSymbol(HIP_SYMBOL(deviceFiInstCount), &fiInstCount, sizeof(long long), 0, hipMemcpyHostToDevice);
		hipMemcpyToSymbol(HIP_SYMBOL(deviceFiThreadIndex), &fiThreadIndex, sizeof(long long), 0, hipMemcpyHostToDevice);
		hipMemcpyToSymbol(HIP_SYMBOL(deviceSeedFactor), &seedFactor, sizeof(double), 0, hipMemcpyHostToDevice);
	}
...
void bambooLogKernelEnd(int staticKernelIndex) {
	...
	if( staticKernelIndex == fiStaticKernelIndex && dynamicKernelIndex == fiDynamicKernelIndex ){
		int fiBit;
		long fiBambooIndex;
		hipMemcpyFromSymbol(&fiBit, HIP_SYMBOL(deviceFiBit), sizeof(int), 0, hipMemcpyDeviceToHost);
		hipMemcpyFromSymbol(&fiBambooIndex, HIP_SYMBOL(deviceBambooIndex), sizeof(long), 0, hipMemcpyDeviceToHost);
		ofstream logf;
	...
	if(error != hipSuccess){
		...
		errf << "Error Detected: " << hipGetErrorString(error) << "\ndynamicKernelIndex: " << dynamicKernelIndex << "\nstaticKernelIndex: " << staticKernelIndex << "\n";
		errf.close();
		exit(-20);
	}
	ofstream ktracef("bamboo.ktrace.log.txt", std::ios_base::app | std::ios_base::out);
