...
__global__ void Correlation_backward_input1(int item, float *gradInput1, int nInputChannels, int inputHeight, int inputWidth, float *gradOutput, int nOutputChannels, int outputHeight, int outputWidth, float *rInput2, int pad_size, int kernel_size, int max_displacement, int stride1, int stride2)
{
    ...
    // Pre-computing common arithmetic expressions
    ...
    // Reducing redundant calculations, using pre-computed expressions
    ...
    // Use shared memory to enhance memory bandwidth
    __shared__ float shared_prod_sum[CUDA_NUM_THREADS];
    shared_prod_sum[tch_off] = 0;

    const float nelems = kernel_size * kernel_size * nInputChannels;
    ...
    for (int tc = tch_off; tc < nOutputChannels; tc += CUDA_NUM_THREADS) {
        ...
        const float val2 = rInput2[indx2];

        for (int j = ymin; j <= ymax; ++j) {
            for (int i = xmin; i <= xmax; ++i) {
                int tindx = n * tdimcyx + tc * tdimyx + j * tdimx + i;
                shared_prod_sum[tch_off] += gradOutput[tindx] * val2;
            }
        }
    }
    ...
    // Serial reduction within thread block
    if (tch_off == 0) {
        float reduce_sum = 0;
        #pragma unroll
        for (int idx = 0; idx < CUDA_NUM_THREADS; idx++) {
            reduce_sum += shared_prod_sum[idx];
        }
