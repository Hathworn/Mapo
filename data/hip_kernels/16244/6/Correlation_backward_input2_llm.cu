...
__global__ void Correlation_backward_input2(int item, float *gradInput2, int nInputChannels, int inputHeight, int inputWidth, float *gradOutput, int nOutputChannels, int outputHeight, int outputWidth, float *rInput1, int pad_size, int kernel_size, int max_displacement, int stride1, int stride2)
{
    ...
    // Cache boundary calculations to avoid repeated computation
    int disp_idx_x = tch_off % displacement_size - displacement_rad;
    int disp_idx_y = tch_off / displacement_size - displacement_rad;

    int i2 = disp_idx_x * stride2;
    int j2 = disp_idx_y * stride2;
    ...
    if (xmax >= 0 && ymax >= 0 && xmin < outputWidth && ymin < outputHeight && xmin <= xmax && ymin <= ymax) {
        ...
        for (int j = ymin; j <= ymax; ++j) {
            for (int i = xmin; i <= xmax; ++i) {
                int tindx = n * tdimcyx + tch_off * tdimyx + j * tdimx + i;
                prod_sum[tch_off] += gradOutput[tindx] * val1;
            }
        }
    }

    // Use reduction pattern to avoid thread divergence
    __syncthreads();
