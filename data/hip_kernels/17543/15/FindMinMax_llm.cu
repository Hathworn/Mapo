#include "hip/hip_runtime.h"
#include "includes.h"

__global__ void FindMinMax(float *d_MinMax, float *d_Data, int width, int pitch, int height)
{
    __shared__ float minvals[128];
    __shared__ float maxvals[128];

    const int tx = threadIdx.x;
    const int x = blockIdx.x * 128 + tx;
    const int y = blockIdx.y * 16;
    int p = y * pitch + x;
    
    // Initialize minvals and maxvals only if x < width
    if (x < width) {
        float val = d_Data[p];
        minvals[tx] = val;
        maxvals[tx] = val;
        
        // Iterate over the height dimension
        for (int ty = 1; ty < 16; ty++) {
            p += pitch;
            if (p < width * pitch * height) { // Check p to avoid out-of-bounds access
                float val = d_Data[p];
                // Update min and max values
                if (val < minvals[tx])
                    minvals[tx] = val;
                if (val > maxvals[tx])
                    maxvals[tx] = val;
            }
        }
    }

    __syncthreads();

    // Reduce min and max in shared memory
    for (int d = b / 2; d > 0; d >>= 1) { // Change reduction to half each step
        if (tx < d) {
            if (minvals[tx + d] < minvals[tx])
                minvals[tx] = minvals[tx + d];
            if (maxvals[tx + d] > maxvals[tx])
                maxvals[tx] = maxvals[tx + d];
        }
        __syncthreads();
    }

    // Write result for this block to global memory
    if (tx == 0) {
        int ptr = 2 * (gridDim.x * blockIdx.y + blockIdx.x);
        d_MinMax[ptr + 0] = minvals[0];
        d_MinMax[ptr + 1] = maxvals[0];
    }
}