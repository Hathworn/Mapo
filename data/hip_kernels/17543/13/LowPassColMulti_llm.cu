...
__global__ void LowPassColMulti(float *d_Result, float *d_Data, int width, int pitch, int height)
{
    ...
    if (colStart < width) {
        ...
        // Optimize global memory access pattern by using coalesced reads
    ...
    if (colStart < width) {
        ...
        // Reduce the arithmetic operations by using common subexpression elimination
        float sumKernel = kernel[0] * (sdata[0*CONVCOL_W] + sdata[8*CONVCOL_W])
                        + kernel[1] * (sdata[1*CONVCOL_W] + sdata[7*CONVCOL_W])
                        + kernel[2] * (sdata[2*CONVCOL_W] + sdata[6*CONVCOL_W])
                        + kernel[3] * (sdata[3*CONVCOL_W] + sdata[5*CONVCOL_W])
                        + kernel[4] * sdata[4*CONVCOL_W];

        for (int y = miny + ty; y <= maxy; y += blockDim.y) {
            d_Result[gPos] = sumKernel;
            sdata += sStep;
            gPos += gStep;
        }
    }
