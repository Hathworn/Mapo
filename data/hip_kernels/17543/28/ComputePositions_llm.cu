...
__global__ void ComputePositions(float *g_Data1, float *g_Data2, float *g_Data3, int *d_Ptrs, float *d_Sift, int numPts, int maxPts, int w, int h)
{
    ...
    // Use registers mostly instead of array indexing for better performance
    ...
    if (fabs(det) < 1e-6f) // Avoid division by extremely small numbers
        return;
    ...
    // Store results
    int w_div = p / w;
    float exp_val = exp2f(pds * d_ConstantB[0]);
    d_Sift[i + 0 * maxPts] = (p % w) + pdx;
    d_Sift[i + 1 * maxPts] = w_div + pdy;
    d_Sift[i + 2 * maxPts] = d_ConstantA[0] * exp_val;
    d_Sift[i + 3 * maxPts] = val0 + dval;
    
    // Tra/det calculation
    float tra = dxx + dyy;
    float new_det = dxx * dyy - dxy * dxy;
    d_Sift[i + 4 * maxPts] = __fdividef(tra * tra, new_det);
}