...
__global__ void Find3DMinMax(int *d_Result, float *d_Data1, float *d_Data2, float *d_Data3, int width, int pitch, int height) {
    ...
    for (int y = 0; y < 34; y++) {
        ...
        if (y > 1 && tx < memWid) {
            ...
            ymin1[tx] = min1;
            ymin2[tx] = fminf(fminf(min1, fminf(data2[memPos0], data2[memPos1])), fminf(data2[memPos2], min3));
            ymin3[tx] = min3;
            ymax1[tx] = max1;
            ymax2[tx] = fmaxf(fmaxf(max1, fmaxf(data2[memPos0], data2[memPos1])), fmaxf(data2[memPos2], max3));
            ymax3[tx] = max3;
        }
        __syncthreads();

        if (y > 1 && tx < MINMAX_W && xWritePos <= xEndClamped) {
            float minv = fminf(fminf(fminf(ymin2[tx], ymin1[tx + 1]), ymin3[tx + 1]), data2[memPos0 + 1]);
            minv = fminf(fminf(minv, data2[memPos2 + 1]), d_Threshold[1]);
            float maxv = fmaxf(fmaxf(fmaxf(ymax2[tx], ymax1[tx + 1]), ymax3[tx + 1]), data2[memPos0 + 1]);
            maxv = fmaxf(fmaxf(maxv, data2[memPos2 + 1]), d_Threshold[0]);
