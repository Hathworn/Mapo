...
__global__ void Find3DMinMax(int *d_Result, float *d_Data1, float *d_Data2, float *d_Data3, int width, int height) {
    ...
    for (int y = 0; y < 34; y++) {
        ...
        // Data cache loading with boundary checks
        if (tx >= (WARP_SIZE - 1)) {
        ...
        if (y > 1 && tx < memWid) {
            // Calculate min and max for each data set
        ...
        if (y > 1 && tx < MINMAX_SIZE && xWritePos <= xEndClamped) {
            // Find the minimum and maximum values with additional reference values
            ...
            // Determine if current point is outside min/max bounds
