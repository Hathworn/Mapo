...
__global__ void upsample_corr_kernel( int *curr_corr, int *next_corr, int curr_h, int curr_w, int next_h, int next_w )
{
    ...
    if (id < next_h * next_w) {
        // Precompute divisions to reduce calculations
        float w_ratio_inv = (float)curr_w / (float)next_w;
        float h_ratio_inv = (float)curr_h / (float)next_h;
        
        int next_x = id % next_w, next_y = id / next_w;
        
        int curr_x = (next_x + 0.5) * w_ratio_inv;
        int curr_y = (next_y + 0.5) * h_ratio_inv;
        ...
        int next_x2 = next_x + (curr_x2 - curr_x) * w_ratio_inv + 0.5;
        int next_y2 = next_y + (curr_y2 - curr_y) * h_ratio_inv + 0.5;
