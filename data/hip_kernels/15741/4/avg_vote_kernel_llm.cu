...
__global__ void avg_vote_kernel( float *A, float *B, int *corrAB, int patch, int c, int h, int w )
{
    ...
    int size = h * w;
    int radius = patch / 2;
    if (_id >= c * size) return; // Early exit if out of bounds
    ...
    // Iterate using 1D loop to possibly improve shared memory usage patterns and reduce control divergence
    for (int offset = -radius; offset <= radius; ++offset) {
        for (int dy = -radius; dy <= radius; ++dy) {
            ...
            if ((new_x1 < 0) || (new_x1 >= w) || (new_y1 < 0) || (new_y1 >= h)) continue;
            ...
            // Check if the coordinates are within bounds
            if ((new_x2 < 0) || (new_x2 >= w) || (new_y2 < 0) || (new_y2 >= h)) continue;
