...
__device__ float setBoundaryCondition(int tstep, float tau, float Vcast, float *hPop, int Section, float *ccml)
{
    ...
    {
        if (zposition >= ccml[i] && zposition <= ccml[i + 1])
        {
...
__global__ void solvePDEKernel(float *hPop, float *T_Last, float *T_New, float *T_Surface, float Tw, float lamda, float pho, float ce, int ny, float dy, int nx, float dx, float tau, int tnpts, int tstep, float Vcast, int Section, float *ccml)
{
    ...
    {
         // Optimize boundary condition calculation with a switch-case structure
        switch ((i == 0) << 0 | (i == (nx - 1)) << 1 | (j == 0) << 2 | (j == (ny - 1)) << 3) 
        {
            case 1: // i == 0 && j == 0
                T_Right = T_Last[tis + L];
            ...
            case 8: // i == 0 && j == (ny - 1)
                ...
                T_Down = T_Last[tis - 1];
                break;
            case 10: // i == (nx - 1) && j == (ny - 1)
                T_Right = T_Last[tis - L] - 2 * dx * h * (T_Last[tis] - Tw) / lamda;
                T_Left = T_Last[tis - L];
                T_Up = T_Last[tis - 1] - 2 * dx * h * (T_Last[tis] - Tw) / lamda;
                T_Down = T_Last[tis - 1];
                break;
            case 4: // i == 0 && j != 0 && j != (ny - 1)
                T_Right = T_Last[tis + L];
                T_Left = T_Last[tis + L] - 2 * dx * h * (T_Last[tis] - Tw) / lamda;
                T_Up = T_Last[tis + 1];
                T_Down = T_Last[tis - 1];
                break;
            case 5: // i != 0 && i != (nx - 1) && j == 0
                T_Right = T_Last[tis + L];
                T_Left = T_Last[tis - L];
                T_Up = T_Last[tis + 1];
                T_Down = T_Last[tis + 1] - 2 * dx * h * (T_Last[tis] - Tw) / lamda;
                break;
            case 6: // i == (nx - 1) && j != 0 && j != (ny - 1)
                T_Right = T_Last[tis - L] - 2 * dx * h * (T_Last[tis] - Tw) / lamda;
                T_Left = T_Last[tis - L];
                T_Up = T_Last[tis + 1];
                T_Down = T_Last[tis - 1];
                break;
            case 12: // i != 0 && i != (nx - 1) && j == (ny - 1)
                T_Right = T_Last[tis + L];
                T_Left = T_Last[tis - L];
                T_Up = T_Last[tis - 1] - 2 * dx * h * (T_Last[tis] - Tw) / lamda;
                T_Down = T_Last[tis - 1];
