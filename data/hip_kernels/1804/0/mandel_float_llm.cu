...
__global__ void mandel_float(uchar4 *ptr, int max_w, int max_h, int w, int h, float cx, float cy, float zoom, int iter_mult) {
    ...
    if (x < w && y < h) {
        ...
        __shared__ unsigned char local_viridis[256][3];
        if (threadIdx.x < 256) {
            local_viridis[threadIdx.x][0] = viridis[threadIdx.x][0];
            local_viridis[threadIdx.x][1] = viridis[threadIdx.x][1];
            local_viridis[threadIdx.x][2] = viridis[threadIdx.x][2];
        }
        __syncthreads();
        ...
        for (unsigned int n = 0; n < max_iterations; ++n) {
            ...
            if (z_re2 + z_im2 > 4.0f) {
                ...
                bgra.x = local_viridis[nn][0];
                bgra.y = local_viridis[nn][1];
                bgra.z = local_viridis[nn][2];
                break;
            }
