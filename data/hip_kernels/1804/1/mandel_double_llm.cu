...
__global__ void mandel_double(uchar4 *ptr, int max_w, int max_h, int w, int h, double cx, double cy, double zoom, int iter_mult)
{
    ...
    // Check if indices are within image boundaries
    if (x < w && y < h) {
        uchar4 bgra = {0x0, 0x0, 0x0, 0x0};  // inside = black
        unsigned int max_iterations = iter_mult * 256;
        zoom = 1.0 / zoom; // Use double precision for zoom variable
        double zoomX = zoom;
        ...
        // Precalculate frequently used values to save operations
        ...
        // Loop to calculate Mandelbrot set membership
        for (unsigned int n = 0; n < max_iterations; ++n) {
            double z_re2 = z_re * z_re, z_im2 = z_im * z_im;
            if (z_re2 + z_im2 > 4.0) { // Use double precision for comparison
                int nn = n & 0xFF;
            ...
            // Update z_im and z_re for next iteration
    ...
    else if (x < max_w && y < max_h) { // Handle border cases
