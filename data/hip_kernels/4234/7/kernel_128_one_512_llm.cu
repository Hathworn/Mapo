...
__global__ void kernel_128_one_512(float *A, float *B, float *bnBias, float *bnScale, float *C) {
    ...
    // Use loop unrolling for weights loading to reduce loop overhead
    for (int k = 0; k < 128; k += 64) {
        ...
        // Use loop unrolling for output computation to improve performance
    ...
    // Optimize the final output computation and storage
