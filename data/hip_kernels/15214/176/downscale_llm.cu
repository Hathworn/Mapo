...
__global__ void downscale(float *gradInput_data, const float *gradOutput_data, long no_elements, int scale_factor, int d1, int d2, int d3) {
    ...
    long gridStride = blockDim.x * gridDim.x;
    
    // Unroll loops for potential performance improvement
    for (; ii < no_elements; ii += gridStride) {
        int base_idx = translate_idx_inv(ii, d1, d2, d3, scale_factor, 0, 0);
        
        float temp = 0.0f;
        for (int i = 0; i < scale_factor; i++) {
            for (int j = 0; j < scale_factor; j++) {
                int ipidx = base_idx + translate_idx_inv(ii, 0, 0, 0, 1, i, j);
                temp += gradOutput_data[ipidx];
            }
        }
        gradInput_data[ii] += temp;
    }
}