...
__global__ void funct(int n, int k, float *x, int *qx) {
    int ix = blockIdx.x * blockDim.x + threadIdx.x; // Calculate global thread index
    if (ix < n * n) { // Boundary check to prevent out-of-bounds access
        int j = ix % n; // Optimize by using modulo for 'j'
