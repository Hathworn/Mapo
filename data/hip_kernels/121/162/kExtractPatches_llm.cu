...
__global__ void kExtractPatches(float* images, float* patches, float* indices, float* width_offset, float* height_offset, int num_images, int img_width, int img_height, int patch_width, int patch_height, int num_colors) {
    ...
    for (unsigned long i = idx; i < total_pixels; i += numThreads) {
        ...
        // Source row and column calculation using offset optimization
        source_row = __float2int_rn(height_offset[image_id]) + dest_row;
        source_col = __float2int_rn(width_offset[image_id]) + dest_col;
        
        // Optimized position calculation order
        pos = source_col + img_width * (source_row + img_height * (color + num_colors * __float2int_rn(indices[image_id])));
