...
__global__ void daxpy_matrix_vector(double* A, double* B, double alpha, double* ret, int rlenA, int clenA, int rlenB, int clenB) {
    ...
    // Guard clause to ensure valid matrix indices
    if(ix >= rlenA || iy >= clenA) return;
    
    int index = ix * clenA + iy;
    if(rlenB == 1) {
        // Use the same column value of B for each element addition
        ret[index] = A[index] + alpha * B[iy];
    } else {
        // Use the same row value of B for each element addition
