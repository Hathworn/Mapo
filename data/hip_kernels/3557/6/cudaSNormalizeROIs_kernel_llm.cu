...
__global__ void cudaSNormalizeROIs_kernel(unsigned int inputSizeX, unsigned int inputSizeY, unsigned int nbProposals,
                                          unsigned int batchSize, unsigned int scoreIdx, unsigned int nbCls,
                                          unsigned int maxParts, unsigned int maxTemplates, bool keepMax,
                                          bool generateParts, bool generateTemplates, const float normX,
                                          const float normY, const float* means, const float* std,
                                          const unsigned int* numPartsPerClass, const unsigned int* numTemplatesPerClass,
                                          const float* ROIRef, const float* ROIEst, const float* ValuesEst,
                                          const float* partsEst, const float* partsVisibilityEst,
                                          const float* templatesEst, float* outputs, int* argMax,
                                          float* partsPrediction, float* partsVisibilityPrediction,
                                          float* templatesPrediction, float scoreThreshold)
{
    ...
    if(index < nbProposals) {
        ...
        if(keepMax) {
            ...
            // Optimize: Unroll loops where beneficial
        ...
        // Optimize: Improve memory access patterns by restructuring calculations
        for(unsigned int clsIdx = indexMin; clsIdx < indexMax; ++clsIdx) {
            ...
            // Optimize: Consolidate calculations
            float expWbbEst = exp(wbbEst);
            float expHbbEst = exp(hbbEst);
            float x = xbbEst * wbbRef + xbbRef + wbbRef / 2.0 - (wbbRef / 2.0) * expWbbEst;
            float y = ybbEst * hbbRef + ybbRef + hbbRef / 2.0 - (hbbRef / 2.0) * expHbbEst;
            float w = wbbRef * expWbbEst;
            float h = hbbRef * expHbbEst;
            
            // Clip values
            // Optimize: Use intrinsic functions for conditional assignments
            if (x < 0.0) {
                w += x;
                x = 0.0;
            }
            ...
            if (ValuesEst[valEstIdx] >= scoreThreshold) {
                ...
                // Optimize: Simplify part/template prediction logic
                if(generateParts) {
                    ...
                    for(unsigned int part = 0; part < numPartsPerClass[clsIdx]; ++part) {
                        const unsigned int inPartIdx = batchPos * 2 * proposalPartIdx + index * 2 * proposalPartIdx + partsIdx * 2 + part * 2;
                        const unsigned int outPartIdx = batchPos * 2 * maxParts * nbCls + index * 2 * maxParts * nbCls + clsIdx * 2 * maxParts + part * 2;
                        ...
                        const unsigned int inPartVisibilityIdx = batchPos * 4 * proposalPartIdx + index * 4 * proposalPartIdx + partsIdx + part;
                        const unsigned int outPartVisibilityIdx = batchPos * maxParts * nbCls + index * maxParts * nbCls + clsIdx * maxParts + part;

                        float idxMax = 0.0;
                        float valueMax = partsVisibilityEst[inPartVisibilityIdx];

                        for(unsigned int v = 1; v < 4; ++v) {
                            if(partsVisibilityEst[v * proposalPartIdx + inPartVisibilityIdx] > valueMax) {
                                idxMax = (float) v;
                ...
                if(generateTemplates) {
                    ...
                    for(unsigned int tpl = 0; tpl < numTemplatesPerClass[clsIdx]; ++tpl) {
                        const unsigned int inTemplateIdx = batchPos * 3 * proposalTemplateIdx + index * 3 * proposalTemplateIdx + templatesIdx * 3 + tpl * 3;
                        const unsigned int outTemplateIdx = batchPos * 3 * maxTemplates * nbCls + index * 3 * maxTemplates * nbCls + clsIdx * 3 * maxTemplates + tpl * 3;
