#include <stdbool.h>
#include <stdio.h>
#include <string.h>
#include <getopt.h>
#include <stdlib.h>
#include <sys/time.h>
#include <hip/hip_runtime.h>
#include "cudaSGatherRP_kernel_llm.cu"
#include <chrono>
#include <iostream>
using namespace std;
using namespace std::chrono;
int main(int argc, char **argv) {
hipSetDevice(0);

    int XSIZE = 512; 
    int YSIZE = 512;
    int BLOCKX = 16;
    int BLOCKY = 16;

    if (argc > 1) XSIZE = atoi(argv[1]);
    if (argc > 2) YSIZE = atoi(argv[2]);
    if (argc > 3) BLOCKX = atoi(argv[3]);
    if (argc > 4) BLOCKY = atoi(argv[4]);
    unsigned int inputSizeX = 1;
unsigned int inputSizeY = 1;
unsigned int nbAnchors = 1;
unsigned int batchSize = 1;
const float *inputs = NULL;
hipMalloc(&inputs, XSIZE*YSIZE);
const float *i = NULL;
hipMalloc(&i, XSIZE*YSIZE);
const float *j = NULL;
hipMalloc(&j, XSIZE*YSIZE);
const float *k = NULL;
hipMalloc(&k, XSIZE*YSIZE);
const float *b = NULL;
hipMalloc(&b, XSIZE*YSIZE);
const int *mask = NULL;
hipMalloc(&mask, XSIZE*YSIZE);
float *outputs = NULL;
hipMalloc(&outputs, XSIZE*YSIZE);
const unsigned int topN = 1;
const unsigned int nbProposals = 1;
int iXSIZE= XSIZE;
int iYSIZE= YSIZE;
while(iXSIZE%BLOCKX!=0) {
iXSIZE++;
}
while(iYSIZE%BLOCKY!=0) {
iYSIZE++;
}
dim3 gridBlock(iXSIZE/BLOCKX, iYSIZE/BLOCKY);
dim3 threadBlock(BLOCKX, BLOCKY);
hipFree(0);
cudaSGatherRP_kernel<<<gridBlock, threadBlock>>>(inputSizeX,inputSizeY,nbAnchors,batchSize,inputs,i,j,k,b,mask,outputs,topN,nbProposals);
hipDeviceSynchronize();
for (int loop_counter = 0; loop_counter < 5; ++loop_counter) {
cudaSGatherRP_kernel<<<gridBlock, threadBlock>>>(inputSizeX,inputSizeY,nbAnchors,batchSize,inputs,i,j,k,b,mask,outputs,topN,nbProposals);
}
hipDeviceSynchronize();
auto start = steady_clock::now();
for (int loop_counter = 0; loop_counter < 5; loop_counter++) {
cudaSGatherRP_kernel<<<gridBlock, threadBlock>>>(inputSizeX,inputSizeY,nbAnchors,batchSize,inputs,i,j,k,b,mask,outputs,topN,nbProposals);
}
hipDeviceSynchronize();
auto end = steady_clock::now();
auto usecs = duration_cast<duration<float, microseconds::period>>(end - start);
cout << '[' << usecs.count() << ',' << '(' << BLOCKX << ',' << BLOCKY << ')' << ',' << '(' << XSIZE << ',' << YSIZE << ')' << ']' << endl;
}