...
__global__ void apply_lifter_and_floor_energy(int num_frames, int num_cols, float cepstral_lifter, bool use_energy, float energy_floor, float *log_energy, float *lifter_coeffs, float *features, int32_t ldf) {
    ...
    // Only apply lifter coefficients if necessary and valid
    if (cepstral_lifter != 0.0f) {
        for (int c = thread_id; c < num_cols; c += blockDim.x) {  // Use blockDim.x for thread block limit
            feats[c] *= lifter_coeffs[c];
        }
    }

    // Use __syncthreads() to ensure all threads finished processing lifter coefficients
    __syncthreads();

    // Only the first thread applies the energy floor
