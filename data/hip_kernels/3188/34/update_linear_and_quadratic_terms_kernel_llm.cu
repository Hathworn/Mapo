...
__global__ void update_linear_and_quadratic_terms_kernel(int32_t n, float old_num_frames, float prior_offset, float* cur_tot_weight, int32_t max_count, float* quadratic, float* linear) {
    ...
    // Use one-dimensional block and grid for better utilization of the GPU
    for (int32_t i = blockIdx.x * blockDim.x + threadIdx.x; i < n;
        i += blockDim.x * gridDim.x) {
        int32_t diag_idx = ((i + 1) * (i + 2) / 2) - 1;
        atomicAdd(&quadratic[diag_idx], prior_scale_change); // Use atomicAdd for thread-safe updates
    }

    // Use only one thread to update linear[0] to prevent race condition
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        atomicAdd(&linear[0], prior_offset * prior_scale_change); // Use atomicAdd for thread-safe updates
    }
}