...
{
    int i = blockIdx.x * POSBLK_SIZE + threadIdx.x; // Simplify thread index calculation
    if (i >= numPts) return;

    int p = d_Ptrs[i];

    // Preload memory access to reduce redundant global memory reads
    ...
    // Compute gradients and second order derivatives
    ...
    // Compute inverse of Hessian determinant components
    ...
    float idet = __fdividef(1.0f, det); // Use intrinsic function for division
    ...
    // Clamp position offsets if they are out of bounds
    ...
    // Compute final position and feature descriptor
    ...
    // Compute trace and determinant for contrast measure
    float tra = dxx + dyy;
    det = dxx * dyy - dxy * dxy;
