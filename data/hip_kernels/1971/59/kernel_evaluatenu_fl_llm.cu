...
__global__ void kernel_evaluatenu_fl(int Nd, float qsum, float *q, float deltanu, float nulow) {
    ...
    if (tid < Nd) {
        ...
        float dgm_half = digamma_fl(thisnu * 0.5f);    // Combine the repetitive digamma_fl computation
        float dgm_half_plus = digamma_fl(thisnu * 0.5f + 0.5f);
        q[tid] = dgm_half_plus - logf((thisnu + 1.0f) * 0.5f); // psi((nu+1)/2) - log((nu+1)/2)
        q[tid] += -dgm_half + logf(thisnu * 0.5f);     // -psi((nu)/2) + log((nu)/2)
        q[tid] += -qsum + 1.0f;                        // Adjust qsum related computation
    }
}