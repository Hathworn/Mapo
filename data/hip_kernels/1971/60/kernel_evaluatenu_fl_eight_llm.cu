...
__global__ void kernel_evaluatenu_fl_eight(int Nd, float qsum, float *q, float deltanu, float nulow, float nu0) {
    ...
    // Each block calculates psi((nu+8)/2)-log((nu+8)/2)
    __shared__ float dgm0; // Use shared memory for dgm0 to reduce redundant calculations
    if (threadIdx.x == 0) {
        dgm0 = digamma_fl(nu0 * 0.5f + 1.0f);
        dgm0 -= logf((nu0 + 2.0f) * 0.5f); // psi((nu0+8)/2)-log((nu0+8)/2)
    }
    __syncthreads();
