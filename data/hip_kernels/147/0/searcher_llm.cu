...
__global__ void searcher(const int* M, int* res, size_t* itersNum) {
    ...
    // Initialize counters vector with unrolled pragma
    int counters[COUNTERS];
    #pragma unroll
    for (int i = 0; i < COUNTERS; i++)
        counters[i] = 1;

    // Initialize counters based on selected part
    counters[0] = 25;
    counters[1] = 5;
    counters[2] = 1;
    counters[3] = 3;
    ...
    while (1) {
        ...
        // Check if search in the selected part is finished
        ...
        for (int i = current; i <= COUNTERS; i++) {
            if (uniq(M, i, counters) == 1) {
                ...
                // Loop to update counters if overflow occurs
        ...
        if (current == COUNTERS && uniq(M, current, counters) == 0) {
            // Store result and iteration count
            #pragma unroll
            for (int i = 0; i < COUNTERS; i++)
