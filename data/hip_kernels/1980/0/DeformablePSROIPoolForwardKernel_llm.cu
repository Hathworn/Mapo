...
// Compute [start, end) interval for spatial sampling
...
double trans_x = no_trans ? 0.0 : bottom_trans[(((n * num_classes + class_id) * 2) * part_size + part_h) * part_size + part_w] * trans_std;
double trans_y = no_trans ? 0.0 : bottom_trans[(((n * num_classes + class_id) * 2 + 1) * part_size + part_h) * part_size + part_w] * trans_std;
...
double sum = 0.0;
int count = 0;
int gw = min(max(floor((double)(pw)*group_size / pooled_width), 0), group_size - 1);
int gh = min(max(floor((double)(ph)*group_size / pooled_height), 0), group_size - 1);
...
top_data[index] = count == 0 ? 0.0 : sum / count;
