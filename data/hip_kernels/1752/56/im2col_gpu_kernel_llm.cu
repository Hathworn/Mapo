...
__global__ void im2col_gpu_kernel(const int n, const float* data_im, const int height, const int width, const int ksize, const int pad, const int stride, const int height_col, const int width_col, float *data_col) {
    ...
    // Loop increment set to blockDim.x * gridDim.x for better parallel execution.
    for (; index < n; index += blockDim.x * gridDim.x) {
        ...
        // Calculate data_col_ptr and data_im_ptr once per outer loop iteration.
        ...
        for (int i = 0; i < ksize; ++i) {
            for (int j = 0; j < ksize; ++j) {
                ...
                // Using conditional ternary operator for concise element assignment.
                *data_col_ptr = (h >= 0 && w >= 0 && h < height && w < width) ? data_im_ptr[i * width + j] : 0;

                data_col_ptr += height_col * width_col; // Move to next position in data_col.
            }
        }
    }
}