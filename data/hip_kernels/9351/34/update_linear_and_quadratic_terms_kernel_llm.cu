...
__global__ void update_linear_and_quadratic_terms_kernel(int32_t n, float old_num_frames, float prior_offset, float* cur_tot_weight, int32_t max_count, float* quadratic, float* linear) {
    ...
    if (max_count != 0.0f) {
        // Calculate scale change based on new and old prior scales
    ...
    __shared__ float shared_prior_change;
    if (threadIdx.x == 0) {
        shared_prior_change = prior_scale_change;
    }
    __syncthreads();

    for (int32_t i = blockIdx.x * blockDim.x + threadIdx.x; i < n; i += blockDim.x * gridDim.x) {
        int32_t diag_idx = ((i + 1) * (i + 2) / 2) - 1;
        // Atomic addition to prevent race conditions in quadratic updates
        atomicAdd(&quadratic[diag_idx], shared_prior_change);
    }

    // Ensure single update by first thread of first block
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        atomicAdd(&linear[0], prior_offset * shared_prior_change);
    }
}