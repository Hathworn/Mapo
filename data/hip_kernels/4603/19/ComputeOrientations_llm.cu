...
__global__ void ComputeOrientations(float *g_Data, int *d_Ptrs, float *d_Orient, int maxPts, int w, int h)
{
    ...
    for (int y = 0; y < 15; y++) {
        int memPos = 16 * y + x;
        int xi = min(max(xp + x, 0), w - 1); // Clamp xi within bounds
        int yi = min(max(yp + y, 0), h - 1); // Clamp yi within bounds
        if (x >= 0 && x < 15)
    ...
    for (int y = 1; y < 14; y++) {
        ...
        if (x >= 1 && x < 14) {
            ...
            int bin = (int)(16.0f * atan2f(dy, dx) / 3.1416f + 16.5f) & 31; // Optimized angle calculation
    ...
    // Reduce histogram and smooth
    for (int stride = 8; stride >= 1; stride /= 2) {
        if (tx < 32)
            for (int y = 0; y < stride; y++)
                hist[y * 32 + tx] += hist[(y + stride * 2) * 32 + tx];
        __syncthreads();
    }

    // Weighted smooth filter
    float v = hist[32 + tx];
    if (tx < 32)
