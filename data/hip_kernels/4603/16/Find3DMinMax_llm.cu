...
__global__ void Find3DMinMax(int *d_Result, float *d_Data1, float *d_Data2, float *d_Data3, int width, int pitch, int height)
{
    ...
    // Use registers instead of shared memory for memory position indices
    ...
    for (int y = 0; y < 32 + 2; y++) {
        ...
        // Conditional to avoid unnecessary reads
        if (tx >= (WARP_SIZE - 1)) {
        ...
        if (y > 1 && tx < MINMAX_W) {
            if (xWritePos <= xEndClamped) {
                // Simplify nested fminf/fmaxf
                float minv = fminf(fminf(ymin2[tx], ymin2[tx+2]), 
                                   fminf(data2[memPos0+1], data2[memPos2+1]));
                minv = fminf(minv, d_Threshold[1]);

                float maxv = fmaxf(fmaxf(ymax2[tx], ymax2[tx+2]), 
                                   fmaxf(data2[memPos0+1], data2[memPos2+1]));
