...
__global__ void downscale(float *gradInput_data, const float *gradOutput_data, long no_elements, int scale_factor, int d1, int d2, int d3) {
    ...
    // Precompute scaled indices start
    int start_idx_x = (ii % d3) * scale_factor;
    ii /= d3;
    int start_idx_z = (ii % d2) * scale_factor;
    ii /= d2;
    int start_idx_y = (ii % d1) * scale_factor;
    ii /= d1;

    // Loop through the scale factors
    for (int i = 0; i < scale_factor; i++) {
        for (int j = 0; j < scale_factor; j++) {
            int ipidx = ((ii * d1 + start_idx_y + i) * d2 * scale_factor + start_idx_z + j) * d3 * scale_factor + start_idx_x;
