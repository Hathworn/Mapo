...
{
    // Global
    int ix = (blockDim.x - 2) * blockIdx.x + threadIdx.x; // Different indexing as we declared more blocks
    ...
    if (ix <= N && iy <= N) {
        if (i != 0 && i != (blockDim.x-1) && j != 0 && j != (blockDim.y-1)) {
            ...
            // Terminating Checks
            atomicAdd(allzeros, newGen[id] != 0);    // Check if all cells are dead
            atomicAdd(change, newGen[id] != oldGen[id]); // Check if life stayed the same
        }
    }
}