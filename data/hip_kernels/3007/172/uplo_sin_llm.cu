...
__global__ void uplo_sin (const int sd, const int unit, const int bottom, const REAL* a, const int offset_a, const int ld_a, REAL* b, const int offset_b, const int ld_b) {
    ...
    if (gid_0 < sd && gid_1 < sd) {
        ...
        if (condition) {
            // Perform the operation if the condition is true
