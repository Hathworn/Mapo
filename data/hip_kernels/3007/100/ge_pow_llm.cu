...
__global__ void ge_pow (const int sd, const int fd, const REAL* a, const int offset_a, const int ld_a, const REAL* b, const int offset_b, const int ld_b, REAL* c, const int offset_c, const int ld_c) {
    ...
    // Use a flat index for coalesced memory access
    const int index = gid_0 + gid_1 * ld_c;

    // Check the validity of thread
    if (gid_0 < sd && gid_1 < fd) {
        // Compute power and store result
        c[offset_c + index] = CAST(pow)(a[offset_a + gid_0 + gid_1 * ld_a], b[offset_b + gid_0 + gid_1 * ld_b]);
    }
}
