#include "hip/hip_runtime.h"
#include "includes.h"

__global__ void rfi_gpu_kernel(unsigned short *d_input, int nchans, int nsamp)
{
    int c = blockIdx.x * blockDim.x + threadIdx.x;
    
    if (c >= nchans) return; // Return early if channel index exceeds limit

    float stdev = 1000000.0f;
    float mean = 0.0f;
    float sum = 0.0f;
    float sum_squares = 0.0f;
    float cutoff = (4.0f * stdev);
    int count;

    // Optimize loop count calculation for early exits
    for (int out = 0; out < 4; out++) {
        sum = 0.0f;
        sum_squares = 0.0f;
        count = 0;

        for (int t = 0; t < nsamp; t++) {
            float data = (float)d_input[c * nsamp + t];
            if (data < (mean + cutoff) && data > (mean - cutoff)) {
                sum += data;
                sum_squares += (data * data);
                count++;
            }
        }
        if (count > 0) { // Prevent division by zero
            mean = sum / count;
            sum_squares = ((sum_squares / count) - (mean * mean));
            stdev = sqrtf(sum_squares); // Use sqrtf instead of sqrt for float
            cutoff = (4.0f * stdev);
        } else {
            mean = 0.0f; // Reset mean if no valid data
        }
    }

    for (int t = 0; t < nsamp - 4; t++) {
        float data = 0.0f;
        // Unroll loop for better performance
        #pragma unroll
        for (int x = 0; x < 4; x++) {
            data += (float)d_input[c * nsamp + t + x];
        }
        data *= 0.25f;
        if (data > (mean + cutoff) || data < (mean - cutoff)) {
            d_input[c * nsamp + t] = (unsigned short)mean;
        }
    }
}