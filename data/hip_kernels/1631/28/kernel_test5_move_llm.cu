...
{
    // Calculate initial pointers for each block
    ...
    // Efficient copy using pointer arithmetic within shared memory
    for (unsigned int i = 0; i < half_count; i++){
        ptr_mid[i] = ptr[i];
    }

    // Efficient vectorized move within the block
    #pragma unroll
    for (unsigned int i = 0; i < half_count - 8; i++){
        ptr[i + 8] = ptr_mid[i];
    }

    // Final moving of the last elements
    #pragma unroll
