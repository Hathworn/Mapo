...
__global__ void histogram(int* Hist, long long* arrayofkeys, long long size, int noofpartitions)
{
    ...
    int *sharedHist = (int *)&sharedpartitions[0]; // Correct offset calculation

    // Initialize shared histogram
    for (int m = thd; m < noofpartitions; m += bD)
        sharedHist[m] = 0;
    ...
    if (thdindex < size)
    {
        ...
        atomicAdd(&(sharedHist[h]), 1); // Coalesced memory access improvement
    }
    __syncthreads();

    // Aggregate into global histogram
    for (int n = thd; n < noofpartitions; n += bD)
        atomicAdd(&(Hist[n]), sharedHist[n]); // Coalesced memory access improvement
