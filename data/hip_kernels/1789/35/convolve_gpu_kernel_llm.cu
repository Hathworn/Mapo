    ...
    if (fil < n) {
        ...
        for (int chan = 0; chan < in_c; ++chan) {
            ...
            for (int f_y = 0; f_y < size; ++f_y) {
                ...
                if (input_y < 0 || input_y >= in_h) continue; // Avoid out of bounds
                
                for (int f_x = 0; f_x < size; ++f_x) {
                    int input_x = x + f_x - pad;
                    if (input_x < 0 || input_x >= in_w) continue; // Avoid out of bounds
        ...
        output[output_index] = sum; // Write sum to output array
    }
}