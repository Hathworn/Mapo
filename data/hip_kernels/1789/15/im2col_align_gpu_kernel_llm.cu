    ...
    for(; index < n; index += blockDim.x * gridDim.x) {
        ...
        for(int i = 0; i < ksize; ++i) {
            for(int j = 0; j < ksize; ++j) {
                ...
                // Check boundaries and load data into shared memory
                float val = (h >= 0 && w >= 0 && h < height && w < width) ? data_im_ptr[i * width + j] : 0;
                
                // Compute output index and update data_col
                int out_index = (channel_out + i * ksize + j) * bit_align + (index % (width_col * height_col));
