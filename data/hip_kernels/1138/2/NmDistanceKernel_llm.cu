    ...
    for (int i = blockIdx.x; i < b; i += gridDim.x) {
        for (int k2 = 0; k2 < m; k2 += batch) {
            ...
            for (int j = threadIdx.x + blockIdx.y * blockDim.x; j < n; j += blockDim.x * gridDim.y) {
                ...
                int best_i = 0;
                float best = FLT_MAX; // Initialize with max float value
                ...
                for (int k = 0; k < end_k; k++) {
                    ...
                    for (int l = 0; l < 5; l++) {
                        int idx = k * 5 + l;
                        float x2 = buf[idx + 0] - x1;
                        float y2 = buf[idx + 1] - y1;
                        float r2 = buf[idx + 2] - r1;
                        float g2 = buf[idx + 3] - g1;
                        float b2 = buf[idx + 4] - b1;
