...
__global__ void computePointHessian0(float *x, float *y, float *z, int points_num, int *valid_points, int valid_points_num, double *dh_ang, double *ph93, double *ph103, double *ph113, double *ph123, double *ph94, double *ph133, double *ph104, double *ph143, double *ph114, double *ph153, double *ph95, double *ph163, double *ph105, double *ph173, double *ph115)
{
    ...
    for (int i = id; i < valid_points_num; i += stride) {
        ...
        // Load original coordinates and convert to double
        ...
        double temp_ph103 = o_x * h_ang[0] + o_y * h_ang[1] + o_z * h_ang[2];
        double temp_ph113 = o_x * h_ang[3] + o_y * h_ang[4] + o_z * h_ang[5];
        double temp_ph133 = o_x * h_ang[6] + o_y * h_ang[7] + o_z * h_ang[8];
        double temp_ph143 = o_x * h_ang[9] + o_y * h_ang[10] + o_z * h_ang[11];
        double temp_ph163 = o_x * h_ang[12] + o_y * h_ang[13] + o_z * h_ang[14];
        double temp_ph173 = o_x * h_ang[15] + o_y * h_ang[16] + o_z * h_ang[17];
        ...
        ph113[i] = temp_ph113;
        ph123[i] = ph94[i] = 0;
        ph133[i] = ph104[i] = temp_ph133;
        ph143[i] = ph114[i] = temp_ph143;
        ph153[i] = ph95[i] = 0;
        ph163[i] = ph105[i] = temp_ph163;
        ph173[i] = ph115[i] = temp_ph173;
    }
}