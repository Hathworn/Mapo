...
__global__ void computePointHessian1(float *x, float *y, float *z, int points_num, int *valid_points, int valid_points_num, double *dh_ang, double *ph124, double *ph134, double *ph144, double *ph154, double *ph125, double *ph164, double *ph135, double *ph174, double *ph145)
{
    ...
    // Load shared memory only necessary times
    if (threadIdx.x < 18) {
    ...
    for (int i = id; i < valid_points_num; i += stride) {
        ...
        // Original coordinates casted only once
        double o_x = static_cast<double>(x[pid]);
        double o_y = static_cast<double>(y[pid]);
        double o_z = static_cast<double>(z[pid]);

        // Simplify calculations and reduce redundant computation
        double p1 = o_x * h_ang[0] + o_y * h_ang[1] + o_z * h_ang[2];
        double p2 = o_x * h_ang[3] + o_y * h_ang[4] + o_z * h_ang[5];
        double p3 = o_x * h_ang[6] + o_y * h_ang[7] + o_z * h_ang[8];
        double p4 = o_x * h_ang[9] + o_y * h_ang[10] + o_z * h_ang[11];
        double p5 = o_x * h_ang[12] + o_y * h_ang[13] + o_z * h_ang[14];
        double p6 = o_x * h_ang[15] + o_y * h_ang[16] + o_z * h_ang[17];
