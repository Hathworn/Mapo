    ...
    if (row < 6 && col < 6) {
        ...
        for (int i = id; i < valid_points_num; i += stride) {
            ...
            for (int j = start; j < end; j++) {
                ...
                if (0 <= tmp_ex && tmp_ex <= 1) { // Check within bounds
                    tmp_ex *= gauss_d1;

                    // Unroll loops manually for performance
                    double cent_diff_x = d_x - centroid_x[vid];
                    double cent_diff_y = d_y - centroid_y[vid];
                    double cent_diff_z = d_z - centroid_z[vid];

                    final_hessian += cent_diff_x * (icov00[vid] * ph0 + icov01[vid] * ph1 + icov02[vid] * ph2) * tmp_ex;
                    final_hessian += cent_diff_y * (icov10[vid] * ph0 + icov11[vid] * ph1 + icov12[vid] * ph2) * tmp_ex;
                    final_hessian += cent_diff_z * (icov20[vid] * ph0 + icov21[vid] * ph1 + icov22[vid] * ph2) * tmp_ex;
                }
            }
