...
__global__ void computePointHessian2(float *x, float *y, float *z, int points_num, int *valid_points, int valid_points_num, double *dh_ang, double *ph155, double *ph165, double *ph175)
{
    ...
    // Load dh_ang into shared memory efficiently
    if (threadIdx.x < 9) {
    ...
    for (int i = id; i < valid_points_num; i += stride) {
        ...
        // Reorder operations to minimize dependency
        double h1 = o_x * h_ang[0] + o_y * h_ang[1] + o_z * h_ang[2];
        double h2 = o_x * h_ang[3] + o_y * h_ang[4] + o_z * h_ang[5];
        double h3 = o_x * h_ang[6] + o_y * h_ang[7] + o_z * h_ang[8];
