...
{
    // Calculate global unique thread ID and the stride for the grid
    int id = threadIdx.x + blockIdx.x * blockDim.x;
    int stride = blockDim.x * gridDim.x;
    int row = blockIdx.y;
    int col = blockIdx.z;

    // Bounds check to ensure valid row and column
    if (row < 6 && col < 6) {
        // Precalculate memory offsets to improve register re-use
        ...
        // Iterate over all valid points in strides
        for (int i = id; i < valid_points_num; i += stride) {
            ...
            // Store the computed value into Hessian matrix
            h[i] = final_hessian;
        }
    }
}