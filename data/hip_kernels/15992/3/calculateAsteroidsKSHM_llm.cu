...
__global__ void calculateAsteroidsKSHM(asteroid* AST, int nAsteroids, blackHole* BH, int nBlackHoles, float dt, int bhSize) {
    extern __shared__ blackHole bhCache[];

    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int stride = gridDim.x * blockDim.x;
    
    if (threadIdx.x < nBlackHoles) {
        bhCache[threadIdx.x] = BH[threadIdx.x];
    }
    __syncthreads();
    ...
    for (int i = idx; i < nAsteroids; i += stride) {
        ...
        for (int j = 0; j < nBlackHoles; j++) {
            ...
            float distSqr = dx * dx + dy * dy + SOFTENING;  // Prevent division by zero
            float invDistCubed = rsqrtf(distSqr * distSqr * distSqr);  // Optimized inverse square root
