...
__global__ void calculateSpaceTimeKSHM(spacePoint* SPBox, int nRows, int nCols, float left, float right, float top, float bottom, blackHole* BH, int nBlackHoles, int bhSize) {
    ...
    // Each block loads the black hole data into shared memory once
    ...
    if (i < nRows && j < nCols) {
        ...
        int idx = i * nCols + j; // Corrected index calculation

        float g = 0.0f;
        for (int b = 0; b < nBlackHoles; b++) {
            float dx = x - bhCache[b].x;
            float dy = y - bhCache[b].y;
            float distSqr = dx * dx + dy * dy + SOFTENING; // Use SOFTENING to avoid division by zero

            float invDist = rsqrtf(distSqr); // Use reciprocal square root for efficiency
