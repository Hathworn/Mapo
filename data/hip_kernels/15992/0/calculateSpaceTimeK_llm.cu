...
__global__ void calculateSpaceTimeK(spacePoint* SPBox, int nRows, int nCols, float left, float right, float top, float bottom, blackHole* BH, int nBlackHoles) {
    ...
    // Limit computation to valid thread IDs within bounds
    if (i < nRows && j < nCols) {
        float x = (i / (float) nRows * (right-left)) + left;
        float y = (j / (float) nCols * (top-bottom)) + bottom;
        int idx = i * nCols + j;  // Correct index calculation (fixing order of multiplication)

        float g = 0.0f;
        for (int b = 0; b < nBlackHoles; b++) {
            float dx = x - BH[b].x;
            float dy = y - BH[b].y;
            float distSqr = dx * dx + dy * dy; // Avoid sqrt and power function for performance

            float invDist = 1.0f / (distSqr + SOFTENING); // Use SOFTENING to avoid division by zero
            g += (BH[b].g * invDist);
        }
