...
__device__ float3 hsv_to_rgb_kernel(float3 hsv)
{
    ...
    if (s == 0) {
        ...
        switch (index) { // Use switch case for better readability and efficiency
    ...
    // Apply clamping outside the conditional for efficiency
...
__device__ float3 rgb_to_hsv_kernel(float3 rgb)
{
    ...
    if (max == 0.0f){
        ...
        if (h < 0.0f) h += 6.0f; // Normalized hue value to always be positive
    }
...
__global__ void levels_image_kernel(float *image, float *rand, int batch, int w, int h, int train, float saturation, float exposure, float translate, float scale, float shift)
{
    ...
    if(id >= size) return; // Prevent out-of-bounds memory access
    ...
    // Unpack random shifts and modifications
    float rshift = rand[0];
    float gshift = rand[1];
    float bshift = rand[2];
    ...
    // Update pixel values and apply scaling, translation, and random shift
