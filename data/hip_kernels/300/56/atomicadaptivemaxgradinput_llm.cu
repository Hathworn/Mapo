...
{
    // Compute the unique output id
    int o = blockIdx.x;
    int i = o;

    // Compute the starting indices and steps
    int xx_start = threadIdx.x;
    int xx_step = blockDim.x;
    int yy_start = blockDim.y * blockIdx.y + threadIdx.y;
    int yy_step = blockDim.y * gridDim.y;

    // Adjust the input/output pointers to the specific plane
    gradOutput += o * output_w * output_h;
    gradInput += i * input_w * input_h;
    indices_x += o * output_w * output_h;
    indices_y += o * output_w * output_h;

    // Loop through the output dimensions with ensured coalesced memory access
    for (int yy = yy_start; yy < output_h; yy += yy_step) {
        ...
        for (int xx = xx_start; xx < output_w; xx += xx_step) {
            ...
            // Reduce warp-level divergence by reordering operations
