...
__global__ void needle_cuda_shared_2(int* reference, int* matrix_cuda, int cols, int penalty, int i, int block_width) {
    ...
    // Load reference matrix into shared memory
    for (int ty = 0; ty < BLOCK_SIZE; ty++) {
        ref[ty][tx] = reference[index + cols * ty];
    }
    __syncthreads();

    // Initialize shared memory border values
    if (tx == 0) {
    ...
    // Perform first half of the computation
    ...
    // Perform second half of the computation
