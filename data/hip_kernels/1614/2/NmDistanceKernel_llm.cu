    ...
    for (int i = blockIdx.x; i < b; i += gridDim.x) {
        for (int k2 = 0; k2 < m; k2 += batch) {
            ...
            for (int j = threadIdx.x + blockIdx.y * blockDim.x; j < n; j += blockDim.x * gridDim.y) {
                ...
                int best_i = 0;
                float best = FLT_MAX;  // Use FLT_MAX for initialization

                int end_ka = end_k - (end_k & 3);  // Optimization for unrolling
                for (int k = 0; k < end_ka; k += 4) {
                    #pragma unroll 4  // Unroll loop for performance
                    for (int u = 0; u < 4; ++u) {
                        float x2 = buf[(k + u) * 5 + 0] - x1;
                        float y2 = buf[(k + u) * 5 + 1] - y1;
                        float r2 = buf[(k + u) * 5 + 2] - r1;
                        float g2 = buf[(k + u) * 5 + 3] - g1;
                        float b2 = buf[(k + u) * 5 + 4] - b1;
