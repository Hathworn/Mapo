...
__global__ void copy_kernel_tobuf(char *dest, char *src, int rx_s, int rx_e, int ry_s, int ry_e, int rz_s, int rz_e, int x_step, int y_step, int z_step, int size_x, int size_y, int size_z, int buf_strides_x, int buf_strides_y, int buf_strides_z, int type_size, int dim, int OPS_soa) {
    ...
    // Combine conditions with logical operations for better readability
    bool within_x_bounds = (x_step == 1 ? idx_x < rx_e : idx_x > rx_e);
    bool within_y_bounds = (y_step == 1 ? idx_y < ry_e : idx_y > ry_e);
    bool within_z_bounds = (z_step == 1 ? idx_z < rz_e : idx_z > rz_e);

    if (within_x_bounds && within_y_bounds && within_z_bounds) {
        // Precompute stride and offset for source and destination pointers
        int src_offset = (idx_z * size_x * size_y + idx_y * size_x + idx_x) * type_size;
        src += OPS_soa ? src_offset : src_offset * dim;
        
        int dest_offset = ((idx_z - rz_s) * z_step * buf_strides_z +
                          (idx_y - ry_s) * y_step * buf_strides_y +
                          (idx_x - rx_s) * x_step * buf_strides_x) * 
                          type_size * dim;
        dest += dest_offset;
        
        // Optimize memory copy for loop unrolling
        for (int d = 0; d < dim; d++) {
            memcpy(dest + d * type_size, src, type_size);
            src += OPS_soa ? size_x * size_y * size_z * type_size : type_size;
        }
    }
}