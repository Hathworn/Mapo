...
__global__ void ComputeOrientations(float *g_Data, int *d_Ptrs, float *d_Orient, int maxPts, int w, int h)
{
    ...
    for (int y = 0; y < 15; y++) {
        ...
        xi = min(max(xi, 0), w - 1);  // Clamp xi to [0, w-1]
        yi = min(max(yi, 0), h - 1);  // Clamp yi to [0, h-1]
    ...
    for (int y = 1; y < 14; y++) {
        ...
        if (x >= 1 && x < 14) {
            ...
            int bin = __float2int_rd(16.0f * atan2f(dy, dx) / 3.1416f + 16.5f) & 31;  // Ensure bin is within [0,31]
    ...
    if (tx == 0) {
        ...
        for (int i = 1; i <= 31; ++i)
            hist[i + 32] = 6 * hist[i] + 4 * (hist[(i + 1) & 31] + hist[(i - 1) & 31]) + (hist[(i + 2) & 31] + hist[(i - 2) & 31]);
    }
    __syncthreads();
