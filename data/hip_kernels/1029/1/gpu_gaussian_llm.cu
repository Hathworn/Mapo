...
__global__ void gpu_gaussian(int width, int height, float *image, float *image_out)
{
    ...
    if (index_x < (width - 2) && index_y < (height - 2))
    {
        ...
        if (threadIdx.x < 2 && threadIdx.y < 2 &&
            (index_x + BLOCK_SIZE < width) && (index_y + BLOCK_SIZE < height))
        {
            sh_block[offset_shared + BLOCK_SIZE_SH + 1] = image[offset_t + width + 1];
            sh_block[offset_shared + BLOCK_SIZE_SH * 2 + 1] = image[offset_t + 2 * width + 1];
            sh_block[offset_shared + BLOCK_SIZE_SH + 2] = image[offset_t + width + 2];
            sh_block[offset_shared + BLOCK_SIZE_SH * 2 + 2] = image[offset_t + 2 * width + 2];
        }
        
        __syncthreads();

        image_out[offset] = gpu_applyFilter(&sh_block[offset_shared], BLOCK_SIZE_SH, gaussian, 3);
    }
}