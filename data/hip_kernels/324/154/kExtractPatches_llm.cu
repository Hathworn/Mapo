...
__global__ void kExtractPatches(float* images, float* patches, float* indices, float* width_offset, float* height_offset, int num_images, int img_width, int img_height, int patch_width, int patch_height, int num_colors) {
    ...
    // Use registers to store reused values
    int img_size = img_width * img_height;
    int img_plane = img_size * num_colors;
    ...
    for (unsigned long i = idx; i < total_pixels; i += numThreads) {
        ...
        source_row = __float2int_rd(height_offset[image_id]) + dest_row; // Use faster intrinsic
        source_col = __float2int_rd(width_offset[image_id]) + dest_col;  // Use faster intrinsic
        pos = img_plane * __float2int_rd(indices[image_id]) + img_size * color + img_width * source_row + source_col;
