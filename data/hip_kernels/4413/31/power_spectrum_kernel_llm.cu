...
__global__ void power_spectrum_kernel(int row_length, float *A_in, int32_t ldi, float *A_out, int32_t ldo, bool use_power) {
    ...
    for (int idx = thread_id; idx < half_length; idx += blockDim.x) {
        // Avoid executing for index 0 in the loop
        if (idx == 0) continue;

        float2 val = reinterpret_cast<float2 *>(Ar)[idx];
        float ret = val.x * val.x + val.y * val.y;
        Aw[idx] = use_power ? ret : sqrtf(ret); // Conditional inline to avoid branching
    }

    // Handle special case at idx 0
    if (thread_id == 0) {
        float real = Ar[0];
        float im = Ar[row_length];

        Aw[0] = use_power ? real * real : fabs(real); // Conditional inline for special case
