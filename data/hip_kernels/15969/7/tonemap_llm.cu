...
__global__ void tonemap(float* d_x, float* d_y, float* d_log_Y, float* d_cdf_norm, float* d_r_new, float* d_g_new, float* d_b_new, float min_log_Y, float max_log_Y, float log_Y_range, int num_bins, int num_pixels_y, int num_pixels_x) {
    int nx = num_pixels_x;
    int image_index_1d = (nx * blockIdx.y * blockDim.y + threadIdx.y * nx) + (blockIdx.x * blockDim.x + threadIdx.x);
    
    // Only proceed if within image boundaries
    if (image_index_1d < num_pixels_x * num_pixels_y) {
        float x = d_x[image_index_1d];
        float y = d_y[image_index_1d];
        float log_Y = d_log_Y[image_index_1d];
        
        // Calculate bin index and new luminance
        ...
        float scale_factor = Y_new / y;
        float X_new = x * scale_factor;
        float Z_new = (1 - x - y) * scale_factor;
