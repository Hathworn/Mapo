...
// Load d_pts1 into shared memory optimized using coalesced access
...
for (int bp2 = 0; bp2 < NPTS; bp2 += M7H) {
    // Load d_pts2 into shared memory optimized using coalesced access
    for (int d = tx; d < NDIM/4; d += M7W) {
    ...
    for (int d = 0; d < NDIM/4; d++) {
        ...
        for (int dy = 0; dy < M7R; dy++) {
            ...
            for (int i = 0; i < NRX; i++) {
                score[dy][i] += v1[i].x * v2.x + v1[i].y * v2.y + v1[i].z * v2.z + v1[i].w * v2.w;
            }
        }
    }

    // Update max_score and index
    for (int dy = 0; dy < M7R; dy++) {
        for (int i = 0; i < NRX; i++) {
