...
__global__ void Match6(float *d_pts1, float *d_pts2, float *d_score, int *d_index) {
    ...
    // Load d_pts1 to shared memory with coalesced access
    for (int d = tx; d < NDIM / 4; d += M5W) {
        for (int j = ty; j < M5W; j += blockDim.y) {
    ...
    for (int bp2 = 0; bp2 < NPTS; bp2 += M5H) {
        // Load d_pts2 to shared memory with coalesced access
        for (int d = tx; d < NDIM / 4; d += M5W) {
        ...
        if (ty < M5H / M5R) {
            ...
            for (int d = 0; d < NDIM / 4; d++) {
                ...
                for (int dy = 0; dy < M5R; dy++) {
                    ...
                    score[dy] += v1.x * v2.x + v1.y * v2.y + v1.z * v2.z + v1.w * v2.w;
                }
            }
    ...
    if (ty == 0) {
        ...
        // Reduce within block to find max score and index
