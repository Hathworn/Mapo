...
__global__ void LowPassRowMulti(float *d_Result, float *d_Data, int width, int pitch, int height)
{
    ...
    // Optimize loading with boundary checks
    if (loadPos < 0)
        data[tx] = d_Data[yptr];
    ...
    // Optimize calculation with reduced arithmetic
    if (xout < width && tx < CONVROW_W) {
        ...
        sum -= data[tx + 4] * kernel[4]; // Adjust for over-counting center
        d_Result[writePos] = sum + data[tx + 4] * kernel[4];
    }
    __syncthreads();
}