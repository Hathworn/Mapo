...
__global__ void ComputeOrientations(float *g_Data, int *d_Ptrs, float *d_Orient, int maxPts, int w, int h)
{
    ...
    for (int y = 0; y < 15; y++) {
        ...
        int xi = xp + x;
        int yi = yp + y;
        xi = max(0, min(w - 1, xi)); // Clamp to border
        yi = max(0, min(h - 1, yi)); // Clamp to border
        if (x >= 0 && x < 15) {
    ...
    // Compute gradients and accumulate histogram
    for (int y = 1; y < 14; y++) {
        int memPos = 16 * y + x;
        if (x >= 1 && x < 14) {
            float dy = data[memPos + 16] - data[memPos - 16];
            float dx = data[memPos + 1] - data[memPos - 1];
            int bin = 16.0f * atan2f(dy, dx) / M_PI + 16.5f;
            bin = (bin == 32) ? 0 : bin; // Wrap bin if it is 32
            float grad = sqrtf(dx * dx + dy * dy);
            atomicAdd(&hist[32 * (x - 1) + bin], grad * gauss[x] * gauss[y]); // Use atomicAdd
        }
    }
    __syncthreads();

    // Reduce histogram in shared memory
    ...
    // Reorganize the histogram values
    if (tx < 32) {
        float v = hist[tx];
        float v_prev = hist[tx == 0 ? 31 : (tx - 1)];
        float v_next = hist[(tx + 1) % 32];
        hist[tx] = (v > v_next && v >= v_prev) ? v : 0.0f;
    }
    __syncthreads();

    // Determine the peak orientations
    if (tx == 0) {
        float maxval1 = 0.0f;
        float maxval2 = 0.0f;
        int i1 = -1;
