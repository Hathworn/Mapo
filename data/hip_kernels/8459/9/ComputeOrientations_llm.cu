#include "hip/hip_runtime.h"
#include "includes.h"

__global__ void ComputeOrientations(float *g_Data, float *d_Sift, int maxPts, int w, int h)
{
    __shared__ float data[16*15];
    __shared__ float hist[32*13];
    __shared__ float gauss[16];
    const int tx = threadIdx.x;
    const int bx = blockIdx.x;

    // Initialize histogram with zeros
    for (int i = 0; i < 13; i++)
        hist[i*32 + tx] = 0.0f;
    __syncthreads();

    // Pre-calculate Gaussian values
    float i2sigma2 = -1.0f / (2.0f * 3.0f * 3.0f);
    if (tx < 15)
        gauss[tx] = exp(i2sigma2 * (tx - 7) * (tx - 7));

    int xp = (int)(d_Sift[bx] - 6.5f);
    int yp = (int)(d_Sift[bx + maxPts] - 6.5f);
    
    // Compute data and apply boundary checks
    for (int y = 0; y < 15; y++) {
        int xi = max(0, min(w - 1, xp + tx - (xp & 15)));
        int yi = max(0, min(h - 1, yp + y));
        int memPos = 16 * y + (tx - (xp & 15));
        if ((tx - (xp & 15)) >= 0 && (tx - (xp & 15)) < 15)
            data[memPos] = g_Data[yi * w + xi];
    }
    __syncthreads();

    // Compute gradients and accumulate orientation histogram
    for (int y = 1; y < 14; y++) {
        int memPos = 16 * y + (tx - (xp & 15));
        if ((tx - (xp & 15)) >= 1 && (tx - (xp & 15)) < 14) {
            float dy = data[memPos + 16] - data[memPos - 16];
            float dx = data[memPos + 1] - data[memPos - 1];
            int bin = (int)(16.0f * atan2f(dy, dx) / 3.1416f + 16.5f) & 31; // Use bitwise AND for modulo operation
            float grad = sqrtf(dx*dx + dy*dy);
            hist[32 * ((tx - (xp & 15)) - 1) + bin] += grad * gauss[tx - (xp & 15)] * gauss[y];
        }
    }
    __syncthreads();

    // Reduce histogram values
    for (int stride = 8; stride > 0; stride >>= 1) {
        for (int y = 0; y < stride; y++)
            hist[y * 32 + tx] += hist[(y + stride) * 32 + tx];
        __syncthreads();
    }

    // Smoothing histogram
    if (tx < 32) {
        float sum = hist[tx] * 6 + 
                    hist[(tx + 1) & 31] * 4 + 
                    hist[(tx + 31) & 31] * 4 +
                    hist[(tx + 2) & 31] + 
                    hist[(tx + 30) & 31];
        hist[tx] = sum;
    }
    __syncthreads();

    // Find peaks within the histogram
    float v = hist[tx];
    hist[tx] = (v > hist[(tx + 1) & 31] && v >= hist[(tx + 31) & 31]) ? v : 0.0f;
    __syncthreads();

    // Determine main orientation(s)
    if (tx == 0) {
        float maxval1 = 0.0f, maxval2 = 0.0f;
        int i1 = -1, i2 = -1;
        for (int i = 0; i < 32; i++) {
            float v = hist[i];
            if (v > maxval1) {
                maxval2 = maxval1;
                maxval1 = v;
                i2 = i1;
                i1 = i;
            } else if (v > maxval2) {
                maxval2 = v;
                i2 = i;
            }
        }
        float val1 = hist[(i1 + 1) & 31];
        float val2 = hist[(i1 + 31) & 31];
        float peak = i1 + 0.5f * (val1 - val2) / (2.0f * maxval1 - val1 - val2);
        d_Sift[bx + 5 * maxPts] = 11.25f * ((peak < 0.0f) ? (peak + 32.0f) : peak);

        if (maxval2 >= 0.8f * maxval1) {
            val1 = hist[(i2 + 1) & 31];
            val2 = hist[(i2 + 31) & 31];
            peak = i2 + 0.5f * (val1 - val2) / (2.0f * maxval2 - val1 - val2);
            d_Sift[bx + 6 * maxPts] = 11.25f * ((peak < 0.0f) ? (peak + 32.0f) : peak);
        } else {
            d_Sift[bx + 6 * maxPts] = i2;
        }
    }
}