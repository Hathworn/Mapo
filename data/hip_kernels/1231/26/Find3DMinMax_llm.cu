...
{
    // Optimize shared memory allocation to a single array for each data
    __shared__ float sharedData[3 * (MINMAX_SIZE + 2)];

    // Use pointer arithmetic to simulate multiple arrays
    float* data1 = sharedData;
    float* data2 = data1 + (MINMAX_SIZE + 2);
    float* data3 = data2 + (MINMAX_SIZE + 2);
    ...
    for (int y = 0; y < 34; y++) {
        output >>= 1;
        int memPos = y * memWid + (tx - WARP_SIZE + 1);
        int yp = 32 * blockIdx.y + y - 1;
        yp = max(yp, 0);
        yp = min(yp, height - 1);
        int readStart = yp * width;

        // Read data with boundary checks
        if (tx >= (WARP_SIZE - 1)) {
        ...
        int memPos2 = y * memWid + tx;
        if (y > 1) {
            if (tx < memWid) {
                // Compute min and max for each segment
        ...
        if (y > 1) {
            if (tx < MINMAX_SIZE) {
                if (xWritePos <= xEndClamped) {
                    // Compute global min and max and update output
    ...
    // Write the result if within the bounds
