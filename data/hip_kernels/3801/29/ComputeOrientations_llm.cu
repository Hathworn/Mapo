...
__global__ void ComputeOrientations(float *g_Data, int *d_Ptrs, float *d_Orient, int maxPts, int w, int h)
{
    ...
    // Smooth histogram
    // Use warp synchronization and reduce loops
    if (tx < 32) {
        hist[32 + tx] = 6 * hist[tx] + 4 * (hist[(tx + 1) % 32] + hist[(tx - 1 + 32) % 32]) + (hist[(tx + 2) % 32] + hist[(tx - 2 + 32) % 32]);
    }
    __syncthreads();
    ...
    // Find and interpolate peaks in the histogram
    if (tx == 0) {
        float maxval1 = 0.0;
        float maxval2 = 0.0;
        int i1 = -1;
