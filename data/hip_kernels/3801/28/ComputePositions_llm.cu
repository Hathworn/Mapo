...
__global__ void ComputePositions(float *g_Data1, float *g_Data2, float *g_Data3, int *d_Ptrs, float *d_Sift, int numPts, int maxPts, int w, int h)
{
    ...
    // Load shared memory data to reduce global memory access
    __shared__ float sharedData[POSBLK_SIZE * 3];
    int idx = threadIdx.x * 3;
    sharedData[idx] = g_Data2[p];
    sharedData[idx + 1] = g_Data2[p - 1];
    sharedData[idx + 2] = g_Data2[p + 1];
    __syncthreads();

    float val[7];
    val[0] = sharedData[idx];
    val[1] = sharedData[idx + 1];
    val[2] = sharedData[idx + 2];

    float dx = 0.5f * (val[2] - val[1]);
    float dxx = 2.0f * val[0] - val[1] - val[2];
    
    // Use shared memory for adjacent row access
    sharedData[idx + 1] = g_Data2[p - w];
    sharedData[idx + 2] = g_Data2[p + w];
    __syncthreads();
    val[3] = sharedData[idx + 1];
    val[4] = sharedData[idx + 2];
    ...
    // Re-organize operations for coalesced memory access
    ...
    // Inline calculation of determinants for optimization
