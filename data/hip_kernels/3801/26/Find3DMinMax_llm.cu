...
__global__ void Find3DMinMax(int *d_Result, float *d_Data1, float *d_Data2, float *d_Data3, int width, int height) {
    ...
    for (int y=0; y<34; y++) {
        // Initialize output bit-shifted to the right
        output >>= 1;
        ...
        // Set the entire data cache contents only for valid threads
        if (tx >= (WARP_SIZE-1)) {
        ...
        if (y > 1 && tx < memWid) {
            // Compute minimum and maximum for each dimension concurrently using shared memory
        ...
        if (y > 1 && tx < MINMAX_SIZE && xWritePos <= xEndClamped) {
            // Use fminf/fmaxf reductions to find overall minima/maxima
            ...
            // Set output bit if condition meets
            if (data2[memPos1+1] < minv || data2[memPos1+1] > maxv)
                output |= 0x80000000;
        }
        __syncthreads();

        // Update positions and cycle yq index
        memPos0 = memPos1;
        memPos1 = memPos2;
        yq = (yq < 2 ? yq + 1 : 0);
    }
