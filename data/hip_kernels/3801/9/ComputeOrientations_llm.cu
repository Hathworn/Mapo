...
__global__ void ComputeOrientations(float *g_Data, float *d_Sift, int maxPts, int w, int h) {
    ...
    // Smoothing histogram
    if (tx < 30)
        hist[tx + 32] = 6 * hist[tx] + 4 * (hist[tx + 1] + hist[(tx == 0 ? 31 : tx - 1)]) +
                        (hist[(tx + 2) % 32] + hist[tx == 0 ? 30 : tx - 2]);
    __syncthreads();
