...
__global__ void selectFcn(float *populationArray, float *tmpPopulationArray, float *fitness, float *Fitness, float *tmpFitness, float sumFitness, float *populationPro, hiprandState_t *states) {
    ...
    float ss = max(hiprand_uniform(&s), 0.0001f); // Ensure ss is always greater than or equal to 0.0001
    ...
    int baseIdx = idx * chromosomeSize; // Precompute base index
    int baseIndex = index * chromosomeSize; // Precompute base index for selected chromosome

    for (int j = 0; j < chromosomeSize; j++) {
        populationArray[baseIdx + j] = tmpPopulationArray[baseIndex + j];
    }
