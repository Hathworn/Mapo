...
__global__ void downscale(float *gradInput_data, const float *gradOutput_data, long no_elements, int scale_factor, int d1, int d2, int d3) {
    ...
    // Precompute indices outside the loops to reduce recomputation
    int base_idx = translate_idx(ii, d1, d2, d3, scale_factor);
    int scale_d3 = d3 * scale_factor;

    for (int i = 0; i < scale_factor; i++) {
        int row_offset = i * scale_d3;
        for (int j = 0; j < scale_factor; j++) {
            int ipidx = base_idx + row_offset + j * scale_d3 / scale_factor;
