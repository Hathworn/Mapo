...
__global__ void gpuMeans(const float * vectsA, size_t na, const float * vectsB, size_t nb, size_t dim, float * means, float * numPairs)
{
    ...
    // Shared memory allocation for thread sums and count
    __shared__ float threadSumsA[NUMTHREADS], threadSumsB[NUMTHREADS], count[NUMTHREADS];

    // Early exit if block index exceeds boundaries
    if(bx >= na || by >= nb)
        return;
    ...
    // Loop over vector components with stride handled by NUMTHREADS
    for(size_t offset = tx; offset < dim; offset += NUMTHREADS) {
        a = vectsA[bx * dim + offset];
        b = vectsB[by * dim + offset];
        // Check for NaN values and update sums and count
        if(!isnan(a) && !isnan(b)) {
            threadSumsA[tx] += a;
            threadSumsB[tx] += b;
            count[tx] += 1.f;
    ...
    // Reduction for thread sums and count
