...
__global__ void gpuPMCC(const float * vectsa, size_t na, const float * vectsb, size_t nb, size_t dim, const float * numPairs, const float * means, const float * sds, float * correlations)
{
    ...
    __shared__ float threadSums[NUMTHREADS]; // Shared memory for thread sums

    if ((x >= na) || (y >= nb)) return;

    if (tx == 0) {
        // Read shared values only once per block
    ...
    for (size_t offset = tx; offset < dim; offset += NUMTHREADS) {
        // Load a and b once per iteration
        a = vectsa[x * dim + offset];
        b = vectsb[y * dim + offset];
        if (!isnan(a) && !isnan(b)) {
            // Calculate scores only for valid numbers
            scoreA = (a - meanA) / sdA;
            scoreB = (b - meanB) / sdB;
    ...
    // Reduction to sum up threadSums
