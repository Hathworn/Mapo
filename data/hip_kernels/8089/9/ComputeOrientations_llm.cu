...
__global__ void ComputeOrientations(float *g_Data, float *d_Sift, int maxPts, int w, int h)
{
    ...
    else if (tx == 31) hist[63] = 6 * hist[31] + 4 * (hist[0] + hist[30]) + (hist[1] + hist[29]);
    __syncthreads();
    ...
    if (tx == 0) {
        ...
        d_Sift[bx + 6 * maxPts] = (maxval2 < 0.8f * maxval1) ? -1.0f : d_Sift[bx + 6 * maxPts];
        if (i2 >= 0) {
