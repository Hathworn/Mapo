...
__global__ void Correlation_backward_input2(int item, float *gradInput2, int nInputChannels, int inputHeight, int inputWidth, float *gradOutput, int nOutputChannels, int outputHeight, int outputWidth, float *rInput1, int pad_size, int kernel_size, int max_displacement, int stride1, int stride2)
{
    ...
    for (int tc = tch_off; tc < nOutputChannels; tc += CUDA_NUM_THREADS) {
        ...
        int xmin = max((x - kernel_rad - max_displacement - i2) / stride1, 0);
        int ymin = max((y - kernel_rad - max_displacement - j2) / stride1, 0);

        int xmax = min((x + kernel_rad - max_displacement - i2) / stride1, outputWidth - 1);
        int ymax = min((y + kernel_rad - max_displacement - j2) / stride1, outputHeight - 1);
    ...
    // Optimize reduction using thread 0
    if (tch_off == 0) {
        float reduce_sum = 0.0f;
        #pragma unroll  // Unroll reduction loop
