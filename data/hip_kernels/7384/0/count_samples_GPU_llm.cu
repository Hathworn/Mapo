...
{
    __shared__ int shared_blocks[threadsPerBlock];  // shared memory for threads in the same block

    int index = threadIdx.x + blockIdx.x * blockDim.x;
    int stride = blockDim.x * num_block;

    // Initialize shared memory for threads
    int inCircle = 0;
    ...
    // Only write to shared memory once all calculations are complete
    shared_blocks[threadIdx.x] = inCircle;
    __syncthreads();  // Ensure all threads have written to shared memory

    // Reduce within the block to get total count
    if (threadIdx.x == 0) {
