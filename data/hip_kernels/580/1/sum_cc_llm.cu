...
__global__ void sum_cc(float *cc_mat, float *cc_sum, float *weights, int n_stations, int n_components, int n_corr, int chunk_offset, int chunk_size) {
    ...
    if ( ((i + chunk_offset) < n_corr) && (i < chunk_size) ){
        // Optimized: Using a local accumulator for better memory access pattern
        float sum = 0.0f;
        float *cc_mat_offset = cc_mat + i * n_stations * n_components;
        for (ch = 0; ch < (n_stations * n_components); ch++) {
            sum += cc_mat_offset[ch] * weights[ch];
        }
        cc_sum[i] = sum; // Write back the accumulated sum
    }
}