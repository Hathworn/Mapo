...
__global__ void Find3DMinMax(int *d_Result, float *d_Data1, float *d_Data2, float *d_Data3, int width, int pitch, int height)
{
    ...
    for (int y = 0; y < 32 + 2; y++) {
        ...
        // Cache data with boundary check improved
        if (tx >= (WARP_SIZE - 1)) {
        ...
        if (y > 1) {
            if (tx < memWid) {
                // Unroll the loop for optimization and combine min/max calculations
                float3 data1Triplet = make_float3(data1[memPos0], data1[memPos1], data1[memPos2]);
                float3 data2Triplet = make_float3(data2[memPos0], data2[memPos1], data2[memPos2]);
                float3 data3Triplet = make_float3(data3[memPos0], data3[memPos1], data3[memPos2]);
                
                ymin1[tx] = min(data1Triplet.x, min(data1Triplet.y, data1Triplet.z));
                ymin2[tx] = min(ymin1[tx], min(data2Triplet.x, min(data2Triplet.y, data2Triplet.z)));
                ymin3[tx] = min(data3Triplet.x, min(data3Triplet.y, data3Triplet.z));
                
                ymax1[tx] = max(data1Triplet.x, max(data1Triplet.y, data1Triplet.z));
                ymax2[tx] = max(ymax1[tx], max(data2Triplet.x, max(data2Triplet.y, data2Triplet.z)));
                ymax3[tx] = max(data3Triplet.x, max(data3Triplet.y, data3Triplet.z));
            }
        }
        __syncthreads();
