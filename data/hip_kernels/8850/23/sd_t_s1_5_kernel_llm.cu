...
__global__ void sd_t_s1_5_kernel(int h1d,int h2d,int h3d,int p4d,int p5d,int p6d,int p4ld_t2,int h1ld_t2,int h3ld_v2,int h2ld_v2,int p6ld_v2,int p5ld_v2,int h3ld_t3,int h2ld_t3,int h1ld_t3,int p6ld_t3,int p5ld_t3,int p4ld_t3,double *t3d, double *t2_d, double *v2_d,int p4, int total_x) {
    ...
    int tid = threadIdx.x;
    int bid = blockIdx.x;
    int thread_x = T2*T1 * bid + tid;
    int grid_stride = gridDim.x * blockDim.x;

    for(int i = tid; i < h1d * p4d; i += blockDim.x) {
        if (i < h1d * p4d) {
            t2_shm[i] = t2_d[i];
        }
    }
    ...
    for (int i = thread_x; i < total_x; i += grid_stride) {
        int rest_x = i;
        h3 = rest_x % h3d;
        rest_x /= h3d;
        h2 = rest_x % h2d;
        ...
        for (h1 = 0; h1 < h1d; h1++) {
            for (p4 = 0; p4 < p4d; p4++) {
                // Utilize shared memory for t2 and directly update t3
