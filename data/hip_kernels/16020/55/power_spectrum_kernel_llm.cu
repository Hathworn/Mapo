...
__global__ void power_spectrum_kernel(int row_length, const float *A_in, int32_t ldi, float *A_out, int32_t ldo, bool use_power) {
    ...
    // Loop unrolling and shared memory usage for better memory access
    __shared__ float shared_real[CU1DBLOCK];
    __shared__ float shared_im[CU1DBLOCK];
    ...
    // Handle special case with optimized memory access
    if (threadIdx.x == 0) {
        float real = Ar[0];
