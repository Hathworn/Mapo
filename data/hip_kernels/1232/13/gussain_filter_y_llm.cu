...
__global__ void gussain_filter_y(float* random, float* kernel, int lw, size_t dim_z, size_t dim_y, size_t dim_x, int mode, float cval) {
    ...
    if(index < total) { // Bound check
        ...
        for(int i = -lw; i <= lw; i++) { // Streamlined loop
            int id = id_y + i;
            if(mode == 0) {
                if(id < 0 || id >= dim_y)
                    new_pixel += cval * kernel[i + lw];
                else
                    new_pixel += kernel[i + lw] * random[id_block * total + id_z * total_xy + id * dim_x + id_x];
            } else {
                id = mirror(id, dim_y);
                new_pixel += kernel[i + lw] * random[id_block * total + id_z * total_xy + id * dim_x + id_x];
            }
