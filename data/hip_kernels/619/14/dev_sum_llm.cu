...
__constant__ float d_diff1filt[diff1filtSize] = {-0.5, 0, 0.5};

#define diff2filtSize 3
__constant__ float d_diff2filt[diff2filtSize] = {1, -2, 1};

#define diff3filtSize 5
__constant__ float d_diff3filt[diff3filtSize] = {-0.5, 1, 0, -1, 0.5};

#define nrT 9

__global__ void dev_sum(float *i1data, float *i2data, float* odata, int len) {
    const int tid = blockIdx.x * blockDim.x + threadIdx.x; // Use standard indexing
    const int threadN = gridDim.x * blockDim.x; // Use standard grid size

    for (int i = tid; i < len; i += threadN) {
        odata[i] = i1data[i] + i2data[i]; // Perform addition
    }
}