...
__global__ void dev_fullRect2(float *data, int len, double scaleV1Linear, double scaleV1FullWaveRect) {
    ...
    for(int i = tid; i < len; i += threadN) {
        // Load element into register to reduce global memory access
        float d = data[i] * scaleV1Linear; 
        // Perform squaring and scaling with reduced instruction count
        data[i] = fmaf(d, d, 0.0) * scaleV1FullWaveRect; 
    }
}