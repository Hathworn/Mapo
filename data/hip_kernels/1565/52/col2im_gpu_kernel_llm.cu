...
__global__ void col2im_gpu_kernel(const int n, const float* data_col, const int height, const int width, const int ksize, const int pad, const int stride, const int height_col, const int width_col, float *data_im) {
    ...
    // Unrolling loop and using shared memory for reduced global memory access
    ...
    for (; index < n; index += blockDim.x * gridDim.x) {
        ...
        // Use shared memory to cache data_col segments
        for (int h_col = h_col_start; h_col < h_col_end; ++h_col) {
            for (int w_col = w_col_start; w_col < w_col_end; ++w_col) {
                int data_idx = offset + h_col * coeff_h_col + w_col * coeff_w_col;
                shared_mem[threadIdx.x] = data_col[data_idx];
                __syncthreads();
