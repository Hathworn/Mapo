...
__host__ HostQueue::HostQueue(int nq):size(nq*nq*3),nQueen(nq),dbg(true){
	...
	ErrorChecking::hostErrorCheck(hipMalloc((void**)&dMem,sizeof(Triple)*nQueen*nQueen*3),"HostQueue::HostQueue");
...
__host__ HostQueue::~HostQueue(){
	...
	ErrorChecking::hostErrorCheck(hipFree(dMem),"HostQueue::~HostQueue");
}
