...
__global__ void TestDimensions()
{
    ...
    // Simplified thread_id calculation using linear indexing
    int const thread_id = ((blockIdx.z * gridDim.x * gridDim.y + blockIdx.y * gridDim.x + blockIdx.x) * num_threads_per_block) +
