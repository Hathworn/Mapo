...
__global__ void downscale(float *gradInput_data, const float *gradOutput_data, long no_elements, int scale_factor, int d1, int d2, int d3) {
    // Calculate flat index for the 2D block grid
    long ii = threadIdx.x + blockDim.x * blockIdx.x + threadIdx.y * blockDim.y * gridDim.x * blockDim.x * blockIdx.y;
    if (ii >= no_elements) return; // Ensure index is within bounds
    ...
    for (int i = 0; i < scale_factor; i++) {
        ...
        for (int j = 0; j < scale_factor; j++) {
            int ipidx = ipidx_base + i * d3 * scale_factor + j; // Pre-compute base index
