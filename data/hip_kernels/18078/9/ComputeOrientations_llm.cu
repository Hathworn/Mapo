...
__global__ void ComputeOrientations(float *g_Data, float *d_Sift, int maxPts, int w, int h)
{
    ...
    __shared__ float hist[32 * 4];  // Reduced shared memory usage by optimizing histogram size
    ...
    // Compute gradient and fill histogram
    for (int y = 1; y < 14; y++) {
        int memPos = 16 * y + x;
        if (x >= 1 && x < 14) {
            ...
            atomicAdd(&hist[32 * ((x - 1) & 3) + bin], grad * gauss[x] * gauss[y]);
        }
    }
    __syncthreads();
    ...
    if (tx < 32) {
        for (int y = 1; y < 4; y++)
            hist[tx] += hist[y * 32 + tx];
    }
    __syncthreads();

    // Compute smoothed histogram
    if (tx < 32) {
        float smoothed_val = 6 * hist[tx] +
                             4 * (hist[(tx + 1) % 32] + hist[(tx + 31) % 32]) +
                             (hist[(tx + 2) % 32] + hist[(tx + 30) % 32]);
        hist[tx] = smoothed_val;
    }
    __syncthreads();
