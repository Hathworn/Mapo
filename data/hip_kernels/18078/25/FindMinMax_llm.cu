...
__global__ void FindMinMax(float *d_MinMax, float *d_Data, int width, int height)
{
    ...
    // Loop through the height and update min and max values
    ...
    // Reduction to find the block-wise min and max
    for (int d = b / 2; d > 0; d >>= 1) { // Use tree-based reduction
        if (tx < d) {
    ...
    // Write the results for this block to the global memory
    if (tx == 0) {
