...
float* emdata_transform_cuda(const float* const matrix,const int nx,const int ny,const int nz) {
	...
	float* data;
	hipError_t error = hipMalloc((void**)&data,nx*ny*nz*sizeof(float));
	if ( error != hipSuccess ) {
...
CudaPeakInfo* calc_max_location_wrap_cuda(const float* in, const int nx, const int ny, const int nz, const int maxdx, const int maxdy, const int maxdz)
{
	...
	if(nz > 1)
	{
		...
		hipMalloc((void **)&device_soln, sizeof(CudaPeakInfo));
		...
		hipDeviceSynchronize();
		hipMemcpy(host_soln,device_soln,sizeof(CudaPeakInfo),hipMemcpyDeviceToHost);
		hipFree(device_soln);
		...
		CudaPeakInfo * device_soln=0;
		hipMalloc((void **)&device_soln, num_calcs*sizeof(CudaPeakInfo));
		CudaPeakInfo * host_soln = 0;
		...
		hipMemcpy(host_soln,device_soln,num_calcs*sizeof(CudaPeakInfo),hipMemcpyDeviceToHost);
		hipFree(device_soln);
...
CudaPeakInfoFloat* calc_max_location_wrap_intp_cuda(const float* in, const int nx, const int ny, const int nz, const int maxdx, const int maxdy, const int maxdz)
{
	...
	if(nz > 1)
	{
		...
		hipMalloc((void **)&device_soln, sizeof(CudaPeakInfoFloat));
		...
		hipDeviceSynchronize();
		hipMemcpy(host_soln,device_soln,sizeof(CudaPeakInfoFloat),hipMemcpyDeviceToHost);
		hipFree(device_soln);
...
float* emdata_column_sum(const float* data, const int nx, const int ny) {
	...
	float* sum = 0;
	hipMalloc((void **)&sum, num_calcs*sizeof(float));
