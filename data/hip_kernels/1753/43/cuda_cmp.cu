...
float dot_cmp_cuda(float* data1, float* data2, const float* dm, const int &nx, const int &ny, const int &nz)
{
	...
	//float * device_results=0;
	//hipMalloc((void **)&device_results, rsize*sizeof(float));
	...
	hipMemcpy(host_results,device_results,sizeof(float),hipMemcpyDeviceToHost);
	...
	*/
	...
	float * device_results=0;
	hipMalloc((void **)&device_results, darraysize*sizeof(float));
	...
	*/
	...
	float * device_soln=0;
	hipMalloc((void **)&device_soln, num_calcs*sizeof(float));
	float * host_soln = 0;
	...
	hipDeviceSynchronize();
	hipMemcpy(host_soln,device_soln,num_calcs*sizeof(float),hipMemcpyDeviceToHost);
	hipFree(device_soln);
...
float2 get_stats_cuda(const float * data, const int nx, const int ny, const int nz)
{
	...
	float * device_soln=0;
	hipMalloc((void **)&device_soln, 2*num_calcs*sizeof(float));
	float * host_soln = 0;
	...
	hipDeviceSynchronize();
	hipMemcpy(host_soln,device_soln,2*num_calcs*sizeof(float),hipMemcpyDeviceToHost);
	hipFree(device_soln);
...
float get_value_at_wrap_cuda(float * data, int tx, int ty, int tz, int nx, int ny, int nz)
{
	...
	hipMemcpy(host_soln,data+index,sizeof(float),hipMemcpyDeviceToHost);
...
float ccc_cmp_cuda(const float* data1, const float* data2, const float* dm, const int &nx, const int &ny, const int &nz)
{
	...
	float * device_soln=0;
	hipMalloc((void **)&device_soln, 5*num_calcs*sizeof(float));
	float * host_soln = 0;
	...
	hipDeviceSynchronize();
	hipMemcpy(host_soln,device_soln,5*num_calcs*sizeof(float),hipMemcpyDeviceToHost);
	hipFree(device_soln);
...
float fsc_tomo_cmp_cuda(const float* data1, const float* data2, const float data1threshold, const float data2threshold, const float minres, const float maxres, const int nx, const int ny, const int nz)
{
	...
	float * device_soln=0;
	hipMalloc((void **)&device_soln, 3*num_calcs*sizeof(float));
	float * host_soln = 0;
	...
	hipDeviceSynchronize();
	hipMemcpy(host_soln,device_soln,3*num_calcs*sizeof(float),hipMemcpyDeviceToHost);
	hipFree(device_soln);
