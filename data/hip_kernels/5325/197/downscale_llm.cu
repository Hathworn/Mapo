...
__global__ void downscale(float *gradInput_data, const float *gradOutput_data, long no_elements, int scale_factor, int d1, int d2, int d3) {
    ...
    // Pre-calculate stride for input data processing to minimize redundant calculations
    const int stride = d2 * d3 * scale_factor;
    float sum = 0.0f; // Local accumulator to reduce memory accesses
    ...
    gradInput_data[ii] += sum; // Store accumulated result
}