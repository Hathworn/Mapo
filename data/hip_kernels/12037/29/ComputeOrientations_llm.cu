...
__global__ void ComputeOrientations(float *g_Data, int *d_Ptrs, float *d_Orient, int maxPts, int w, int h)
{
    ...
    if (tx < 13) {  // Initialize histogram using only the required threads
        for (int i = 0; i < 32; i++)
            hist[i*13+tx] = 0.0f;
    }
    __syncthreads();
    ...
    for (int y = 1; y < 14; y++) {
        ...
        if (x >= 1 && x < 14) {
            ...
            atomicAdd(&hist[32*(x-1)+bin], grad * gauss[x] * gauss[y]);  // Improved atomic addition for safe update
        }
    }
    __syncthreads();
    ...
    // Parabolic fit for peak orientation
    if (tx == 0) {
        float maxval1 = 0.0;
        float maxval2 = 0.0;
        int i1 = -1;
