...
__global__ void Find3DMinMax(int *d_Result, float *d_Data1, float *d_Data2, float *d_Data3, int width, int pitch, int height)
{
    ...
    for (int y = 0; y < 34; y++) {  // Unroll loop
        output >>= 1;
        ...
        if (tx >= (WARP_SIZE - 1)) {
            // Streamline input range check and data assignment
        ...
        if (y > 1) {
            if (tx < memWid) {
                // Use min/max with loops for scalability
        ...
        if (y > 1) {
            if (tx < MINMAX_W) {
                if (xWritePos <= xEndClamped) {
                    ...
                    float minv = fminf(fminf(fminf(fminf(ymin2[tx], ymin2[tx + 2]), ymin1[tx + 1]), ymin3[tx + 1]), data2[memPos0 + 1]);
                    minv = fminf(minv, d_Threshold[1]);
                    float maxv = fmaxf(fmaxf(fmaxf(fmaxf(ymax2[tx], ymax2[tx + 2]), ymax1[tx + 1]), ymax3[tx + 1]), data2[memPos0 + 1]);
