...
__global__ void ComputeOrientations(float *g_Data, float *d_Sift, int maxPts, int w, int h)
{
    ...
    for (int y=0; y<15; y++) {
        ...
        int xi = xp + x;
        int yi = yp + y;
        xi = max(0, min(xi, w - 1)); // Clamp xi within bounds
        yi = max(0, min(yi, h - 1)); // Clamp yi within bounds
    ...
    // Compute gradient and orientation histograms
    for (int y=1; y<14; y++) {
        int memPos = 16*y + x;
        if (x >= 1 && x < 14) {
            float dy = data[memPos+16] - data[memPos-16];
            float dx = data[memPos+1] - data[memPos-1];
            int bin = static_cast<int>(16.0f * atan2f(dy, dx) / 3.1416f + 16.5f) & 31; // Use modulo for bin wrapping
    ...
    if (tx >= 0 && tx <= 31) {
        ...
        hist[32 + tx] = 6*hist[tx] + 4*(hist[right] + hist[left]) + (hist[(right+1)&31] + hist[(left+1)&31]);
    }
    __syncthreads();
