...
// Improved memory access patterns using shared memory and loop unrolling
__global__ void Find3DMinMax(int *d_Result, float *d_Data1, float *d_Data2, float *d_Data3, int width, int height)
{
    ...
    for (int y = 0; y < 34; y++) {
        ...
        if (y > 1) {
            // Using loop unrolling for min/max calculations
            if (tx < memWid) {
