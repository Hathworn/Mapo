...
__global__ void removeBorderGridFlipd(float* dst, const unsigned int size, const unsigned int delta, const unsigned int blockSize, const float remplace) {
    // Calculate global thread index for 1D access
    int globalIdx = blockIdx.y * gridDim.x * blockDim.x + blockIdx.x * blockDim.x + threadIdx.x;
    // Check bounds for both position within grid and block id
    if (globalIdx < size && (globalIdx / delta) < blockSize) {
        dst[globalIdx] = remplace;
    }
}