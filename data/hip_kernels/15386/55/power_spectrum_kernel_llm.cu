#include "hip/hip_runtime.h"
#include "includes.h"

__global__ void power_spectrum_kernel(int row_length, const float *A_in, int32_t ldi, float *A_out, int32_t ldo, bool use_power) {
    // Use shared memory for enhanced performance
    __shared__ float shared_in[CU1DBLOCK * 2];
    
    int thread_id = threadIdx.x;
    int block_id = blockIdx.x;
    const float *Ar = A_in + block_id * ldi;
    float *Aw = A_out + block_id * ldo;

    int half_length = row_length / 2;

    // Load data into shared memory for coalesced access
    if (thread_id < row_length) {
        shared_in[thread_id] = Ar[thread_id];
    }
    __syncthreads();

    for (int idx = thread_id; idx < half_length; idx += CU1DBLOCK) {
        // ignore special case
        if (idx == 0) continue;

        float2 val = reinterpret_cast<const float2 *>(shared_in)[idx];
        float ret = val.x * val.x + val.y * val.y;

        if (use_power) {
            Aw[idx] = ret;
        } else {
            Aw[idx] = sqrtf(ret);
        }
    }

    // handle special case
    if (threadIdx.x == 0) {
        float real = shared_in[0];
        float im = shared_in[row_length];

        if (use_power) {
            Aw[0] = real * real;
            Aw[half_length] = im * im;
        } else {
            Aw[0] = fabs(real);
            Aw[half_length] = fabs(im);
        }
    }
}