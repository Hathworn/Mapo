...
__global__ void query_ball_point_gpu(int b, int n, int m, float radius, int nsample, const float *xyz1, const float *xyz2, int *idx, int *pts_cnt) {
    ...
    for (int j = index; j < m; j += stride) {
        ...
        for (int k = 0; k < n; ++k) {
            ...
            float dy = y2 - y1;
            float dz = z2 - z1;
            float dist2 = dx * dx + dy * dy + dz * dz; // Calculate squared distance
            if (dist2 < radius * radius) { // Compare squared distance
                if (cnt == 0) {
                    for (int l = 0; l < nsample; ++l) idx[j * nsample + l] = k; // Set all indices to k
                }
                idx[j * nsample + cnt] = k;
                cnt += 1;
            }
