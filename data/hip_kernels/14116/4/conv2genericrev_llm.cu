...
__global__ void conv2genericrev(float *input, float *kernel, float *output, int input_n, int input_h, int input_w, int kernel_n, int kernel_h, int kernel_w, float alpha, int stride_h, int stride_w)
{
    ...
    for (xx = 0; xx < output_w; xx++) {
        // Efficient index calculation for input and kernel
        ...
        for (int ky = 0; ky < kernel_h; ky++) {
            ...
            input_p += input_w; // Move to the next row in input
            kernel_p += kernel_w; // Move to the next row in kernel
        }
        *(output_p++) = sum;
    }
    __syncthreads();
    ...
    if (yy == 0) {
        // More efficient shared memory reduction
