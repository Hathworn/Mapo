#include "hip/hip_runtime.h"
#include "includes.h"

__global__ void FindMinMax(float *d_MinMax, float *d_Data, int width, int pitch, int height) {
    __shared__ float minvals[128];
    __shared__ float maxvals[128];
    const int tx = threadIdx.x;
    const int x = blockIdx.x * 128 + tx;
    const int y = blockIdx.y * 16;
    int p = y * pitch + x;

    // Initialize shared memory with boundary check
    float val = (x < width) ? d_Data[p] : d_Data[p - x];
    minvals[tx] = val;
    maxvals[tx] = val;
    __syncthreads();

    // Unroll loop for better performance
    for (int ty = 1; ty < 16; ++ty) {
        p += pitch;
        if (x < width) {
            val = d_Data[p];
            if (val < minvals[tx]) minvals[tx] = val;
            if (val > maxvals[tx]) maxvals[tx] = val;
        }
    }
    __syncthreads();

    // Reduce to find min and max in shared memory
    for (int offset = blockDim.x / 2; offset > 0; offset >>= 1) {
        if (tx < offset) {
            if (minvals[tx + offset] < minvals[tx]) minvals[tx] = minvals[tx + offset];
            if (maxvals[tx + offset] > maxvals[tx]) maxvals[tx] = maxvals[tx + offset];
        }
        __syncthreads();
    }

    // Write result for this block to global memory
    if (tx == 0) {
        int ptr = 2 * (gridDim.x * blockIdx.y + blockIdx.x);
        d_MinMax[ptr] = minvals[0];
        d_MinMax[ptr + 1] = maxvals[0];
    }
}