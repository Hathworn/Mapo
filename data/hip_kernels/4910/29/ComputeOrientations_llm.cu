...
__global__ void ComputeOrientations(float *g_Data, int *d_Ptrs, float *d_Orient, int maxPts, int w, int h) {
    ...
    // Calculate gradients and fill histogram
    for (int y = 1; y < 14; y++) {
        int memPos = 16 * y + x;
        if (x >= 1 && x < 14) {
    ...
    // Accumulate histogram vertically
    for (int y = 0; y < 5; y++) {
    ...
    // Combine additional histogram values
    hist[tx] += hist[32 + tx];
    __syncthreads();
