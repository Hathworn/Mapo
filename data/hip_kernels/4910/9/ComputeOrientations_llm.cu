...
__global__ void ComputeOrientations(float *g_Data, float *d_Sift, int maxPts, int w, int h)
{
    ...
    // Initialize histograms to zero in parallel
    if (tx < 13) {
        for (int i = 0; i < 32; ++i)
            hist[i + tx * 32] = 0.0f;
    }
    ...
    // Load neighborhood patch to shared memory
    for (int y = 0; y < 15; y++) {
        int memPos = 16 * y + x;
        int xi = xp + x;
        int yi = yp + y;
        xi = max(0, min(w - 1, xi)); // Clamp to image boundaries
        yi = max(0, min(h - 1, yi)); // Clamp to image boundaries
        if (x >= 0 && x < 15)
            data[memPos] = g_Data[yi * w + xi];
    }
    __syncthreads();

    // Compute gradients and histogram values
    for (int y = 1; y < 14; y++) {
        int memPos = 16 * y + x;
        if (x >= 1 && x < 14) {
