...
__global__ void LowPassColMulti(float *d_Result, float *d_Data, int width, int pitch, int height)
{
    ...
    if (colStart < width) {
        ...
        for (int y = totStart + ty; y <= totEnd; y += blockDim.y) {
            if (y < 0) {
                sdata[0] = d_Data[colStart];  // Top boundary case
            } else if (y >= height) {
                sdata[0] = d_Data[colEnd];  // Bottom boundary case
            } else {
                sdata[0] = d_Data[gPos];  // Main data copy
            }
            sdata += sStep;
            gPos += gStep;
        }
    ...
    if (colStart < width) {
        ...
        // Compute convolution result by iterating over the rows
