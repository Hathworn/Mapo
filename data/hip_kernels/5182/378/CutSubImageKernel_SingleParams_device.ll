; ModuleID = '../data/hip_kernels/5182/378/main.cu'
source_filename = "../data/hip_kernels/5182/378/main.cu"
target datalayout = "e-p:64:64-p1:64:64-p2:32:32-p3:32:32-p4:64:64-p5:32:32-p6:32:32-i64:64-v16:16-v24:32-v32:32-v48:64-v96:128-v192:256-v256:256-v512:512-v1024:1024-v2048:2048-n32:64-S32-A5-G1-ni:7"
target triple = "amdgcn-amd-amdhsa"

; Function Attrs: argmemonly mustprogress nofree norecurse nosync nounwind willreturn
define protected amdgpu_kernel void @_Z30CutSubImageKernel_SingleParamsPfS_fffbiiii(float addrspace(1)* nocapture readonly %0, float addrspace(1)* nocapture writeonly %1, float %2, float %3, float %4, i1 %5, i32 %6, i32 %7, i32 %8, i32 %9) local_unnamed_addr #0 {
  %11 = tail call align 4 dereferenceable(64) i8 addrspace(4)* @llvm.amdgcn.dispatch.ptr()
  %12 = getelementptr i8, i8 addrspace(4)* %11, i64 4
  %13 = bitcast i8 addrspace(4)* %12 to i16 addrspace(4)*
  %14 = load i16, i16 addrspace(4)* %13, align 4, !range !4, !invariant.load !5
  %15 = zext i16 %14 to i32
  %16 = getelementptr inbounds i8, i8 addrspace(4)* %11, i64 12
  %17 = bitcast i8 addrspace(4)* %16 to i32 addrspace(4)*
  %18 = load i32, i32 addrspace(4)* %17, align 4, !tbaa !6
  %19 = tail call i32 @llvm.amdgcn.workgroup.id.x()
  %20 = tail call i32 @llvm.amdgcn.workgroup.id.y()
  %21 = udiv i32 %18, %15
  %22 = mul i32 %21, %15
  %23 = icmp ugt i32 %18, %22
  %24 = zext i1 %23 to i32
  %25 = add i32 %21, %24
  %26 = mul i32 %25, %20
  %27 = tail call i32 @llvm.amdgcn.workitem.id.x(), !range !15
  %28 = add i32 %26, %19
  %29 = mul i32 %28, %15
  %30 = add i32 %29, %27
  %31 = mul nsw i32 %9, %8
  %32 = icmp slt i32 %30, %31
  br i1 %32, label %33, label %104

33:                                               ; preds = %10
  %34 = add nsw i32 %6, -1
  %35 = add nsw i32 %7, -1
  %36 = tail call i32 @llvm.smin.i32(i32 %34, i32 %35)
  %37 = sitofp i32 %36 to float
  %38 = fmul contract float %37, %4
  %39 = fptosi float %38 to i32
  %40 = tail call i32 @llvm.smax.i32(i32 %39, i32 1)
  %41 = tail call i32 @llvm.smin.i32(i32 %36, i32 %40)
  %42 = sitofp i32 %6 to float
  %43 = fadd contract float %2, 1.000000e+00
  %44 = fmul contract float %43, %42
  %45 = fmul contract float %44, 5.000000e-01
  %46 = fptosi float %45 to i32
  %47 = sdiv i32 %41, 2
  %48 = sub nsw i32 %46, %47
  %49 = sitofp i32 %7 to float
  %50 = fadd contract float %3, 1.000000e+00
  %51 = fmul contract float %50, %49
  %52 = fmul contract float %51, 5.000000e-01
  %53 = fptosi float %52 to i32
  %54 = sub nsw i32 %53, %47
  br i1 %5, label %55, label %63

55:                                               ; preds = %33
  %56 = tail call i32 @llvm.smax.i32(i32 %48, i32 1)
  %57 = tail call i32 @llvm.smax.i32(i32 %54, i32 1)
  %58 = xor i32 %41, -1
  %59 = add i32 %58, %6
  %60 = tail call i32 @llvm.smin.i32(i32 %56, i32 %59)
  %61 = add i32 %58, %7
  %62 = tail call i32 @llvm.smin.i32(i32 %57, i32 %61)
  br label %63

63:                                               ; preds = %55, %33
  %64 = phi i32 [ %62, %55 ], [ %54, %33 ]
  %65 = phi i32 [ %60, %55 ], [ %48, %33 ]
  %66 = freeze i32 %30
  %67 = freeze i32 %8
  %68 = sdiv i32 %66, %67
  %69 = mul i32 %68, %67
  %70 = sub i32 %66, %69
  %71 = add nsw i32 %41, -1
  %72 = sitofp i32 %71 to float
  %73 = add nsw i32 %8, -1
  %74 = sitofp i32 %73 to float
  %75 = fdiv contract float %72, %74
  %76 = sitofp i32 %70 to float
  %77 = fmul contract float %75, %76
  %78 = fptosi float %77 to i32
  %79 = add i32 %65, %78
  %80 = icmp sgt i32 %79, -1
  br i1 %80, label %81, label %104

81:                                               ; preds = %63
  %82 = add nsw i32 %9, -1
  %83 = sitofp i32 %82 to float
  %84 = fdiv contract float %72, %83
  %85 = sitofp i32 %68 to float
  %86 = fmul contract float %84, %85
  %87 = fptosi float %86 to i32
  %88 = add nsw i32 %64, %87
  %89 = icmp sgt i32 %88, -1
  %90 = icmp slt i32 %79, %6
  %91 = select i1 %89, i1 %90, i1 false
  %92 = icmp slt i32 %88, %7
  %93 = select i1 %91, i1 %92, i1 false
  br i1 %93, label %94, label %104

94:                                               ; preds = %81
  %95 = mul nsw i32 %88, %6
  %96 = add i32 %79, %95
  %97 = sext i32 %96 to i64
  %98 = getelementptr inbounds float, float addrspace(1)* %0, i64 %97
  %99 = load float, float addrspace(1)* %98, align 4, !tbaa !16, !amdgpu.noclobber !5
  %100 = mul nsw i32 %68, %8
  %101 = add nsw i32 %100, %70
  %102 = sext i32 %101 to i64
  %103 = getelementptr inbounds float, float addrspace(1)* %1, i64 %102
  store float %99, float addrspace(1)* %103, align 4, !tbaa !16
  br label %104

104:                                              ; preds = %63, %81, %94, %10
  ret void
}

; Function Attrs: mustprogress nofree nosync nounwind readnone speculatable willreturn
declare align 4 i8 addrspace(4)* @llvm.amdgcn.dispatch.ptr() #1

; Function Attrs: mustprogress nofree nosync nounwind readnone speculatable willreturn
declare i32 @llvm.amdgcn.workitem.id.x() #1

; Function Attrs: mustprogress nofree nosync nounwind readnone speculatable willreturn
declare i32 @llvm.amdgcn.workgroup.id.x() #1

; Function Attrs: mustprogress nofree nosync nounwind readnone speculatable willreturn
declare i32 @llvm.amdgcn.workgroup.id.y() #1

; Function Attrs: nocallback nofree nosync nounwind readnone speculatable willreturn
declare i32 @llvm.smin.i32(i32, i32) #2

; Function Attrs: nocallback nofree nosync nounwind readnone speculatable willreturn
declare i32 @llvm.smax.i32(i32, i32) #2

attributes #0 = { argmemonly mustprogress nofree norecurse nosync nounwind willreturn "amdgpu-flat-work-group-size"="1,256" "frame-pointer"="none" "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="gfx906" "target-features"="+16-bit-insts,+ci-insts,+dl-insts,+dot1-insts,+dot2-insts,+dot7-insts,+dpp,+flat-address-space,+gfx8-insts,+gfx9-insts,+s-memrealtime,+s-memtime-inst,+sramecc" "uniform-work-group-size"="true" }
attributes #1 = { mustprogress nofree nosync nounwind readnone speculatable willreturn }
attributes #2 = { nocallback nofree nosync nounwind readnone speculatable willreturn }

!llvm.module.flags = !{!0, !1}
!opencl.ocl.version = !{!2}
!llvm.ident = !{!3}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 7, !"PIC Level", i32 1}
!2 = !{i32 2, i32 0}
!3 = !{!"clang version 15.0.0 (http://10.15.3.7/dcutoolkit/driverruntime/llvm-project.git 340750feeda88c9c2ce8ad481b11d9aa7f033d39)"}
!4 = !{i16 1, i16 1025}
!5 = !{}
!6 = !{!7, !11, i64 12}
!7 = !{!"hsa_kernel_dispatch_packet_s", !8, i64 0, !8, i64 2, !8, i64 4, !8, i64 6, !8, i64 8, !8, i64 10, !11, i64 12, !11, i64 16, !11, i64 20, !11, i64 24, !11, i64 28, !12, i64 32, !13, i64 40, !12, i64 48, !14, i64 56}
!8 = !{!"short", !9, i64 0}
!9 = !{!"omnipotent char", !10, i64 0}
!10 = !{!"Simple C/C++ TBAA"}
!11 = !{!"int", !9, i64 0}
!12 = !{!"long", !9, i64 0}
!13 = !{!"any pointer", !9, i64 0}
!14 = !{!"hsa_signal_s", !12, i64 0}
!15 = !{i32 0, i32 1024}
!16 = !{!17, !17, i64 0}
!17 = !{!"float", !18, i64 0}
!18 = !{!"omnipotent char", !19, i64 0}
!19 = !{!"Simple C++ TBAA"}
