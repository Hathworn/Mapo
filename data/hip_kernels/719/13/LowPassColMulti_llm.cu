...
__global__ void LowPassColMulti(float *d_Result, float *d_Data, int width, int pitch, int height)
{
    ...
    if (colStart < width) {
        ...
        for (int y = miny + ty; y <= maxy; y += blockDim.y) {
            // Cache kernel results to reduce redundant global load
            float val0 = (sdata[0*CONVCOL_W] + sdata[8*CONVCOL_W]) * kernel[0];
            float val1 = (sdata[1*CONVCOL_W] + sdata[7*CONVCOL_W]) * kernel[1];
            float val2 = (sdata[2*CONVCOL_W] + sdata[6*CONVCOL_W]) * kernel[2];
            float val3 = (sdata[3*CONVCOL_W] + sdata[5*CONVCOL_W]) * kernel[3];
            float val4 = sdata[4*CONVCOL_W] * kernel[4];
            d_Result[gPos] = val0 + val1 + val2 + val3 + val4;
            sdata += sStep;
            gPos += gStep;
        }
    }
