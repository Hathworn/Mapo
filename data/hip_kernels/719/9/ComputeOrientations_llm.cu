...
__global__ void ComputeOrientations(float *g_Data, float *d_Sift, int maxPts, int w, int h)
{
    ...
    // Compute gradients and populate orientation histogram
    for (int y=1; y < 14; y++) {
        int memPos = 16 * y + x;
        if (x >= 1 && x < 14) {
            float dy = data[memPos + 16] - data[memPos - 16];
            float dx = data[memPos + 1] - data[memPos - 1];
            int bin = static_cast<int>(16.0f * atan2f(dy, dx) / 3.1416f + 16.5f) & 31; // Ensure bin is within bounds
    ...
    // Smooth histogram
    if (tx < 32) {
        int left = (tx == 0) ? 31 : tx - 1;
        int right = (tx == 31) ? 0 : tx + 1;
        hist[32 + tx] = 6 * hist[tx] + 4 * (hist[right] + hist[left]) +
                        (hist[(right + 1) & 31] + hist[(left + 31) & 31]);
    }
    __syncthreads();
    ...
    // Find main and secondary orientation
    if (tx == 0) {
