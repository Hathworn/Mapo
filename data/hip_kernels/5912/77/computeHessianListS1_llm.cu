...
__global__ void computeHessianListS1(float *trans_x, float *trans_y, float *trans_z, int *valid_points, int *starting_voxel_id, int *voxel_id, int valid_points_num, double *centroid_x, double *centroid_y, double *centroid_z, double gauss_d1, double gauss_d2, double *hessians, double *e_x_cov_x, double *tmp_hessian, double *cov_dxd_pi, double *point_gradients, int valid_voxel_num)
{
    ...
    if (row < 6 && col < 6) {
        ...
        for (int i = id; i < valid_points_num; i += stride) {
            ...
            int start_vox_id = starting_voxel_id[i];
            int end_vox_id = starting_voxel_id[i + 1];
            double gauss_d2_neg = -gauss_d2;  // Cache repeated computation

            for (int j = start_vox_id; j < end_vox_id; j++) {
                int vid = voxel_id[j];

                double tmp_ex = e_x_cov_x[j];

                // Check if tmp_ex is valid.
                bool is_tmp_ex_valid = !(tmp_ex > 1 || tmp_ex < 0 || tmp_ex != tmp_ex);
                if (is_tmp_ex_valid) {
                    double cov_dxd0 = cov_dxd_pi_mat0[j];
                    double cov_dxd1 = cov_dxd_pi_mat1[j];
                    double cov_dxd2 = cov_dxd_pi_mat2[j];

                    tmp_ex *= gauss_d1;  // Directly apply gauss_d1

                    double tmp_h_j = tmp_h[j];  // Cache repeated access

                    final_hessian += gauss_d2_neg * ((d_x - centroid_x[vid]) * cov_dxd0 + (d_y - centroid_y[vid]) * cov_dxd1 + (d_z - centroid_z[vid]) * cov_dxd2) * tmp_h_j * tmp_ex;
