...
{
    // Calculate unique global thread index. Use faster operations for this task.
    int id = blockIdx.x * blockDim.x + threadIdx.x;
    if (id >= size) return;

    // Unroll the index calculations to improve performance.
    int i = id % minw;
    int j = (id / minw) % minh;
    int k = (id / (minw * minh)) % minc;
    int b = (id / (minw * minh * minc)) % batch;

    // Optimize index calculations for memory access
