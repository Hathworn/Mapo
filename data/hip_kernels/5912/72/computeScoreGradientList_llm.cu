...
__global__ void computeScoreGradientList(float *trans_x, float *trans_y, float *trans_z, int *valid_points, int *starting_voxel_id, int *voxel_id, int valid_points_num, double *centroid_x, double *centroid_y, double *centroid_z, int voxel_num, double *e_x_cov_x, double *cov_dxd_pi, double gauss_d1, int valid_voxel_num, double *score_gradients)
{
    ...
    if (col < 6) {
        ...
        for (int i = id; i < valid_points_num; i += stride) {
            ...
            int end_voxel = starting_voxel_id[i + 1];

            for (int j = start_voxel; j < end_voxel; j++) {
                int vid = voxel_id[j];
                double tmp_ex = e_x_cov_x[j];

                if (tmp_ex <= 1 && tmp_ex >= 0) { // Simplified condition check for valid range
                    tmp_ex *= gauss_d1;
                    ...
                    // Optimized computation using pre-calculated differences
                    tmp_sg += (diff_x * cov_dxd_pi_mat0[j] + diff_y * cov_dxd_pi_mat1[j] + diff_z * cov_dxd_pi_mat2[j]) * tmp_ex;
                }
            }

            sg[i] = tmp_sg; // Update score gradient for the current point
        }
    }
}