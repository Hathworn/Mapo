    ...
    if (row < 6 && col < 6) {
        ...
        for (int i = id; i < valid_points_num; i += stride) {
            ...
            for (int j = starting_voxel_id[i]; j < starting_voxel_id[i + 1]; j++) {
                ...
                // Check and continue if tmp_ex is not valid
                if (tmp_ex <= 1 && tmp_ex >= 0 && tmp_ex == tmp_ex) {
                    tmp_ex *= gauss_d1;

                    // Compute final hessian
                    double cx = d_x - centroid_x[vid];
                    double cy = d_y - centroid_y[vid];
                    double cz = d_z - centroid_z[vid];

                    final_hessian += (cx * (icov00[vid] * ph0 + icov01[vid] * ph1 + icov02[vid] * ph2)) * tmp_ex;
                    final_hessian += (cy * (icov10[vid] * ph0 + icov11[vid] * ph1 + icov12[vid] * ph2)) * tmp_ex;
                    final_hessian += (cz * (icov20[vid] * ph0 + icov21[vid] * ph1 + icov22[vid] * ph2)) * tmp_ex;
                }
            }
