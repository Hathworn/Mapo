...
__global__ void computePointHessian1(float *x, float *y, float *z, int points_num, int *valid_points, int valid_points_num, double *dh_ang, double *ph124, double *ph134, double *ph144, double *ph154, double *ph125, double *ph164, double *ph135, double *ph174, double *ph145)
{
    ...
    // Load shared memory once, improve access speed
    if (threadIdx.x < 18) {
    ...
    for (int i = id; i < valid_points_num; i += stride) {
        ...
        // Convert to double once to avoid redundant casting
        ...
        // Compute once, update multiple outputs
        double temp1 = o_x * h_ang[9] + o_y * h_ang[10] + o_z * h_ang[11];
        ph154[i] = ph125[i] = temp1;

        double temp2 = o_x * h_ang[12] + o_y * h_ang[13] + o_z * h_ang[14];
        ph164[i] = ph135[i] = temp2;

        double temp3 = o_x * h_ang[15] + o_y * h_ang[16] + o_z * h_ang[17];
        ph174[i] = ph145[i] = temp3;
    }
}