...
__global__ void matrix_atan(double *A, double *C, unsigned int size) {
    ...
    __shared__ double sharedA[256]; // assuming blockDim.x <= 256
    int index = blockIdx.x * blockDim.x + threadIdx.x;
    if(index < size) {
        sharedA[threadIdx.x] = A[index]; // Load into shared memory
        __syncthreads();
        C[index] = atan(sharedA[threadIdx.x]); // Compute atan using shared memory
    }
}