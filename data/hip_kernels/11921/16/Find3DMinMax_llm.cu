...
__global__ void Find3DMinMax(int *d_Result, float *d_Data1, float *d_Data2, float *d_Data3, int width, int pitch, int height) {
    ...
    for (int y = 0; y < 34; y++) {
        ...
        // Load data into shared memory efficiently by ensuring all threads participate
        ...
        if (y > 1) {
            if (tx < memWid) {
                // Combine min/max evaluations to reduce instructions
        ...
        if (y > 1) {
            if (tx < MINMAX_W) {
                if (xWritePos <= xEndClamped) {
                    // Unroll loops for reduced instruction overhead
