...
__global__ void FindMinMax(float *d_MinMax, float *d_Data, int width, int height)
{
    ...
    // Load initial values into shared memory, handling edge case
    if (x < width) {
        float val = d_Data[p];
        minvals[tx] = val;
        maxvals[tx] = val;
    ...
    // Iterate over rows within a block
    for (int ty = 1; ty < 16; ty++) {
        p += width;
        if (x < width) {  // Fix condition to be independent of tx
            float val = d_Data[p];
            if (val < minvals[tx])
                minvals[tx] = val;
            if (val > maxvals[tx])
    ...
    // Parallel reduction to find block min and max
    for (int d = b / 2; d > 0; d /= 2) {  // Modify loop to use better reduction pattern
        if (tx < d) {
