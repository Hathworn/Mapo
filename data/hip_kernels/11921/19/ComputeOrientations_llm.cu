...
__global__ void ComputeOrientations(float *g_Data, int *d_Ptrs, float *d_Orient, int maxPts, int w, int h)
{
    ...
    for (int y = 1; y < 14; y++) {
        ...
        if (x >= 1 && x < 14) {
            ...
            int bin = (int)(16.0f * atan2f(dy, dx) / 3.1416f + 16.5f);
            bin = (bin == 32) ? 0 : bin;
    ...
    // Reduce histograms in a tree-like fashion
    for (int y = 0; y < 5; y++)
    ...
    // Smooth histogram with a weighted sum
    if (tx < 32) {
        hist[32 + tx] = 6 * hist[tx] 
            + 4 * (hist[(tx + 1) & 31] + hist[(tx - 1) & 31])
            + (hist[(tx + 2) & 31] + hist[(tx - 2) & 31]);
    }
    __syncthreads();
    
    // Determine orientation with maximum response
    float v = hist[32 + tx];
    ...
    if (tx == 0) {
        ...
        // Compute peak positions and orientations
