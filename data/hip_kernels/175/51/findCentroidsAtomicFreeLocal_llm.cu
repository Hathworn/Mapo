...
__global__ void findCentroidsAtomicFreeLocal(int afLocal, int* responses, int nPixels, int* cluster, int* centroidMass, unsigned int* centroidCount)
{
    ...
    int pixel_end = min((af_id + 1) * afLocal, nPixels);  // Optimize calculation with 'min' function
    ...
    int idx = (af_id * gridDim.y * blockDim.x) + (filter_id * 32) + cluster_id;  // Clarify and reformat calculation
