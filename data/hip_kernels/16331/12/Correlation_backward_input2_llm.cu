...
{
    // Calculate shared data and indices
    int n = item;
    ...
    for (int tc = tch_off; tc < nOutputChannels; tc += CUDA_NUM_THREADS) {
        ...
        int xmin = max(0, (x - kernel_rad - max_displacement - i2) / stride1);
        int ymin = max(0, (y - kernel_rad - max_displacement - j2) / stride1);
        int xmax = min(outputWidth - 1, (x + kernel_rad - max_displacement - i2) / stride1);
        int ymax = min(outputHeight - 1, (y + kernel_rad - max_displacement - j2) / stride1);
