...
// Optimized global kernel for vec_expm1f
__global__ void vec_expm1f(size_t n, float *result, const float *x)
{
    // Calculate the global thread identifier
    int id = blockIdx.x * blockDim.x + threadIdx.x;

    // Ensure the thread is within the bounds of the array
    if (id < n)
    {
        // Apply expm1f function to each element
