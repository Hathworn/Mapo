...
// Optimized kernel function using shared memory for better memory access performance
__global__ void vec_exp2f(size_t n, float *result, float *x) {
    // Allocate shared memory for the block
    __shared__ float shared_x[256]; // Assuming block size of 256
    ...
    if (id < n) {
        ...
        result[id] = exp2f(shared_x[threadIdx.x]);
    }
}