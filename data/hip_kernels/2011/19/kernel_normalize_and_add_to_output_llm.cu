...
{
    const unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;  // Direct arithmetic
    const unsigned int j = blockIdx.y * blockDim.y + threadIdx.y;
    const unsigned int k = blockIdx.z * blockDim.z + threadIdx.z;

    if (i >= c_volSize.x || j >= c_volSize.y || k >= c_volSize.z) return;
    
    const long int out_idx = i + (j + k * c_volSize.y) * (c_volSize.x);  // Row-major index calculation
    
    const float eps = 1e-6f;

    if (c_normalize) {
        // Use fabsf for float absolute value
        dev_vol_out[out_idx] = fabsf(dev_accumulate_weights[out_idx]) > eps ?
