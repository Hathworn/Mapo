...
__global__ void upscale(float *input, float *output, long no_elements, int scale_factor, int d1, int d2, int d3) {
    ...
    if (ii >= no_elements) return; // Check boundary
    int ipidx = translate_idx(ii, d1, d2, d3, scale_factor); // Translate index
    output[ii] = input[ipidx]; // Copy scaled input to output
}