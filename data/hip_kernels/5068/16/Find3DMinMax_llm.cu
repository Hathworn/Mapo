...
// Optimize memory access by reducing redundancy and enhancing parallelism
__global__ void Find3DMinMax(int *d_Result, float *d_Data1, float *d_Data2, float *d_Data3, int width, int pitch, int height) {
    ...
    const int xEnd = min(xStart + MINMAX_W, width) - 1;
    ...
    for (int y = 0; y < 34; y++) { // Combine loop limits
        output >>= 1;
        ...
        if (y > 1 && tx < MINMAX_W && xWritePos <= xEnd) {
