...
__global__ void ComputeOrientations(float *g_Data, float *d_Sift, int maxPts, int w, int h)
{
    ...
    // Compute gradient and build histogram
    for (int y = 1; y < 14; y++) {
        int memPos = 16 * y + x;
        if (x >= 1 && x < 14) {
            float dy = data[memPos + 16] - data[memPos - 16];
            float dx = data[memPos + 1] - data[memPos - 1];
            int bin = __float2int_rn(16.0f * atan2f(dy, dx) / 3.1416f + 16.5f) & 31;  // Optimized bin calculation
    ...
    for (int offset = 8; offset > 0; offset /= 2) {
        if (offset <= 5) {
            int y = tx / 32 * offset;
            if (tx < offset * 32)
                hist[y * 32 + tx] += hist[(y + (8 / offset) * 4) * 32 + tx];
            __syncthreads();
        }
    }
    ...
    {
        float val = hist[tx];
        float smoothed = 6 * val + 4 * (hist[(tx + 1) & 31] + hist[(tx + 31) & 31]) + 
                         (hist[(tx + 2) & 31] + hist[(tx + 30) & 31]);
        hist[32 + tx] = smoothed;
    }
    __syncthreads();
    ...
    if (tx == 0) {
        ...
        // Quadratic interpolation to find precise peak position
