...
__global__ void LowPassRowMulti(float *d_Result, float *d_Data, int width, int pitch, int height)
{
    ...
    // Load into shared memory with boundary checks
    if (loadPos < 0)
        data[tx] = d_Data[yptr];
    ...
    // Ensure write only happens for valid xout values
    if (xout < width) {
        // Reorganized computation for better ILP
        float sum = (data[tx] + data[tx + 8]) * kernel[0];
        sum += (data[tx + 1] + data[tx + 7]) * kernel[1];
        sum += (data[tx + 2] + data[tx + 6]) * kernel[2];
        sum += (data[tx + 3] + data[tx + 5]) * kernel[3];
