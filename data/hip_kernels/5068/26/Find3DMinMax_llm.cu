...
__global__ void Find3DMinMax(int *d_Result, float *d_Data1, float *d_Data2, float *d_Data3, int width, int height)
{
    ...
    for (int y = 0; y < 34; y++) {
        ...
        if (y > 1 && tx < memWid) {
            // Use min/max functions with fused operations
        ...
        if (y > 1 && tx < MINMAX_SIZE && xWritePos <= xEndClamped) {
            // Calculate min/max values considering both cache and data
            float minv = fminf(ymin2[tx], fminf(ymin2[tx + 2], fminf(ymin1[tx + 1], 
                    fminf(ymin3[tx + 1], fminf(data2[memPos0 + 1], data2[memPos2 + 1])))));
            minv = fminf(minv, d_ConstantA[1]);
            float maxv = fmaxf(ymax2[tx], fmaxf(ymax2[tx + 2], fmaxf(ymax1[tx + 1], 
                    fmaxf(ymax3[tx + 1], fmaxf(data2[memPos0 + 1], data2[memPos2 + 1])))));
