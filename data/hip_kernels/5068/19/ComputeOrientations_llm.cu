...
__global__ void ComputeOrientations(float *g_Data, int *d_Ptrs, float *d_Orient, int maxPts, int w, int h)
{
    ...
    for (int y = 1; y < 14; y++) {
        ...
        if (x >= 1 && x < 14) {
            ...
            atomicAdd(&hist[32 * (x - 1) + bin], grad * gauss[x] * gauss[y]);  // Use atomicAdd for concurrent updates
        }
    }
    __syncthreads();
    ...
    // Reduce histogram
    if (tx < 32) {
        for (int offset = 5; offset >= 1; offset >>= 1) {
            if (tx + offset < 32 && offset > 1)
                hist[tx] += hist[(offset + tx) * 32];
            __syncthreads();
        }
        hist[tx] += hist[tx + 32];
    }
