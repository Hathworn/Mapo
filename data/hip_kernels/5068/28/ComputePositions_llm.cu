...
__global__ void ComputePositions(float *g_Data1, float *g_Data2, float *g_Data3, int *d_Ptrs, float *d_Sift, int numPts, int maxPts, int w, int h)
{
    ...
    // Load values reused multiple times into registers
    ...
    // Use temporary variables to reduce redundant global memory accesses
    float val7 = g_Data2[p+w+1];
    float val8 = g_Data2[p-w-1];
    float val9 = g_Data2[p-w+1];
    float val10 = g_Data2[p+w-1];

    float dxy = 0.25f * (val7 + val8 - val9 - val10);
    
    float val11 = g_Data3[p+1];
    float val12 = g_Data1[p-1];
    
    float dxs = 0.25f * (val11 + val12 - g_Data1[p+1] - g_Data3[p-1]);

    float val13 = g_Data3[p+w];
    float val14 = g_Data1[p-w];

    float dys = 0.25f * (val13 + val14 - g_Data3[p-w] - g_Data1[p+w]);
