...
__global__ void Correlation_backward_input2(int item, float *gradInput2, int nInputChannels, int inputHeight, int inputWidth, float *gradOutput, int nOutputChannels, int outputHeight, int outputWidth, float *rInput1, int pad_size, int kernel_size, int max_displacement, int stride1, int stride2)
{
    ...
    // Optimized shared memory allocation and usage
    extern __shared__ float prod_sum_shared[];
    prod_sum_shared[tch_off] = 0;

    // Compute all contributing gradOutput elements
    for (int tc = tch_off; tc < nOutputChannels; tc += blockDim.x) {
        ...
        for (int j = ymin; j <= ymax; ++j) {
            for (int i = xmin; i <= xmax; ++i) {
                ...
                prod_sum_shared[tch_off] += gradOutput[tindx] * val1;
            }
        }
    }
    ...
    // Reduce and store final result
    if (tch_off == 0) {
        float reduce_sum = 0;
        for (int idx = 0; idx < blockDim.x; idx++) {
            reduce_sum += prod_sum_shared[idx];
        }
