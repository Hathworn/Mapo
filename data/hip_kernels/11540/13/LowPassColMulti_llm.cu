...
__global__ void LowPassColMulti(float *d_Result, float *d_Data, int width, int pitch, int height)
{
    ...
    if (colStart < width) {
        ...
        for (int y = miny + ty; y <= maxy; y += blockDim.y) {
            float sum1 = sdata[0 * CONVCOL_W] + sdata[8 * CONVCOL_W];
            float sum2 = sdata[1 * CONVCOL_W] + sdata[7 * CONVCOL_W];
            float sum3 = sdata[2 * CONVCOL_W] + sdata[6 * CONVCOL_W];
            float sum4 = sdata[3 * CONVCOL_W] + sdata[5 * CONVCOL_W];
            d_Result[gPos] = sum1 * kernel[0] + sum2 * kernel[1] +
                             sum3 * kernel[2] + sum4 * kernel[3] +
