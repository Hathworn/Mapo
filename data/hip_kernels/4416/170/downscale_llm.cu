...
__global__ void downscale(float *gradInput_data, const float *gradOutput_data, long no_elements, int scale_factor, int d1, int d2, int d3) {
    ...
    for (int offset_x = 0; offset_x < scale_factor; ++offset_x) {
        for (int offset_y = 0; offset_y < scale_factor; ++offset_y) {
            int ipidx = translate_idx_inv(ii, d1, d2, d3, scale_factor, offset_x, offset_y);
