...
__global__ void avg_vote_kernel(float* A, float* B, int* corrAB, int patch, int c, int h, int w) {
    ...
    if (_id < c * size) {
        ...
        int base_idx1 = dc * size;
        int base_corr_idx = id * 2;

        // Use shared memory for tiling
        __shared__ float shared_A[TB];
            ...
            for (int dy = -radius; dy <= radius; dy++) {
                ...
                if (new_x1 >= 0 && new_x1 < w && new_y1 >= 0 && new_y1 < h) {
                    ...
                    if (new_x2 >= 0 && new_x2 < w && new_y2 >= 0 && new_y2 < h) {
                        ...
                        if (threadIdx.x < TB) {
                            shared_A[threadIdx.x] = A[base_idx1 + new_id2];
                        }
                        __syncthreads();

                        sum += shared_A[threadIdx.x];
                        cnt++;
                    }
                }
            }
