...
__global__ void patchmatch2_conv_kernel( float *A, float *B, float *AP, float *BP, float *conv, int *prev_corrAB_upsampled, int patch, int s_rad, int c, int h, int w )
{
    ...
    if (_id < size1 * s_n) {
        ...
        // Shared memory for local tiles for better memory access
        extern __shared__ float shared_A[];
        float *shared_B = shared_A + (c * patch * patch);
            ...
            for (int dx = -kernel_radius; dx <= kernel_radius; dx++) {
                ...
                if (0 <= xx1 && xx1 < w1 && 0 <= yy1 && yy1 < h1 &&
                    ...
                    for (int dc = 0; dc < c; dc++) {
                        shared_A[dc * patch * patch + (dy + kernel_radius) * patch + (dx + kernel_radius)] = A[dc * size1 + _id1];
                        shared_B[dc * patch * patch + (dy + kernel_radius) * patch + (dx + kernel_radius)] = B[dc * size2 + _id2];
                    }
                    cnt++;
                }
            }
        ...
        // Calculate convolution using shared memory
        for (int dy = -kernel_radius; dy <= kernel_radius; dy++) {
            for (int dx = -kernel_radius; dx <= kernel_radius; dx++) {
                for (int dc = 0; dc < c; dc++) {
                    int idx = dc * patch * patch + (dy + kernel_radius) * patch + (dx + kernel_radius);
                    conv_result += shared_A[idx] * shared_B[idx];
                    
                    // Adding AP and BP terms
                    float term1 = AP[dc * size1 + _id];
                    float term2 = BP[dc * size2 + _id];
