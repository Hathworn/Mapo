...
__global__ void upsample_corr_kernel(int *curr_corr, int *next_corr, int curr_h, int curr_w, int next_h, int next_w)
{
    ...
    if (id < next_h * next_w) {
        ...
        int curr_x = (next_x + 0.5f) / w_ratio;
        int curr_y = (next_y + 0.5f) / h_ratio;
        ...
        int next_x2 = next_x + (curr_x2 - curr_x) * w_ratio + 0.5f;
        int next_y2 = next_y + (curr_y2 - curr_y) * h_ratio + 0.5f;
