...
__global__ void patchmatch_r_conv_kernel( float *input, float *target, float *conv, int patch, int stride, int c1, int h1, int w1, int h2, int w2 )
{
    ...
    if (id < N) {
        ...
        // Initialize shared memory for input and target patches
        extern __shared__ float shared_data[];
        float* input_shared = shared_data;
        float* target_shared = shared_data + c1 * TB;
        ...
        for (int dy = -kernel_radius; dy <= kernel_radius; dy += stride) {
            for (int dx = -kernel_radius; dx <= kernel_radius; dx += stride) {
                ...
                if (0 <= xx1 && xx1 < w1 && 0 <= yy1 && yy1 < h1 &&
                    ...
                    for (int c = 0; c < c1; c++) {
                        if (threadIdx.x < c1) {
                            input_shared[threadIdx.x] = input[c * size1 + _id1];
                            target_shared[threadIdx.x] = target[c * size2 + _id2];
                        }
                    }
                    __syncthreads(); // Ensure all threads have loaded their data

                    // Compute the convolution and norms
                    for (int c = 0; c < c1; c++) {
                        float term1 = input_shared[c];
                        float term2 = target_shared[c];
                        conv_result += term1 * term2;
                        norm_1      += term1 * term1;
                        norm_2      += term2 * term2;
                    }
                    __syncthreads(); // Ensure all threads are done before the next iteration
                }
            }
        }
