...
__global__ void LowPassRowMulti(float *d_Result, float *d_Data, int width, int pitch, int height)
{
    ...
    // Use a compact loop for convolution calculation.
    if (xout < width && tx < CONVROW_W) {
        float sum = data[tx+0] * kernel[0] + data[tx+8] * kernel[0];
        sum += data[tx+1] * kernel[1] + data[tx+7] * kernel[1];
        sum += data[tx+2] * kernel[2] + data[tx+6] * kernel[2];
        sum += data[tx+3] * kernel[3] + data[tx+5] * kernel[3];
