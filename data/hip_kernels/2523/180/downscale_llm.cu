...
__global__ void downscale(float *gradInput_data, const float *gradOutput_data, long no_elements, int scale_factor, int d1, int d2, int d3) {
    ...
    // Early exit if the thread index exceeds the number of elements
    if (ii >= no_elements) return;
    
    // Pre-calculate the base index
    int ii_base = ii;
    
    // Loop unrolling to improve performance; handles scale_factor iterations
    for (int i = 0; i < scale_factor; ++i) {
        for (int j = 0; j < scale_factor; ++j) {
            int ipidx = translate_idx_inv(ii_base, d1, d2, d3, scale_factor, i, j);
            gradInput_data[ii_base] += gradOutput_data[ipidx];
        }
    }
}