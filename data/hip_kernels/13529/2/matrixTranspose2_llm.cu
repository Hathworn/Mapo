...
// Optimize data access and shared memory usage for better performance
__global__ void matrixTranspose2(const float *F, float *C)
{
    ...
    // Use unrolled loop for efficient memory access
    #pragma unroll
    ...
    // Unrolled loop for improved memory writing
    #pragma unroll
    for (int j = 0; j < TILE_DIM; j += BLOCK_ROWS)
    {
