...
__global__ void gGRUFastBackward(float* outState, float* outXW, float* outSU, float* outB, const float* state, const float* xW, const float* sU, const float* b, const float* mask, const float* adj, size_t rows, size_t cols, bool final) {
    int j = blockIdx.x + blockIdx.y * gridDim.x;  // Calculate unique block ID
    if(j < rows) {
        float m = !mask || mask[j];
        ...
        for(int i = threadIdx.x; i < cols; i += blockDim.x) {  // Use thread index directly
            int k = i + cols;
            int l = i + 2 * cols;
            ...
            // df/d(xW_r) ...
            float dfdxW_r = m * r * (1 - r) * t * adj * (final ? (rowSU[l] + b[l]) : rowSU[l]);
            if(outXW)
                rowOutXW[i] += dfdxW_r;
            if(outSU)
                rowOutSU[i] += dfdxW_r;
            ...
            if(outB)
                atomicAdd(outB + l, dfdxW_x * (final ? r : 1));
        }
    }
}