...
__global__ void power_spectrum_kernel(int row_length, float *A_in, int32_t ldi, float *A_out, int32_t ldo, bool use_power) {
    ...
    // Unrolling loop for better performance and coalesced memory access
    for (int idx = thread_id; idx < half_length; idx += blockDim.x) {
        if (idx == 0) continue; // Skip first index in this loop

        float2 val = reinterpret_cast<float2 *>(Ar)[idx];
        float ret = val.x * val.x + val.y * val.y;
        Aw[idx] = use_power ? ret : sqrtf(ret); // Using ternary operator for clarity
    }

    // Handle the special cases outside the loop to minimize conditional checks
    if (thread_id == 0) {
        float real = Ar[0];
