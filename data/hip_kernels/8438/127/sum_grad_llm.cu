...
__global__ void sum_grad(float* in, int inSize0, int inSize1, int inSize2, int inSize3, int nElement, float* out, int outStride0, int outStride1, int outStride2, int dim) {
    ...
    for (int i = tid; i < nElement; i += stride) {
        // Calculate indices using modulo and division for better readability
        int inDim3 = i % inSize3;
        int inOff2 = i / inSize3;
        int inDim2 = inOff2 % inSize2;
        int inOff1 = inOff2 / inSize2;
        int inDim1 = inOff1 % inSize1;
        int inDim0 = inOff1 / inSize1;
        
        // Use a switch statement for dim to simplify code
        int outOff;
        switch (dim) {
            case 0:
            ...
            case 3:
                ...
                break;
            default:
                return; // If dim is invalid, return immediately
        }
