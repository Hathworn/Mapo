...
__global__ void ComputeOrientations(float *g_Data, int *d_Ptrs, float *d_Orient, int maxPts, int w, int h)
{
    ...
    for (int y = 0; y < 15; y++) {
        ...
        int xi = xp + x;
        int yi = yp + y;
        xi = min(max(xi, 0), w - 1);  // Clamp boundary
        yi = min(max(yi, 0), h - 1);  // Clamp boundary
        if (x >= 0 && x < 15)
            data[memPos] = g_Data[yi * w + xi];
    }
    __syncthreads();

    // Compute gradient and histogram
    for (int y = 1; y < 14; y++) {
        int memPos = 16 * y + x;
        if (x >= 1 && x < 14) {
    ...
    // Sum histograms across different blocks
    ...
    // Smooth the histogram using a filter
    if (tx == 0)
    ...
    // Find top peaks in the histogram
    if (tx == 0) {
