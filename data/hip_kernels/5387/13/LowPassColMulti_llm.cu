...
__global__ void LowPassColMulti(float *d_Result, float *d_Data, int width, int pitch, int height)
{
    ...
    if (colStart < width) {
        ...
        // Unrolled loop to maximize memory coalescing and reduce arithmetic operations 
        for (int y = totStart + ty; y <= totEnd; y += blockDim.y) {
            float value;
            if (y < 0)
                value = d_Data[colStart];
            else if (y >= height)
                value = d_Data[colEnd];
            else
                value = d_Data[gPos];
    ...
    if (colStart < width) {
        ...
        // Unrolled computation for improved execution efficiency
