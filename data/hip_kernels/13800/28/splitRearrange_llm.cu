...
__global__ void splitRearrange (int numElems, int iter, unsigned int* keys_i, unsigned int* keys_o, unsigned int* values_i, unsigned int* values_o, unsigned int* histo){
    ...
    // Load histogram data into shared memory, optimize by using multiple threads
    ...
    // Calculate masks and store them in shared memory
    ...
    // Compute new index using shared memory histogram data
    ...
    // Correct indices based on matches in masks
    ...
    // Write output values only if index is within bounds
    if (index < numElems){
