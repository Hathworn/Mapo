...
__global__ void set_row_perm(int *d_bin_size, int *d_bin_offset, int *d_max_row_nz, int *d_row_perm, int M, int min, int mmin)
{
    ...
    // Loop unrolling to reduce the number of comparisons
    if (nz_per_row <= (min << 0)) {
        if (nz_per_row <= mmin) {
    ...
    // Assume more iterations based on BIN_NUM structure could be here
    // as per this same pattern
    
    else { // Default case for the last bin
