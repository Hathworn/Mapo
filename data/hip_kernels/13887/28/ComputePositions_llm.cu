...
__global__ void ComputePositions(float *g_Data1, float *g_Data2, float *g_Data3, int *d_Ptrs, float *d_Sift, int numPts, int maxPts, int w, int h) {
    ...
    // Load values into shared memory to reduce redundant global memory accesses
    __shared__ float sharedMem[POSBLK_SIZE][7];
    sharedMem[threadIdx.x][0] = g_Data2[p];
    sharedMem[threadIdx.x][1] = g_Data2[p - 1];
    sharedMem[threadIdx.x][2] = g_Data2[p + 1];
    sharedMem[threadIdx.x][3] = g_Data2[p - w];
    sharedMem[threadIdx.x][4] = g_Data2[p + w];
    sharedMem[threadIdx.x][5] = g_Data3[p];
    sharedMem[threadIdx.x][6] = g_Data1[p];
    __syncthreads();

    // Use shared memory values
    float* val = sharedMem[threadIdx.x];
    ...
    // Optimize arithmetic and operations usage
    ...
    // Reduce divisions using __fdividef where it is useful
