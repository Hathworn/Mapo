...
{
    __shared__ float data[16 * 15]; // Shared array to store data
    __shared__ float hist[32 * 13]; // Shared histogram bins
    __shared__ float gauss[16]; // Gaussian weights for smoothing
    ...
    // Calculate Gaussian weights, only once
    ...
    for (int y = 0; y < 15; y++) {
        int memPos = 16 * y + x;
        int xi = max(0, min(xp + x, w - 1));
        int yi = max(0, min(yp + y, h - 1));
        if (x >= 0 && x < 15) 
