...
__global__ void reduceCompleteUnrollWarps8Float(float *g_idata, float *g_odata, unsigned int n)
{
    ...
    // unrolling 8
    float sum = 0; // Accumulate sum in register
    if (idx + 7 * blockDim.x < n)
    {
        ...
        idata[tid] = sum; // Store the computed sum in shared memory
    }
    ...
    if (tid < 32)
    {
        volatile float *vsmem = idata; // Use volatile pointer to avoid register optimizations
