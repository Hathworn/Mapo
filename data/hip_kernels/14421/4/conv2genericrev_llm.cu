...
__global__ void conv2genericrev(float *input, float *kernel, float *output, int input_n, int input_h, int input_w, int kernel_n, int kernel_h, int kernel_w, float alpha, int stride_h, int stride_w)
{
    ...
    // Use shared memory for the output
    extern __shared__ float shared_output[];

    // Map each thread to a specific output position
    float *output_s = shared_output + (tid * output_h * output_w);

    // Conduction convolution
    int xx, yy, kx, ky;
    yy = threadIdx.y;
    ...
    for (xx = 0; xx < output_w; xx++) {
        ...
        // Unrolled loop for more efficient memory access
        for (ky = 0; ky < kernel_h; ky++, input_p += input_w, kernel_p += kernel_w) {
    ...
    // Aggregate results and write back
    if (yy == 0) {
