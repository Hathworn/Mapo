...
__global__ void patchmatch2_conv_kernel( float *A, float *B, float *AP, float *BP, float *conv, int *prev_corrAB_upsampled, int patch, int s_rad, int c, int h, int w )
{
    ...
    if (_id < size1 * s_n) {
        ...
        // Use shared memory for efficient local data re-use
            ...
            for (int dx = -kernel_radius; dx <= kernel_radius; dx++) {
                    ...
                    0 <= xx2 && xx2 < w2 && 0 <= yy2 && yy2 < h2)
                {
                    ...
                    for (int dc = 0; dc < c; dc++) {
                        sharedA[threadIdx.x] = A[dc * size1 + _id1];
                        sharedB[threadIdx.x] = B[dc * size2 + _id2];
                        conv_result += sharedA[threadIdx.x] * sharedB[threadIdx.x];

                        sharedAP[threadIdx.x] = AP[dc * size1 + _id1];
                        sharedBP[threadIdx.x] = BP[dc * size2 + _id2];
                        conv_result += sharedAP[threadIdx.x] * sharedBP[threadIdx.x];
                    }
                    cnt++;
                }
            }
