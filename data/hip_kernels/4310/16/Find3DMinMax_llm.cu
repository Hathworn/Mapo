...
__global__ void Find3DMinMax(int *d_Result, float *d_Data1, float *d_Data2, float *d_Data3, int width, int pitch, int height)
{
    ...
    for (int y=0; y < 34; y++) { // Initialize y loop and utilize constant limit instad of (32+2)
        output >>= 1;
        ...
        if (tx >= (WARP_SIZE - 1) && xReadPos >= 0 && xReadPos < width) { // Combine conditions and access only valid indices
        ...
        if (y > 1 && tx < memWid) {
            ...
            // Store min and max in shared memory
            ymin1[tx] = min1;
        ...
        if (y > 1 && tx < MINMAX_W && xWritePos <= xEndClamped) {
            // Calculate overall min and max with threshold checks
        ...
        // Rotate positions in shared memory
        memPos0 = memPos1;
        memPos1 = memPos2;
        yq = (yq < 2 ? yq + 1 : 0);
    }
