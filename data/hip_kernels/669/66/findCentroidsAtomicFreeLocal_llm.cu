...
__global__ void findCentroidsAtomicFreeLocal(int afLocal, int* responses, int nPixels, int* cluster, int* centroidMass, unsigned int* centroidCount)
{
    ...
    // Use a stride to minimize divergence
    for (int i = pixel_start; i < pixel_end; i += blockDim.x * gridDim.x) {
        if (i < nPixels && cluster[i] == cluster_id) {
            local_responses += filter_responses[i];
            local_count++;
        }
    }

    // Use a unique index for writing results
