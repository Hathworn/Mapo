...
__global__ void findLabels(int nPixels, int filterCount, int clusterCount, float* responses, float* centroids, int* clusters, int* changes) {
    ...
    // Load first half of centroids into shared memory
    if (threadIdx.x < 32) {
        for (int i = 0; i < 34; i++) {
    ...
    // Compute distances for first half of clusters
    if (x < nPixels) {
    ...
    // Load second half of centroids into shared memory
    if (threadIdx.x < 32) {
        for (int i = 0; i < 34; i++) {
    ...
    // Compute distances for second half of clusters
    if (x < nPixels) {
        for (int label = 0; label < 32; label++) {
            float accumulant = 0.0f;
            int index = x;
    ...
    // Aggregate change counts across threads in a block
    if (threadIdx.x == 0) {
