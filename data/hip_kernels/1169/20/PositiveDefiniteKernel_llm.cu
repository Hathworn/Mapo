...
__global__ void PositiveDefiniteKernel( char *hessian_pd, float *hessian, int imageW, int imageH, int imageD )
{
    ...
    // Ensure the indices are within grid boundaries
    if (baseX >= imageW || baseY >= imageH || baseZ >= imageD) return;
    
    const int size = imageW * imageH * imageD;
    const int idx = (baseZ * imageH + baseY) * imageW + baseX;

    // Load hessian matrix elements efficiently
    ...
    // Convert boolean result to char
    hessian_pd[idx] = static_cast<char>(isPositiveDefinite);
}