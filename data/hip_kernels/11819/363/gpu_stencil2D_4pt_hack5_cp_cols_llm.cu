...
// Compute indices once and reuse them for efficiency
int idx = 2 * M * blockIdx.x + t + base_global_row;
int dst_idx_base = base_global_idx + t * N;
int idx_nextCol = idx + M;
bool legalCurRow = (base_global_row + t) < M;

if(legalCurRow) {
    shared_cols[idx] = dst[dst_idx_base]; // Directly using base index
}
if(legalNextCol && legalCurRow) {
    shared_cols[idx_nextCol] = dst[dst_idx_base + 1]; // Base index +1 for next column
}
__syncthreads();
