...
// Calculate base global index for current slice, row, and column
...
// Determine validity of indices within bounds
...
for (int ty = 0; ty < tile_y; ++ty) {
    ...
    // If the current column and row are within bounds, copy slice data
