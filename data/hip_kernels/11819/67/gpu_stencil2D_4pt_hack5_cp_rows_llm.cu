...
__global__ void gpu_stencil2D_4pt_hack5_cp_rows(double * dst, double * shared_cols, double *shared_rows, int tile_y, int M, int N){
    ...
    // Copy current row to shared memory if legal column 
    if(legalCurCol){
        shared_rows[idx] = dst[base_global_idx + t];
    }

    // Copy next row to shared memory if legal next row and column 
    ...
    // Print debug information for specific blocks and threads
