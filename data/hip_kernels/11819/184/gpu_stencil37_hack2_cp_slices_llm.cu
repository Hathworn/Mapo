...
// Precompute indices to avoid recalculating within the loop
int s_area_double = s_area * 2;
int tile_offset = tx;

for(int ty=0;ty<tile_y;++ty){
bool legalCurRow = (base_global_row + ty)<n_rows;
int s_idx = blockIdx.z*s_area_double + (base_global_row+ty)*s_xpitch + base_global_col + tile_offset;
int d_idx = base_global_idx + ty*d_xpitch + tile_offset;
