...
__global__ void gpu_stencil2D_4pt_hack5_cp_cols(double * dst, double * shared_cols, double *shared_rows, int tile_x, int tile_y, int M, int N) {
    ...
    // Use predicate and avoid branch divergence
    double value1 = legalCurRow ? dst[base_global_idx + t * N] : 0.0;
    double value2 = (legalNextCol && legalCurRow) ? dst[base_global_idx + t * N + 1] : 0.0;

    if (legalCurRow) {
        shared_cols[idx] = value1;
    }
    if (legalNextCol && legalCurRow) {
        shared_cols[idx_nextCol] = value2;
    }
