...
__global__ void gpu_stencil2D_4pt(double *dst, double *src, int M, int N) {
    ...
    if (threadIdx.x == 0 && threadIdx.y == 0) {
        ...
        for (j = 0; j < HALO; j++) {
            for (i = 0; i < smRowDim - HALO * 2; i++) {
                indexLeftHalo = (HALO + blockIdx.x * blockDim.x * TILE_SIZE + i) * N + (blockIdx.y * blockDim.y * TILE_SIZE) + j;
                indexRightHalo = (HALO + blockIdx.x * blockDim.x * TILE_SIZE + i) * N + ((blockIdx.y + 1) * blockDim.y * TILE_SIZE) + HALO + j;
                shSrc[(HALO + i) * smColDim + j] = src[indexLeftHalo];
                shSrc[(HALO + i + 1) * smColDim - HALO + j] = src[indexRightHalo];
            }
        }
    }
