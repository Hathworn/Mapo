...
int base_global_slice = tile_z * blockIdx.z; // Base global slice calculation
int base_global_row = tile_y  * blockIdx.y;  // Base global row calculation
int base_global_col = blockDim.x * blockIdx.x;

int dst_area = d_ypitch * d_xpitch;          // Destination area calculation
int s_area = gridDim.y * s_xpitch * 2;

int base_global_idx = base_global_slice * dst_area + base_global_row * d_xpitch + base_global_col;

int nextRow = base_global_row + 1;           // Calculate next row
...
for(int tz = 0; tz < tile_z; ++tz) {
    bool legalCurSlice = (base_global_slice + tz) < n_slices;  // Check legal slice
    ...
    if (legalCurCol && legalCurSlice) {
        shared_rows[idx] = dst[idx_dst];    // Copy current row
    }
    if (legalCurCol && legalCurSlice && legalNextRow) {
        shared_rows[idx + s_xpitch] = dst[idx_dst + d_xpitch];  // Copy next row if legal
    }
}
...
if(blockIdx.y == 0 && blockIdx.x == 0 && blockIdx.z == 0) {
    if ((threadIdx.x == 0 || threadIdx.x == 1 || threadIdx.x == 2) && threadIdx.y == 0) {
        ...
        // Calculate multiple shared address positions for debugging
        int s_addr00 = base_global_slice + blockIdx.x * blockDim.x + threadIdx.x;
        int s_addr01 = s_addr00 + s_xpitch;
        int s_addr02 = s_addr00 + s_xpitch * 2;
        int s_addr10 = s_area * (base_global_slice + 1) + blockIdx.x * blockDim.x + threadIdx.x;
        int s_addr11 = s_addr10 + s_xpitch;
        int s_addr12 = s_addr11 + s_xpitch;
        int s_addr20 = s_area * (base_global_slice + 2) + blockIdx.x * blockDim.x + threadIdx.x;
        int s_addr21 = s_addr20 + s_xpitch;
        int s_addr22 = s_addr21 + s_xpitch;
