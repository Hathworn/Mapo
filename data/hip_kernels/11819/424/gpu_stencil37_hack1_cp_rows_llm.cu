...
// Precompute index offsets to improve performance
...
// Cache frequently used values in register
int tx = threadIdx.x;
bool legalCurCol = (base_global_col + tx) < n_cols;

// Perform memory operations in loops to minimize redundant calculations
for (int tz = 0; tz < tile_z; ++tz) {
    ...
    // Check boundary conditions before global memory accesses
    if (legalCurCol && legalCurSlice) {
        shared_rows[idx] = dst[idx_dst];
    }
    if (legalCurCol && legalCurSlice && (base_global_row + 1) < n_rows) {
