...
bool legalNextRow = nextRow < M && legalCurCol;
int idx = (base_global_row / tile_y) * 2 * N + t + base_global_col;
int idx_nextrow = idx + N;

if (legalCurCol) {
    shared_rows[idx] = dst[base_global_idx + t];              // Copy current row
}
if (legalNextRow) {
    shared_rows[idx_nextrow] = dst[base_global_idx + N + t];  // Copy next row if legal
}
__syncthreads();
