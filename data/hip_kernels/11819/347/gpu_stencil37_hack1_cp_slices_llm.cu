...
// Calculate base indices for global memory reads
...
// Precompute conditions to reduce redundant calculations
int tx = threadIdx.x;
bool legalNextSlice = (base_global_slice + 1) < n_slices;
bool legalCurCol = (base_global_col + tx) < n_cols;

// Flatten nested loops by calculating within bounds directly
...
if(threadIdx.x == 0 || threadIdx.x == 1 || threadIdx.x == 2){
    ...
    int addr1 = addr + n_cols;
    int addr2 = addr + n_cols * 2;
