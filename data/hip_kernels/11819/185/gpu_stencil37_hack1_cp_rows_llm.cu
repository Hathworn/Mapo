...
__global__ void gpu_stencil37_hack1_cp_rows(double * dst, double * shared_rows, double *shared_cols, double *shared_slices, int n_rows, int n_cols, int n_slices, int tile_x, int tile_y, int tile_z){
    ...
    // Calculate base indices and thread indices
    const int base_global_slice = tile_z * blockIdx.z;
    const int base_global_row   = tile_y * blockIdx.y;
    const int base_global_col   = blockDim.x * blockIdx.x;
    const int dst_area = n_rows * n_cols;
    const int s_area = gridDim.y * n_cols * 2;
    const int base_global_idx = base_global_slice * dst_area + base_global_row * n_cols + base_global_col;
    const int tx = threadIdx.x;

    // Check legal row, column
    const int nextRow = base_global_row + 1;
    const bool legalNextRow = nextRow < n_rows;
    const bool legalCurCol = (base_global_col + tx) < n_cols;

    // Loop unrolling optimization
    for(int tz = 0; tz < tile_z; ++tz){
        const bool legalCurSlice = (base_global_slice + tz) < n_slices;
        const int idx_dst = base_global_idx + tz * dst_area + tx;
        const int idx = (base_global_slice + tz) * s_area + blockIdx.y * n_cols * 2 + blockIdx.x * blockDim.x + tx;
