...
int tz_stride = area_dst; // Cache stride for z-loop
int idx_stride = 2 * n_rows; // Cache stride for shared_cols

for(int tz = 0; tz < tile_z; ++tz){
    int tz_offset_dst = tz * tz_stride;
    int tz_offset_shared = (base_global_slice + tz) * area_shared;
    
    bool legalCurSlice = base_global_slice + tz < n_slices;
    int idx_dst = base_global_idx + tz_offset_dst + ty * n_cols;
    int idx = tz_offset_shared + blockIdx.x * idx_stride + blockIdx.y * blockDim.y + ty;

    if(legalCurRow && legalCurSlice) {
        shared_cols[idx] = dst[idx_dst]; // Copy current col
        if(legalNextCol){
            shared_cols[idx + n_rows] = dst[idx_dst + 1]; // Copy next col if valid
        }
    }
