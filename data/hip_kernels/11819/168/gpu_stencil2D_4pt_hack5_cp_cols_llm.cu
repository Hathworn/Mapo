...
double local_dst0 = 0.0, local_dst1 = 0.0; // Use local variables to store values

if (legalCurRow) {
    local_dst0 = dst[base_global_idx + t * N]; // Cache first value in register
    shared_cols[idx] = local_dst0; // Write cached value to shared memory
    if (legalNextCol) {
        local_dst1 = dst[base_global_idx + t * N + 1]; // Cache second value if needed
        shared_cols[idx_nextCol] = local_dst1; // Write cached value to shared memory
    }
}
