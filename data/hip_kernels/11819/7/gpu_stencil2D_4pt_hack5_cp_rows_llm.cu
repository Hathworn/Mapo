...
__global__ void gpu_stencil2D_4pt_hack5_cp_rows(double *dst, double *shared_cols, double *shared_rows, int tile_y, int M, int N) {
    ...
    int shared_row_idx = (base_global_row / tile_y) * 2 * N + base_global_col + t;

    // Use coalesced access to shared memory
    if (legalCurCol) shared_rows[shared_row_idx] = dst[base_global_idx + t];
    if (legalNextRow && legalCurCol) shared_rows[shared_row_idx + N] = dst[base_global_idx + N + t];

    __syncthreads();

#ifdef CUDA_CUDA_DEBUG
    if (blockIdx.y == 0 && blockIdx.x == 2 && (t == 0 || t == 1)) {
        printf("addr:%d, val = %f\n", shared_row_idx + N, shared_rows[shared_row_idx + N]);
    }
#endif
