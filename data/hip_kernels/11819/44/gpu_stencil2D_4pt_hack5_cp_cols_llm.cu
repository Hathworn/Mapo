...
int row_offset = t * N;  // Calculate row offset once

if(legalCurRow){
    shared_cols[idx] = dst[base_global_idx + row_offset];  // Use row_offset
}
if(legalNextCol && legalCurRow){
    shared_cols[idx_nextCol] = dst[base_global_idx + row_offset + 1];  // Use calculated row_offset
}
__syncthreads();
