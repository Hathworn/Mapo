...
#endif
// Pre-calculate repeated computations for performance
...
for(int tz = 0; tz < tile_z; ++tz){
    ...
    if (legalCurCol && legalCurSlice) {
        shared_rows[idx] = dst[idx_dst]; // Copy row data
        if (legalNextRow) {
            shared_rows[idx+n_cols] = dst[idx_dst+n_cols]; // Copy next row data
        }
    }
}
