...
int t = threadIdx.y;
int global_col_offset = base_global_idx + t * N;
bool legalCurRow = (base_global_row + t) < M;

// Pre-compute common indices to optimize memory access
int idx = M * (2 * blockIdx.x + t) + base_global_row;
int idx_nextCol = idx + M;

if (legalCurRow) {
    shared_cols[idx] = dst[global_col_offset];
}
if ((base_global_col + 1 < N) && legalCurRow) {
    shared_cols[idx_nextCol] = dst[global_col_offset + 1];
}
