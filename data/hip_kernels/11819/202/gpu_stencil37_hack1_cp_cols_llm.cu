...
__global__ void gpu_stencil37_hack1_cp_cols(double* dst, double* shared_rows, double* shared_cols, double* shared_slices, int n_rows, int n_cols, int n_slices, int tile_x, int tile_y, int tile_z) {
    ...
    for (int tz = 0; tz < tile_z; ++tz) {
        ...
        int idx = (base_global_slice + tz) * gridDim.x * n_rows * 2 + blockIdx.x * n_rows * 2 + blockIdx.y * blockDim.y + ty;

        // Optimize memory access by coalescing and reducing unnecessary calculations
        ...
        __syncthreads(); // Ensure previous memory operations are completed before next iteration
    }
