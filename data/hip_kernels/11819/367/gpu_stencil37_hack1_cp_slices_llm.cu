...
__global__ void gpu_stencil37_hack1_cp_slices(double *dst, double *shared_rows, double *shared_cols, double *shared_slices, int n_rows, int n_cols, int n_slices, int tile_x, int tile_y, int tile_z) {
    ...
    // Pre-calculate constant offsets for loop optimization
    int idx_base = blockIdx.z * area * 2 + base_global_row * n_cols + base_global_col;
    int dst_base = base_global_idx;

    for (int ty = 0; ty < tile_y; ++ty) {
        bool legalCurRow = (base_global_row + ty) < n_rows;
        int idx = idx_base + ty * n_cols + tx;
        int dst_idx = dst_base + ty * n_cols + tx;

        // Use a single conditional block to reduce branching overhead
        if (legalCurCol && legalCurRow) {
            shared_slices[idx] = dst[dst_idx];
            if (legalNextSlice) {
                shared_slices[idx + area] = dst[dst_idx + area];
            }
        }
    }
