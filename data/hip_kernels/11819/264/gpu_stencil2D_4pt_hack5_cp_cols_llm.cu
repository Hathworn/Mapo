...
__global__ void gpu_stencil2D_4pt_hack5_cp_cols(double * dst, double * shared_cols, double *shared_rows, int tile_x, int tile_y, int M, int N) {
    ...
    // Use a single if-statement and combine conditions
    if (legalCurRow) {
        int global_idx_t = base_global_idx + t * N; // Pre-compute global index
        shared_cols[idx] = dst[global_idx_t];
        if (base_global_col + 1 < N) { // Inline 'legalNextCol' check
            shared_cols[idx + M] = dst[global_idx_t + 1];
        }
    }
    __syncthreads();
