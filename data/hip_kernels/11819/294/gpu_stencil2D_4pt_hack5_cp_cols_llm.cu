...
__global__ void gpu_stencil2D_4pt_hack5_cp_cols(double * dst, double * shared_cols, double *shared_rows, int tile_x, int tile_y, int M, int N) {
    ...
    // Calculate the current and next indices for shared memory
    int idx = 2 * M * blockIdx.x + t + base_global_row;
    int idx_nextCol = idx + M;
    
    // Check bounds once to optimize memory access
    bool legalCurRow = (base_global_row + t) < M;

    if (legalCurRow) {
        shared_cols[idx] = dst[base_global_idx + t * N]; // Load current column data
    }
    
    if (legalCurRow && legalNextCol) {
        shared_cols[idx_nextCol] = dst[base_global_idx + t * N + 1]; // Load next column data if within bounds
    }
