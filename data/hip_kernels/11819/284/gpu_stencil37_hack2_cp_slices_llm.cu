...
// Shared memory utilization to hold a slice copy temporarily
extern __shared__ double shmem[];

int tx = threadIdx.x;
bool legalCurCol = (base_global_col + tx)<n_cols;

#pragma unroll // Use loop unrolling for performance
for(int ty=0; ty<tile_y; ++ty){
    ...
    if(legalCurCol&&legalCurRow){
        shmem[ty] = dst[d_idx];
        shared_slices[s_idx] = shmem[ty];
    }
    if(legalNextSlice&&legalCurCol&&legalCurRow){
        shmem[ty+tile_y] = dst[d_idx+d_area];
        shared_slices[s_idx+s_area] = shmem[ty+tile_y];
    }
}
