...
// Use nested structure to minimize calculations within the loop
for (int ty = 0; ty < tile_y; ++ty) {
    int global_row = base_global_row + ty;
    bool legalCurRow = global_row < n_rows;
    int s_idx = blockIdx.z*s_area*2 + global_row*s_xpitch + base_global_col + tx;
