...
extern __shared__ double s_cols[];
...
s_cols[idx] = dst[idx_dst]; // Use shared memory for faster access
}
if(legalCurRow && legalCurSlice && legalNextCol){
s_cols[idx + s_ypitch] = dst[idx_dst + 1]; // Use shared memory for faster access
}
...
printf("copy cols: blockIdx.x=%d, blockIdx.y=%d,blockIdx.z=%d,shared_cols: z:%d, addr:%d, val = %f\n",blockIdx.x, blockIdx.y,blockIdx.z,0,addr,s_cols[addr]);
printf("copy cols: blockIdx.x=%d, blockIdx.y=%d,blockIdx.z=%d,shared_cols: z:%d, addr:%d, val = %f\n",blockIdx.x, blockIdx.y,blockIdx.z,1,addr1,s_cols[addr1]);
printf("copy cols: blockIdx.x=%d, blockIdx.y=%d,blockIdx.z=%d,shared_cols: z:%d, addr:%d, val = %f\n",blockIdx.x, blockIdx.y,blockIdx.z,2,addr2,s_cols[addr2]);
}
}
