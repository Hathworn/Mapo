...
__global__ void gpu_stencil2D_4pt_hack5_cp_rows(double * dst, double * shared_cols, double *shared_rows, int tile_y, int M, int N) {
    ...
    // Calculate shared memory index and global memory index only once
    int idx = (base_global_row / tile_y) * 2 * N + base_global_col + t;
    int dst_idx = base_global_idx + t;
    
    if (legalCurCol) {
        shared_rows[idx] = dst[dst_idx];
    }

    // Avoid redundant memory access checks and calculations
    if (legalNextRow && legalCurCol) {
        shared_rows[idx + N] = dst[dst_idx + N];
    }
