...
// Precompute values and perform boundary checks once
bool legalNextRow = nextRow < n_rows;
bool legalCurCol = (base_global_col + tx) < n_cols;

// Loop over tile slices
#pragma unroll
for(int tz = 0; tz < tile_z; ++tz) {
    int sliceOffset = base_global_slice + tz;
    bool legalCurSlice = sliceOffset < n_slices;
    int idx_dst = base_global_idx + tz * dst_area + tx;
    int idx = sliceOffset * s_area + blockIdx.y * n_cols * 2 + blockIdx.x * blockDim.x + tx;

    // Copy data only if legal indices
