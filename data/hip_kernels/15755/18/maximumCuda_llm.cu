...
__global__ void maximumCuda(const uint8_t *in1, uint32_t rowSizeIn1, const uint8_t *in2, uint32_t rowSizeIn2, uint8_t *out, uint32_t rowSizeOut, uint32_t width, uint32_t height) {
    ...
    if (x < width && y < height) {
        ...
        // Load inputs and compute maximum
        uint8_t val1 = in1[offsetIn1];
        uint8_t val2 = in2[offsetIn2];
        out[offsetOut] = max(val1, val2); // Use intrinsic function for clarity and potential optimization
    }
}