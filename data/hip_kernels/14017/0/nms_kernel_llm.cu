...
__global__ void nms_kernel(const int n_boxes, const float nms_overlap_thres, const float *dev_boxes, unsigned long long *dev_mask) {
    ...
    // Load block boxes into shared memory in a coalesced manner
    for (int k = threadIdx.x; k < col_size * 5; k += blockDim.x) {
        block_boxes[k] = dev_boxes[(threadsPerBlock * col_start) * 5 + k];
    }
    __syncthreads();
    ...
    if (threadIdx.x < row_size) {
        ...
        // Evaluate IoU for relevant boxes
