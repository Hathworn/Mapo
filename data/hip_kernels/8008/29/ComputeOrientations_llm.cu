...
__global__ void ComputeOrientations(float *g_Data, int *d_Ptrs, float *d_Orient, int maxPts, int w, int h)
{
    ...
    // Compute gradient and update histogram
    for (int y = 1; y < 14; y++) {
        int memPos = 16 * y + x;
        if (x >= 1 && x < 14) {
            float dy = data[memPos + 16] - data[memPos - 16];
            float dx = data[memPos + 1] - data[memPos - 1];
            int bin = __float2int_rz(16.0f * atan2f(dy, dx) / 3.1416f + 16.5f);
            bin = (bin == 32) ? 0 : bin;
            float grad = hypotf(dx, dy);  // Efficient gradient calculation
    ...
    if (tx >= 0 && tx < 32) {
        float left = hist[(tx - 1 + 32) & 31];
        float right = hist[(tx + 1) & 31];
        hist[32 + tx] = 6.0f * hist[tx] + 4.0f * (right + left) + hist[(tx + 2) & 31] + hist[(tx - 2 + 32) & 31];
    }
    __syncthreads();
