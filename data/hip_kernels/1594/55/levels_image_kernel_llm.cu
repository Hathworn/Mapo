...
__device__ float3 hsv_to_rgb_kernel(float3 hsv)
{
    ...
    if (s == 0) {
        ...
        // Use switch-case for clarity and potential performance gains
        switch (index) {
    ...
    // Clamp using fminf and fmaxf for potential performance benefits
...
__device__ float3 rgb_to_hsv_kernel(float3 rgb)
{
    ...
    if (max == 0) {
        ...
        // Use fmodf to ensure h is positive
        if (h < 0) h += 6;
    }
...
__global__ void levels_image_kernel(float *image, float *rand, int batch, int w, int h, int train, float saturation, float exposure, float translate, float scale, float shift)
{
    ...
    // Pre-fetch random values to reduce register pressure
    float rshift = rand[0];
    float gshift = rand[1];
    float bshift = rand[2];
