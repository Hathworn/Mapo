...
__global__ static void resizeBatchKernel(const uint16_t *p_Src, int nSrcPitch, int nSrcHeight, uint16_t *p_dst, int nDstWidth, int nDstHeight) {
    ...
    // Optimize by pre-calculating commonly used multiples; Efficient index access
    int nDstWHalf = nDstWidth / 2;
    int nDstHW = nDstHeight * nDstWidth;
    int nDstHWidth = nDstHeight * nSrcPitch * scale;
    
    if (scale == 4) {
        uint32_t v0, y0, u0, y2, u1, y1, u2, y3, v1, y5, v2, y4;

        if (tid < nDstWidth / 6 && tidd < nDstHeight) {
            int j = tidd * nDstHWidth;
            int k = tid * 32;
            ...
            p_dst[j + k + 5] = y5;
            k = tid * 3;
            j = tidd * nDstWHalf + nDstHW;
            p_dst[j + k + 0] = u0;
            p_dst[j + k + 1] = u1;
            p_dst[j + k + 2] = u2;
            j = tidd * nDstWHalf + nDstHW * 3 / 2;
            p_dst[j + k + 0] = v0;
            p_dst[j + k + 1] = v1;
            p_dst[j + k + 2] = v2;
        }
        ...
        if (tid < nDstWidth / 2 && tidd < nDstHeight) {
            int j = tidd * nDstHWidth;
            int k = tid * 16;
            pF.x = (uint32_t)p_Src[j + k + 0] + ((uint32_t)p_Src[j + k + 1] << 16);
