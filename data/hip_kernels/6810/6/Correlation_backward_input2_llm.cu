...
{
    // Calculate common indices once to avoid recomputation
    int n = item;
    ...
    for (int tc = tch_off; tc < nOutputChannels; tc += CUDA_NUM_THREADS) {
        ...
        if (xmax < 0 || ymax < 0 || xmin >= outputWidth || ymin >= outputHeight) continue;
        ...
        // Combine nested loops into a single loop over index space
        int area = (xmax - xmin + 1) * (ymax - ymin + 1);
        for (int idx = 0; idx < area; ++idx) {
            int i = idx % (xmax - xmin + 1) + xmin;
            int j = idx / (xmax - xmin + 1) + ymin;
