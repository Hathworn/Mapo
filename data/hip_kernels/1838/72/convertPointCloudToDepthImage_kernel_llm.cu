...
__global__ void convertPointCloudToDepthImage_kernel(unsigned int *depth_image, const float4 *point_cloud, int n_cols, int n_rows, int n_points, float nodal_point_x, float nodal_point_y, float focal_length_x, float focal_length_y, const float *T, const float *R) {
    ...
    if (ind < n_points) {
        // Fetch point and transform to camera frame simultaneously
        ...
        float inv_z = __frcp_rn(z); // Use reciprocal instruction
        
        // Project in image with fused multiply-add
        int x_pix = __float2int_rn(fmaf(focal_length_x, x * inv_z, nodal_point_x));
        int y_pix = __float2int_rn(fmaf(focal_length_y, y * inv_z, nodal_point_y));

        // Check if inside image with single condition
