...
__global__ void normal_eqs_disparity_weighted_GPU(float *d_CD, const float *d_disparity_compact, const float4 *d_Zbuffer_normals_compact, const int *d_ind_disparity_Zbuffer, float fx, float fy, float ox, float oy, float b, int n_cols, const int *d_n_values_disparity, const int *d_start_ind_disparity, const float *d_abs_res_scales, float w_disp, const float *d_dTR) {
    ...
    int n_val_accum = blockDim.x * gridDim.x;
    int n_disparity = d_n_values_disparity[blockIdx.y];
    int n_accum = (int)ceilf((float)n_disparity / n_val_accum);
    ...
    for (int in_ind = blockDim.x * (blockIdx.x * n_accum) + threadIdx.x;
        ...
        if (in_ind < n_disparity) { // is this a valid sample?
            ...
            float y = floorf(pixel_ind / n_cols);
            float x = pixel_ind - y * n_cols;
            ...
            A[1] += w * (nx * ny);
            A[2] += w * (nx * nz);
            A[3] += w * (Ym * nx * nz - Zm * nx * ny);
            A[4] += w * (Zm * (nx * nx) - Xm * nx * nz);
            A[5] += w * (-Ym * (nx * nx) + Xm * nx * ny);

            A[6] += w * (ny * ny);
            A[7] += w * (ny * nz);
            A[8] += w * (-Zm * (ny * ny) + Ym * ny * nz);
            A[9] += w * (-Xm * ny * nz + Zm * nx * ny);
            A[10] += w * (Xm * (ny * ny) - Ym * nx * ny);

            A[11] += w * (nz * nz);
            A[12] += w * (Ym * (nz * nz) - Zm * ny * nz);
            A[13] += w * (-Xm * (nz * nz) + Zm * nx * nz);
            A[14] += w * (Xm * ny * nz - Ym * nx * nz);

            A[15] += w * ((Ym * Ym) * (nz * nz) + (Zm * Zm) * (ny * ny) - Ym * Zm * ny * nz * 2.0f);
            A[16] += w * (-Xm * Ym * (nz * nz) - (Zm * Zm) * nx * ny + Xm * Zm * ny * nz + Ym * Zm * nx * nz);
            A[17] += w * (-Xm * Zm * (ny * ny) - (Ym * Ym) * nx * nz + Xm * Ym * ny * nz + Ym * Zm * nx * ny);

            A[18] += w * ((Xm * Xm) * (nz * nz) + (Zm * Zm) * (nx * nx) - Xm * Zm * nx * nz * 2.0f);
            A[19] += w * (-Ym * Zm * (nx * nx) - (Xm * Xm) * ny * nz + Xm * Ym * nx * nz + Xm * Zm * nx * ny);

            A[20] += w * ((Xm * Xm) * (ny * ny) + (Ym * Ym) * (nx * nx) - Xm * Ym * nx * ny * 2.0f);

            // B-vector
            A[21] += w * (Xd * (nx * nx) - Xm * (nx * nx) + Yd * nx * ny - Ym * nx * ny + Zd * nx * nz - Zm * nx * nz);
            A[22] += w * (Yd * (ny * ny) - Ym * (ny * ny) + Xd * nx * ny - Xm * nx * ny + Zd * ny * nz - Zm * ny * nz);
            A[23] += w * (Zd * (nz * nz) - Zm * (nz * nz) + Xd * nx * nz - Xm * nx * nz + Yd * ny * nz - Ym * ny * nz);
            A[24] += w * (-Yd * Zm * (ny * ny) + Ym * Zd * (nz * nz) + Ym * Zm * (ny * ny) - Ym * Zm * (nz * nz) - (Ym * Ym) * ny * nz + (Zm * Zm) * ny * nz +
                     Xd * Ym * nx * nz - Xm * Ym * nx * nz - Xd * Zm * nx * ny + Yd * Ym * ny * nz + Xm * Zm * nx * ny - Zd * Zm * ny * nz);
            A[25] += w * (Xd * Zm * (nx * nx) - Xm * Zd * (nz * nz) - Xm * Zm * (nx * nx) + Xm * Zm * (nz * nz) + (Xm * Xm) * nx * nz - (Zm * Zm) * nx * nz -
                     Xd * Xm * nx * nz - Xm * Yd * ny * nz + Xm * Ym * ny * nz + Yd * Zm * nx * ny - Ym * Zm * nx * ny + Zd * Zm * nx * nz);
            A[26] += w * (-Xd * Ym * (nx * nx) + Xm * Yd * (ny * ny) + Xm * Ym * (nx * nx) - Xm * Ym * (ny * ny) - (Xm * Xm) * nx * ny + (Ym * Ym) * nx * ny +
    ...
    #pragma unroll
    for (int i = 0; i < 27; i++) {
        d_CD[out_ind + i * n_val_accum] = w_disp * A[i];
    }
}