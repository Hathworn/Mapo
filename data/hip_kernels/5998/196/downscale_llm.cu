...
__global__ void downscale(float* gradInput_data, const float* gradOutput_data, long no_elements, int scale_factor, int d1, int d2, int d3) {
    ...
    float aggregated_grad = 0.0f;
    
    // Use register to accumulate the result
    for (int i = 0; i < scale_factor; ++i) {
        for (int j = 0; j < scale_factor; ++j) {
            int ipidx = base_idx + i * d3 * scale_factor + j;
            aggregated_grad += gradOutput_data[ipidx];
        }
    }
    
    gradInput_data[ii] = aggregated_grad; // Store the accumulated result
}