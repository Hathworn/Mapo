...
__global__ void GPUHeatContactFunction(float *a, float *b, int *body) {
    // Calculate global thread ID for linearized 3D grid indexing
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int idy = blockIdx.y * blockDim.y + threadIdx.y;
    int idz = blockIdx.z * blockDim.z + threadIdx.z;
    int index = (idz * NY + idy) * NX + idx;

    // Check bounds to ensure index is within valid range
    if (idx < NX && idy < NY && idz < NZ) {
        // Read input temperatures
        float tempA = a[index];
        float tempB = b[index];

        // Perform temperature averaging
        float avgTemp = 0.5f * (tempA + tempB);

        // Write result back, using atomics if needed for safe multi-threading
        a[index] = avgTemp;
        b[index] = avgTemp;
    }
}