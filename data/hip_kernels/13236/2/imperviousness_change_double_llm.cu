...
/*	+++++DEFINEs+++++	*/
__global__ void imperviousness_change_double(const unsigned char *dev_BIN1, const unsigned char *dev_BIN2, unsigned int WIDTH, unsigned int HEIGHT, double *dev_LTAKE_map)
{
    ...
    // Check bounds
    if (tix < WIDTH * HEIGHT) {
        // Simplified computation by eliminating redundant type casting
        dev_LTAKE_map[tix] = static_cast<double>(dev_BIN2[tix]) - static_cast<double>(dev_BIN1[tix]);
    }
}