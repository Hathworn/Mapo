...
__global__ void downscale(float *gradInput_data, const float *gradOutput_data, long no_elements, int scale_factor, int d1, int d2, int d3) {
    ...
    // Pre-calculate scaled dimensions variables for loop
    int scaled_d3 = d3 * scale_factor;
    int scaled_d2 = d2 * scale_factor;

    // Accumulate contributions from each translation offset
    for (int i = 0; i < scale_factor; ++i) {
        for (int j = 0; j < scale_factor; ++j) {
            int ipidx = translate_idx_inv(ii, d1, scaled_d2, scaled_d3, scale_factor, i, j);
