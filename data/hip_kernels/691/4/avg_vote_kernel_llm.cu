...
__global__ void avg_vote_kernel(float *A, float *B, int *corrAB, int patch, int c, int h, int w) {
    ...
    // Optimize by immediately returning if the thread index exceeds bounds
    ...
    for (int dx = -radius; dx <= radius; dx++) {
        for (int dy = -radius; dy <= radius; dy++) {
            ...
            // Combine the boundary check conditions for new_id1
            if (new_x1 >= 0 && new_x1 < w && new_y1 >= 0 && new_y1 < h) {
                int new_id1 = new_y1 * w + new_x1;
                int x2 = corrAB[new_id1 * 2];
                int y2 = corrAB[new_id1 * 2 + 1];
                int new_x2 = x2 - dx, new_y2 = y2 - dy;

                // Combine the boundary check conditions for new_id2
    ...
    // Use a conditional operator to set B
    B[dc * size + id] = (cnt != 0) ? sum / cnt : 0;
}