...
__global__ void im2col_gpu_kernel(const int n, const float* data_im, const int height, const int width, const int ksize, const int pad, const int stride, const int height_col, const int width_col, float *data_col) {
    ...
    // Use shared memory to optimize multiple accesses
    extern __shared__ float shared_data_im[];
    ...
    for (; index < n; index += blockDim.x * gridDim.x) {
        ...
        if (threadIdx.x < ksize * ksize) {
            int i = threadIdx.x / ksize;
            int j = threadIdx.x % ksize;
            int h = h_in + i;
            int w = w_in + j;
            shared_data_im[threadIdx.x] = (h >= 0 && w >= 0 && h < height && w < width) ? data_im_ptr[i * width + j] : 0;
        }
        __syncthreads();

        for (int i = 0; i < ksize; ++i) {
            for (int j = 0; j < ksize; ++j) {
                *data_col_ptr = shared_data_im[i * ksize + j];
