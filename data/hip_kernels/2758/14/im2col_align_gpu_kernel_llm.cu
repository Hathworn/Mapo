...
__global__ void im2col_align_gpu_kernel(const int n, const float* data_im, const int height, const int width, const int ksize, const int pad, const int stride, const int height_col, const int width_col, float *data_col, const int bit_align)
{
    ...
    // Use shared memory for data_col writes to coalesce global memory access
    ...
    for (; index < n; index += total_threads) {
        ...
        for (int i = 0; i < ksize; ++i) {
            for (int j = 0; j < ksize; ++j) {
                ...
                // Check if accessing shared memory within bounds
                if (threadIdx.x < blockDim.x) {
                   shared_data[threadIdx.x] = val;
                   __syncthreads();
                   data_col[out_index] = shared_data[threadIdx.x];
                   __syncthreads();
                }
