...
__global__ void convolve_bin_gpu_kernel(float *input, float *weights, float *output, int in_w, int in_h, int in_c, int n, int size, int pad, int new_lda, float *mean_arr_gpu)
{
    ...
    if (fil < n) {
        ...
        // Use __shared__ memory for faster access to weights
        extern __shared__ uint32_t shared_mem[];
        uint32_t* weights_shared = shared_mem;
        uint32_t* input_shared = weights_shared + (3 * 3 * 1024 * 6 / 32 + 1); // Offset for input_shared
        ...
        for (int chan = 0; chan < in_c; ++chan) {
            ...
            int const add_input_index = input_pre_index % 32;
