...
__global__ void convolve_bin_gpu_kernel(float *input, float *weights, float *output, int in_w, int in_h, int in_c, int n, int size, int pad, int new_lda, float *mean_arr_gpu) {
    ...
    if (fil < n)  // Process only valid filter indices
    {
        ...
        for (int chan = 0; chan < in_c; ++chan) {
            ...
            // Loop unoptimization by removing redundancy in condition checks
            for (int f_y = 0; f_y < size; ++f_y)
            {
                int input_y = y + f_y - pad;
                if (input_y < 0 || input_y >= in_h) continue;  // Combine loop to avoid unnecessary checks

                for (int f_x = 0; f_x < size; ++f_x)
                {
                    int input_x = x + f_x - pad;
                    if (input_x < 0 || input_x >= in_w) continue;  // Combine loop to avoid unnecessary checks
