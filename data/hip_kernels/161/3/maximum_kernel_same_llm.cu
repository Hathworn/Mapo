...
__global__ void maximum_kernel_same(const float * vg_a, size_t pitch_a, size_t n_a, const float * vg_b, size_t pitch_b, size_t n_b, size_t k, float * d, size_t pitch_d, float p)
{
    ...
    if(y < n_a && x < y) {
        ...
        float threadMax = 0.0f;  // Use a register for the max value per thread
        for(size_t offset = threadIdx.x; offset < k; offset += blockDim.x) {
            float t = abs(vg_a[x * pitch_a + offset] - vg_a[y * pitch_a + offset]);
            threadMax = max(t, threadMax);  // Reduce max computation in registers
        }
        temp[threadIdx.x] = threadMax; 
        ...
        if(threadIdx.x == 0) {
            float s = temp[0];
            d[y * pitch_d + x] = s;
            d[x * pitch_d + y] = s;  // Fill both symmetric positions
        }
    }
}