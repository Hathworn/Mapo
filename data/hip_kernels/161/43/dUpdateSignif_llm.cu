...
__global__ void dUpdateSignif(const float * gpuData, size_t n, float * gpuResults) {
    ...
    for (size_t i = 0; i < THREADWORK && (start + i) < n; i++) { // Loop unroll condition
        size_t inrow = (start + i) * 5;
        size_t outrow = (start + i) * 6;

        float cor = gpuData[inrow + 3];
        float npairs = gpuData[inrow + 4];
        float tscore = 0.0;

        if (cor >= 0.999) tscore = 10000.0;
        else {
            float radicand = (npairs - 2.f) / (1.f - cor * cor); // Optimize division
