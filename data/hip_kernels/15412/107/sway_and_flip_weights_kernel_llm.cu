...
__global__ void sway_and_flip_weights_kernel(const float *src_weight_gpu, float *weight_deform_gpu, int nweights, int n, int kernel_size, int angle, int reverse) {
    ...
    if (i < nweights) {
        ...
        // Pre-calculate shared constants for better performance
        const float angle_radians = angle * 3.14159265f / 180;
        float cos_a = cosf(angle_radians);
        float sin_a = sinf(angle_radians);

        if (reverse) {
            cos_a = cosf(-angle_radians);
            sin_a = sinf(-angle_radians);
        }
        ...
        if (stage_id == 0) {
            ...
            const float coef = (kernel_area) / (kernel_area - dropout_sum);
            for (int y = 0; y < kernel_size; ++y) {
                for (int x = 0; x < kernel_size; ++x) {
