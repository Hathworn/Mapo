...
__global__ void activate_array_hard_mish_kernel(float *x, int n, float *activation_input, float *output_gpu)
{
    ...
    int stride = blockDim.x * gridDim.x; // Calculate stride for multiple block processing
    for (int index = i; index < n; index += stride) {
        // Iterate over entire array with stride
        float x_val = x[index];
        if (activation_input) 
            activation_input[index] = x_val; // store value before activation
        output_gpu[index] = hard_mish_yashas(x_val);
    }
}