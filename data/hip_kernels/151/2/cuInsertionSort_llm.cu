...
__global__ void cuInsertionSort(float *dist, int width, int pitch, int height, int k) {
  ...
  if (xIndex < width) {
    ...
    for (l = pitch; l < k * pitch; l += pitch) {
      ...
      if (v < max_value) {
        i = 0;
        while (i < l && *(p + i) <= v) i += pitch;
        
        // Use memmove for efficient memory manipulation
        memmove(p + i + pitch, p + i, (l - i) * sizeof(float));
        *(p + i) = v;
    ...
    for (l = k * pitch; l < height * pitch; l += pitch) {
      ...
      if (v < max_value) {
        i = 0;
        while (i < k * pitch && *(p + i) <= v) i += pitch;

        // Use memmove for efficient memory manipulation
        memmove(p + i + pitch, p + i, ((k - 1) * pitch - i) * sizeof(float));
        *(p + i) = v;
