...
__global__ void query_ball_point_gpu(int b, int n, int m, float radius, int nsample, const float *xyz1, const float *xyz2, int *idx, int *pts_cnt) {
    ...
    for (int j = index; j < m; j += stride) {
        ...
        for (int k = 0; k < n; ++k) {
            ...
            float dy = y2 - y1;
            float dz = z2 - z1;
            float dist_sq = dx * dx + dy * dy + dz * dz; // Calculate squared distance to avoid unnecessary sqrt
            
            if (dist_sq < radius * radius) { // Compare squared distances for better performance
