...
__global__ void LowPassRowMulti(float *d_Result, float *d_Data, int width, int pitch, int height) {
    ...
    // Optimize boundary handling by using min/max to replace conditionals
    int clampedLoadPos = max(0, min(loadPos, width - 1));
    data[tx] = d_Data[yptr + clampedLoadPos];

    __syncthreads();

    // Avoid redundant checks by using tx constraint
    if (xout < width && tx < CONVROW_W) {
        // Unroll the loop for performance improvement
        float result = (data[tx + 0] + data[tx + 8]) * kernel[0] +
