#include "hip/hip_runtime.h"
#include "includes.h"

__global__ void ComputeOrientations(float *g_Data, int *d_Ptrs, float *d_Orient, int maxPts, int w, int h)
{
    __shared__ float data[16*15];
    __shared__ float hist[32*13];
    __shared__ float gauss[16];
    const int tx = threadIdx.x;
    const int bx = blockIdx.x;
    if (tx < 13) {
        hist[tx*32] = 0.0f;
        hist[tx*32 + 1] = 0.0f;
        hist[tx*32 + 2] = 0.0f;
        hist[tx*32 + 3] = 0.0f;
        hist[tx*32 + 4] = 0.0f;
        hist[tx*32 + 5] = 0.0f;
        hist[tx*32 + 6] = 0.0f;
        hist[tx*32 + 7] = 0.0f;
        hist[tx*32 + 8] = 0.0f;
        hist[tx*32 + 9] = 0.0f;
        hist[tx*32 + 10] = 0.0f;
        hist[tx*32 + 11] = 0.0f;
        hist[tx*32 + 12] = 0.0f;
        hist[tx*32 + 13] = 0.0f;
        hist[tx*32 + 14] = 0.0f;
        hist[tx*32 + 15] = 0.0f;
        hist[tx*32 + 16] = 0.0f;
        hist[tx*32 + 17] = 0.0f;
        hist[tx*32 + 18] = 0.0f;
        hist[tx*32 + 19] = 0.0f;
        hist[tx*32 + 20] = 0.0f;
        hist[tx*32 + 21] = 0.0f;
        hist[tx*32 + 22] = 0.0f;
        hist[tx*32 + 23] = 0.0f;
        hist[tx*32 + 24] = 0.0f;
        hist[tx*32 + 25] = 0.0f;
        hist[tx*32 + 26] = 0.0f;
        hist[tx*32 + 27] = 0.0f;
        hist[tx*32 + 28] = 0.0f;
        hist[tx*32 + 29] = 0.0f;
        hist[tx*32 + 30] = 0.0f;
        hist[tx*32 + 31] = 0.0f;
    }
    __syncthreads();

    float i2sigma2 = -1.0f/(2.0f*3.0f*3.0f);
    if (tx < 15)
        gauss[tx] = exp(i2sigma2*(tx-7)*(tx-7));

    int p = d_Ptrs[bx];
    int yp = p/w - 7;
    int xp = p % w - 7;
    int px = xp & 15;
    int x = tx - px;

    for (int y = 0; y < 15; y++) {
        int memPos = 16*y + x;
        int xi = xp + x;
        int yi = yp + y;
        xi = max(0, min(w-1, xi));   // Clamped to valid range
        yi = max(0, min(h-1, yi));   // Clamped to valid range
        if (x >= 0 && x < 15)
            data[memPos] = g_Data[yi * w + xi];
    }
    __syncthreads();

    for (int y = 1; y < 14; y++) {
        int memPos = 16 * y + x;
        if (x >= 1 && x < 14) {
            float dy = data[memPos + 16] - data[memPos - 16];
            float dx = data[memPos + 1] - data[memPos - 1];
            int bin = int(16.0f * atan2f(dy, dx) / 3.1416f + 16.5f) & 31;
            float grad = sqrtf(dx * dx + dy * dy);
            atomicAdd(&hist[32 * (x - 1) + bin], grad * gauss[x] * gauss[y]); // Use atomic for concurrent operations
        }
    }
    __syncthreads();

    // Reduce histogram
    for (int offset = 8; offset > 0; offset >>= 1) {
        for (int y = 0; y < 32; y += 16) {
            if (tx < offset) {
                hist[tx + y] += hist[tx + y + offset];
            }
        }
        __syncthreads();
    }

    // Compute orientations
    int tid = tx & 31;
    if (tx < 32) {
        float v = hist[tid + 32];
        float v_next = hist[(tid + 1) & 31];
        float v_prev = hist[(tid + 31) & 31];
        hist[tid] = (v > v_next && v >= v_prev) ? v : 0.0f;
    }
    __syncthreads();

    // Find max orientations
    if (tx == 0) {
        float maxval1 = 0.0f;
        float maxval2 = 0.0f;
        int i1 = -1;
        int i2 = -1;

        for (int i = 0; i < 32; i++) {
            float v = hist[i];
            if (v > maxval1) {
                maxval2 = maxval1;
                maxval1 = v;
                i2 = i1;
                i1 = i;
            } else if (v > maxval2) {
                maxval2 = v;
                i2 = i;
            }
        }

        if (i1 >= 0) {
            float val1 = hist[32 + ((i1 + 1) & 31)];
            float val2 = hist[32 + ((i1 + 31) & 31)];
            float peak = i1 + 0.5f * (val1 - val2) / (2.0f * maxval1 - val1 - val2);
            d_Orient[bx] = 11.25f * (peak < 0.0f ? peak + 32.0f : peak);
        }

        if (maxval2 < 0.8f * maxval1)
            i2 = -1;

        if (i2 >= 0) {
            float val1 = hist[32 + ((i2 + 1) & 31)];
            float val2 = hist[32 + ((i2 + 31) & 31)];
            float peak = i2 + 0.5f * (val1 - val2) / (2.0f * maxval2 - val1 - val2);
            d_Orient[bx + maxPts] = 11.25f * (peak < 0.0f ? peak + 32.0f : peak);
        } else {
            d_Orient[bx + maxPts] = -1.0f;
        }
    }
}