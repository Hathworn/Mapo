...
__global__ void Find3DMinMax(int *d_Result, float *d_Data1, float *d_Data2, float *d_Data3, int width, int pitch, int height)
{
    ...
    for (int y = 0; y < 34; y++) { // Optimize loop iteration range
        ...
        if (y > 1 && tx < memWid) {
            // Optimize min/max calculation using fminf/fmaxf directly
        ...
        if (y > 1 && tx < MINMAX_W && xWritePos <= xEndClamped) {
            // Simplified min/max checks and threshold logic
            float minv = fminf(ymin2[tx], fminf(ymin2[tx + 2], fminf(ymin1[tx + 1], ymin3[tx + 1])));
            minv = fminf(minv, fminf(data2[memPos0 + 1], data2[memPos2 + 1]));
            minv = fminf(minv, d_Threshold[1]);

            float maxv = fmaxf(ymax2[tx], fmaxf(ymax2[tx + 2], fmaxf(ymax1[tx + 1], ymax3[tx + 1])));
