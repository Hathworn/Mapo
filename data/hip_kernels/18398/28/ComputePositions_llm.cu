...
{
    int i = blockIdx.x * POSBLK_SIZE + threadIdx.x; // Replace __mul24 with normal multiplication
    if (i >= numPts)
        return;
    ...
    if (pdx < -0.5f || pdx > 0.5f || pdy < -0.5f || pdy > 0.5f || pds < -0.5f || pds > 0.5f) {
        pdx = __fdividef(dx, dxx); // Use fast divide
        pdy = __fdividef(dy, dyy); // Use fast divide
        pds = __fdividef(ds, dss); // Use fast divide
    }
    ...
    float tra = dxx + dyy;
    det = dxx * dyy - dxy * dxy;
    d_Sift[i + 4 * maxPts] = __fdividef(tra * tra, det); // Use fast divide
}