...
__global__ void LowPassColMulti(float *d_Result, float *d_Data, int width, int pitch, int height)
{
    ...
    if (colStart < width) {
        float *kernel = d_Kernel + scale * 16; // Kernel adjustment for scales
        const int size = pitch * height * scale; // Scale specific memory adjustment
        d_Result += size; // Adjust result pointer based on scale
        d_Data += size; // Adjust data pointer based on scale
        float *sdata = data + ty * CONVCOL_W + tx;
        int gPos = colStart + (totStart + ty) * pitch;

        // Load to shared memory
        for (int y = totStart + ty; y <= miny + RADIUS; y += blockDim.y) {
        ...
        // Compute convolution
        if (ty + miny <= maxy) { // Check thread validity in compute
            sdata = data + ty * CONVCOL_W + tx;
            gPos = colStart + (miny + ty) * pitch;
            for (int y = miny + ty; y <= maxy; y += blockDim.y) {
                ...
                d_Result[gPos] = result; // Store result
                sdata += sStep;
                gPos += gStep;
            }
        }
