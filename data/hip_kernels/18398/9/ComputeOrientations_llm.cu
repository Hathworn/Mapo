...
__global__ void ComputeOrientations(float *g_Data, float *d_Sift, int maxPts, int w, int h)
{
    ...
    for (int stride = 8; stride > 0; stride >>= 1) {
        if (tx < 32 * stride)
            hist[tx] += hist[(stride * 32) + tx];
        __syncthreads();
    }
    
    // Parabolic interpolation and orientation assignment
    if (tx < 32) {
        hist[tx] = 6 * hist[tx] + 4 * (hist[(tx + 1) & 31] + hist[(tx + 31) & 31]) + 
    ...
    if (tx == 0) {
        ...
        if (maxval2 < 0.8f * maxval1)
            i2 = -1;
        if (i2 >= 0) {
            float val1 = hist[(i2 + 1) & 31];
            float val2 = hist[(i2 + 31) & 31];
