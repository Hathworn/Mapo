...
__global__ void ComputeOrientations(float *g_Data, int *d_Ptrs, float *d_Orient, int maxPts, int w, int h)
{
    ...
    for (int y=1; y<14; y++) {
        ...
        if (x >= 1 && x < 14) {
            ...
            bin = bin == 32 ? 0 : bin;  // Ensure bin wraps around
    ...
    // Reduce histogram to summarize orientations
    ...
    // Smooth histogram
    if (tx <= 31) {
        hist[32 + tx] = 6 * hist[tx] + 4 * (hist[(tx + 1) % 32] + hist[(tx + 31) % 32]) + 
                        (hist[(tx + 2) % 32] + hist[(tx + 30) % 32]);
    }

    __syncthreads();

    // Identify orientation peaks
    float v = hist[32 + tx];
