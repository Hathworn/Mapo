#include "hip/hip_runtime.h"
#include "includes.h"

__global__ void FindMinMax(float *d_MinMax, float *d_Data, int width, int pitch, int height)
{
    __shared__ float minvals[128];
    __shared__ float maxvals[128];
    
    const int tx = threadIdx.x;
    const int x = blockIdx.x * blockDim.x + tx;
    const int y = blockIdx.y * 16;
    int p = y * pitch + x;

    // Ensure x is within bounds, simplify conditional logic
    if (x < width) {
        float val = d_Data[p];
        minvals[tx] = val;
        maxvals[tx] = val;
        for (int ty = 1; ty < 16; ty++) {
            p += pitch;
            val = d_Data[p];
            // Direct comparisons without checking tx bounds
            if (val < minvals[tx]) minvals[tx] = val;
            if (val > maxvals[tx]) maxvals[tx] = val;
        }
    } else {
        minvals[tx] = maxvals[tx] = FLT_MAX; // Avoid writing out of bounds data
    }
    __syncthreads();

    // Optimize reduction logic, minimize modulo operations
    for (int d = blockDim.x / 2; d > 0; d >>= 1) {
        if (tx < d && (x + d) < width) {
            if (minvals[tx + d] < minvals[tx]) minvals[tx] = minvals[tx + d];
            if (maxvals[tx + d] > maxvals[tx]) maxvals[tx] = maxvals[tx + d];
        }
        __syncthreads();
    }

    // Write results from first thread in block
    if (tx == 0) {
        int ptr = 2 * (gridDim.x * blockIdx.y + blockIdx.x);
        d_MinMax[ptr] = minvals[0];
        d_MinMax[ptr + 1] = maxvals[0];
    }
}