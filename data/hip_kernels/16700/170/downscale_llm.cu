...
__global__ void downscale(float* __restrict__ gradInput_data, const float* __restrict__ gradOutput_data, long no_elements, int scale_factor, int d1, int d2, int d3) {
    ...
    // Reduce repeated calculations by precomputing variables
    const int d2_scaled = d2 * scale_factor;
    const int d3_scaled = d3 * scale_factor;
    
    // Optimize loop by reducing multiplicative operations
    for (int i = 0; i < scale_factor; i++) {
        for (int j = 0; j < scale_factor; j++) {
            int ipidx = (translate_idx_inv(ii, d1, d2_scaled, d3_scaled, scale_factor, i, j));
            atomicAdd(&gradInput_data[ii], gradOutput_data[ipidx]); // Use atomicAdd for safety
        }
    }
}