...
__global__ void kernel_calc_gjL_2(int layer_id, int *l, int *s_ext, int *sw_ext, float *z_ext_arr, float *a_ext_arr, float *t_arr, float *gjl_ext, float *w_ext_arr) {
    ...
    if (idx >= neuron_count - 1) return; // Return if index out of bounds early

    // Reduce register usage by removing unnecessary variables
    float sum = 0;
    int neuron_count_next = l[layer_id + 1];

    for (int k = lidx; k < neuron_count_next - 1; k += h) {
        sum += w_ext_arr[sw_ext[layer_id] + idx * (neuron_count_next - 1) + k] * gjl_ext[s_ext[layer_id + 1] + k];
    }
    ...
    if (lidx == 0) {
        ...
        // Optimize reduction of shared memory values
        sum = 0;
        for (int i = 0; i < h; i++) {
            sum += sm_g[pidx * h + i];
        }
