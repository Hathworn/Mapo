#include "hip/hip_runtime.h"
#include "includes.h"
__global__ void cuComputeNormGlobal(float* A, int wA, int pA, float* B, int wB, int pB, int dim, float* AB, float norm, bool inf) {

    // Shared memory for sub-matrix of A and B
    __shared__ float shared_A[BLOCK_DIM][BLOCK_DIM];
    __shared__ float shared_B[BLOCK_DIM][BLOCK_DIM];

    // Thread index
    int tx = threadIdx.x;
    int ty = threadIdx.y;

    // Initialize SSD variable based on conditions
    float ssd = (inf && norm < 0) ? 100.0f : 0.0f;

    // Loop parameters
    int begin_A = BLOCK_DIM * blockIdx.y;
    int begin_B = BLOCK_DIM * blockIdx.x;
    int step_A = BLOCK_DIM * pA;
    int step_B = BLOCK_DIM * pB;
    int end_A = begin_A + (dim - 1) * pA;

    // Conditions
    bool cond0 = (begin_A + tx < wA); // Shared memory write condition
    bool cond1 = (begin_B + tx < wB); // Output and shared memory write condition
    bool cond2 = (begin_A + ty < wA); // Output writing condition

    // Loop over sub-matrices of A and B
    for (int a = begin_A, b = begin_B; a <= end_A; a += step_A, b += step_B) {

        // Load matrices into shared memory
        if (a / pA + ty < dim) {
            shared_A[ty][tx] = cond0 ? A[a + pA * ty + tx] : 0;
            shared_B[ty][tx] = cond1 ? B[b + pB * ty + tx] : 0;
        } else {
            shared_A[ty][tx] = 0;
            shared_B[ty][tx] = 0;
        }

        // Synchronize threads for shared memory
        __syncthreads();

        // Compute sub-matrix difference
        if (cond2 && cond1) {
            for (int k = 0; k < BLOCK_DIM; ++k) {
                float tmp = abs(shared_A[k][ty] - shared_B[k][tx]);

                if (inf) {
                    ssd = norm > 0 ? max(ssd, tmp) : min(ssd, tmp);
                } else {
                    ssd += pow(tmp, norm);
                }
            }
            if (!inf) {
                ssd = pow(ssd, 1 / norm);
            }
        }

        // Synchronize threads before next iteration
        __syncthreads();
    }

    // Write the result to the output matrix
    if (cond2 && cond1) {
        AB[(begin_A + ty) * pB + begin_B + tx] = ssd;
    }
}