; ModuleID = '../data/hip_kernels/14264/1/main.cu'
source_filename = "../data/hip_kernels/14264/1/main.cu"
target datalayout = "e-p:64:64-p1:64:64-p2:32:32-p3:32:32-p4:64:64-p5:32:32-p6:32:32-i64:64-v16:16-v24:32-v32:32-v48:64-v96:128-v192:256-v256:256-v512:512-v1024:1024-v2048:2048-n32:64-S32-A5-G1-ni:7"
target triple = "amdgcn-amd-amdhsa"

; Function Attrs: argmemonly mustprogress nofree norecurse nosync nounwind
define protected amdgpu_kernel void @_Z31createFinalHiddenFeaturesKernelPKfS0_Pfii(float addrspace(1)* nocapture readonly %0, float addrspace(1)* nocapture readnone %1, float addrspace(1)* nocapture %2, i32 %3, i32 %4) local_unnamed_addr #0 {
  %6 = tail call i32 @llvm.amdgcn.workgroup.id.x()
  %7 = tail call align 4 dereferenceable(64) i8 addrspace(4)* @llvm.amdgcn.dispatch.ptr()
  %8 = getelementptr i8, i8 addrspace(4)* %7, i64 4
  %9 = bitcast i8 addrspace(4)* %8 to i16 addrspace(4)*
  %10 = load i16, i16 addrspace(4)* %9, align 4, !range !4, !invariant.load !5
  %11 = zext i16 %10 to i32
  %12 = getelementptr inbounds i8, i8 addrspace(4)* %7, i64 12
  %13 = bitcast i8 addrspace(4)* %12 to i32 addrspace(4)*
  %14 = load i32, i32 addrspace(4)* %13, align 4, !tbaa !6
  %15 = mul i32 %6, %11
  %16 = tail call i32 @llvm.amdgcn.workitem.id.x(), !range !15
  %17 = add i32 %15, %16
  %18 = icmp ult i32 %17, %4
  br i1 %18, label %19, label %175

19:                                               ; preds = %5
  %20 = icmp eq i32 %3, 0
  %21 = udiv i32 %14, %11
  %22 = mul i32 %21, %11
  %23 = icmp ugt i32 %14, %22
  %24 = zext i1 %23 to i32
  %25 = add i32 %21, %24
  %26 = mul i32 %25, %11
  %27 = and i32 %3, 1
  %28 = icmp eq i32 %3, 1
  %29 = and i32 %3, -2
  %30 = icmp eq i32 %27, 0
  br label %31

31:                                               ; preds = %19, %170
  %32 = phi i32 [ %17, %19 ], [ %173, %170 ]
  %33 = zext i32 %32 to i64
  br i1 %20, label %170, label %34

34:                                               ; preds = %31
  %35 = getelementptr inbounds float, float addrspace(1)* %2, i64 %33
  %36 = load float, float addrspace(1)* %35, align 4, !tbaa !16
  br i1 %28, label %125, label %37

37:                                               ; preds = %34, %37
  %38 = phi float [ %121, %37 ], [ 0.000000e+00, %34 ]
  %39 = phi i32 [ %122, %37 ], [ 0, %34 ]
  %40 = phi i32 [ %123, %37 ], [ 0, %34 ]
  %41 = mul i32 %39, 5
  %42 = mul i32 %41, %4
  %43 = add i32 %42, %32
  %44 = zext i32 %43 to i64
  %45 = getelementptr inbounds float, float addrspace(1)* %0, i64 %44
  %46 = load float, float addrspace(1)* %45, align 4, !tbaa !16
  %47 = fmul contract float %46, %36
  %48 = fadd contract float %38, %47
  %49 = or i32 %41, 1
  %50 = mul i32 %49, %4
  %51 = add i32 %50, %32
  %52 = zext i32 %51 to i64
  %53 = getelementptr inbounds float, float addrspace(1)* %0, i64 %52
  %54 = load float, float addrspace(1)* %53, align 4, !tbaa !16
  %55 = fmul contract float %54, %36
  %56 = fadd contract float %48, %55
  %57 = add i32 %41, 2
  %58 = mul i32 %57, %4
  %59 = add i32 %58, %32
  %60 = zext i32 %59 to i64
  %61 = getelementptr inbounds float, float addrspace(1)* %0, i64 %60
  %62 = load float, float addrspace(1)* %61, align 4, !tbaa !16
  %63 = fmul contract float %62, %36
  %64 = fadd contract float %56, %63
  %65 = add i32 %41, 3
  %66 = mul i32 %65, %4
  %67 = add i32 %66, %32
  %68 = zext i32 %67 to i64
  %69 = getelementptr inbounds float, float addrspace(1)* %0, i64 %68
  %70 = load float, float addrspace(1)* %69, align 4, !tbaa !16
  %71 = fmul contract float %70, %36
  %72 = fadd contract float %64, %71
  %73 = add i32 %41, 4
  %74 = mul i32 %73, %4
  %75 = add i32 %74, %32
  %76 = zext i32 %75 to i64
  %77 = getelementptr inbounds float, float addrspace(1)* %0, i64 %76
  %78 = load float, float addrspace(1)* %77, align 4, !tbaa !16
  %79 = fmul contract float %78, %36
  %80 = fadd contract float %72, %79
  %81 = mul i32 %39, 5
  %82 = add i32 %81, 5
  %83 = mul i32 %82, %4
  %84 = add i32 %83, %32
  %85 = zext i32 %84 to i64
  %86 = getelementptr inbounds float, float addrspace(1)* %0, i64 %85
  %87 = load float, float addrspace(1)* %86, align 4, !tbaa !16
  %88 = fmul contract float %87, %36
  %89 = fadd contract float %80, %88
  %90 = add i32 %81, 6
  %91 = mul i32 %90, %4
  %92 = add i32 %91, %32
  %93 = zext i32 %92 to i64
  %94 = getelementptr inbounds float, float addrspace(1)* %0, i64 %93
  %95 = load float, float addrspace(1)* %94, align 4, !tbaa !16
  %96 = fmul contract float %95, %36
  %97 = fadd contract float %89, %96
  %98 = add i32 %81, 7
  %99 = mul i32 %98, %4
  %100 = add i32 %99, %32
  %101 = zext i32 %100 to i64
  %102 = getelementptr inbounds float, float addrspace(1)* %0, i64 %101
  %103 = load float, float addrspace(1)* %102, align 4, !tbaa !16
  %104 = fmul contract float %103, %36
  %105 = fadd contract float %97, %104
  %106 = add i32 %81, 8
  %107 = mul i32 %106, %4
  %108 = add i32 %107, %32
  %109 = zext i32 %108 to i64
  %110 = getelementptr inbounds float, float addrspace(1)* %0, i64 %109
  %111 = load float, float addrspace(1)* %110, align 4, !tbaa !16
  %112 = fmul contract float %111, %36
  %113 = fadd contract float %105, %112
  %114 = add i32 %81, 9
  %115 = mul i32 %114, %4
  %116 = add i32 %115, %32
  %117 = zext i32 %116 to i64
  %118 = getelementptr inbounds float, float addrspace(1)* %0, i64 %117
  %119 = load float, float addrspace(1)* %118, align 4, !tbaa !16
  %120 = fmul contract float %119, %36
  %121 = fadd contract float %113, %120
  %122 = add nuw i32 %39, 2
  %123 = add i32 %40, 2
  %124 = icmp eq i32 %123, %29
  br i1 %124, label %125, label %37, !llvm.loop !20

125:                                              ; preds = %37, %34
  %126 = phi float [ undef, %34 ], [ %121, %37 ]
  %127 = phi float [ 0.000000e+00, %34 ], [ %121, %37 ]
  %128 = phi i32 [ 0, %34 ], [ %122, %37 ]
  br i1 %30, label %170, label %129

129:                                              ; preds = %125
  %130 = mul i32 %128, 5
  %131 = mul i32 %130, %4
  %132 = add i32 %131, %32
  %133 = zext i32 %132 to i64
  %134 = getelementptr inbounds float, float addrspace(1)* %0, i64 %133
  %135 = load float, float addrspace(1)* %134, align 4, !tbaa !16
  %136 = fmul contract float %135, %36
  %137 = fadd contract float %127, %136
  %138 = add i32 %130, 1
  %139 = mul i32 %138, %4
  %140 = add i32 %139, %32
  %141 = zext i32 %140 to i64
  %142 = getelementptr inbounds float, float addrspace(1)* %0, i64 %141
  %143 = load float, float addrspace(1)* %142, align 4, !tbaa !16
  %144 = fmul contract float %143, %36
  %145 = fadd contract float %137, %144
  %146 = add i32 %130, 2
  %147 = mul i32 %146, %4
  %148 = add i32 %147, %32
  %149 = zext i32 %148 to i64
  %150 = getelementptr inbounds float, float addrspace(1)* %0, i64 %149
  %151 = load float, float addrspace(1)* %150, align 4, !tbaa !16
  %152 = fmul contract float %151, %36
  %153 = fadd contract float %145, %152
  %154 = add i32 %130, 3
  %155 = mul i32 %154, %4
  %156 = add i32 %155, %32
  %157 = zext i32 %156 to i64
  %158 = getelementptr inbounds float, float addrspace(1)* %0, i64 %157
  %159 = load float, float addrspace(1)* %158, align 4, !tbaa !16
  %160 = fmul contract float %159, %36
  %161 = fadd contract float %153, %160
  %162 = add i32 %130, 4
  %163 = mul i32 %162, %4
  %164 = add i32 %163, %32
  %165 = zext i32 %164 to i64
  %166 = getelementptr inbounds float, float addrspace(1)* %0, i64 %165
  %167 = load float, float addrspace(1)* %166, align 4, !tbaa !16
  %168 = fmul contract float %167, %36
  %169 = fadd contract float %161, %168
  br label %170

170:                                              ; preds = %129, %125, %31
  %171 = phi float [ 0.000000e+00, %31 ], [ %126, %125 ], [ %169, %129 ]
  %172 = getelementptr inbounds float, float addrspace(1)* %2, i64 %33
  store float %171, float addrspace(1)* %172, align 4, !tbaa !16
  %173 = add i32 %26, %32
  %174 = icmp ult i32 %173, %4
  br i1 %174, label %31, label %175, !llvm.loop !22

175:                                              ; preds = %170, %5
  ret void
}

; Function Attrs: mustprogress nofree nosync nounwind readnone speculatable willreturn
declare align 4 i8 addrspace(4)* @llvm.amdgcn.dispatch.ptr() #1

; Function Attrs: mustprogress nofree nosync nounwind readnone speculatable willreturn
declare i32 @llvm.amdgcn.workitem.id.x() #1

; Function Attrs: mustprogress nofree nosync nounwind readnone speculatable willreturn
declare i32 @llvm.amdgcn.workgroup.id.x() #1

attributes #0 = { argmemonly mustprogress nofree norecurse nosync nounwind "amdgpu-flat-work-group-size"="1,256" "frame-pointer"="none" "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="gfx906" "target-features"="+16-bit-insts,+ci-insts,+dl-insts,+dot1-insts,+dot2-insts,+dot7-insts,+dpp,+flat-address-space,+gfx8-insts,+gfx9-insts,+s-memrealtime,+s-memtime-inst,+sramecc" "uniform-work-group-size"="true" }
attributes #1 = { mustprogress nofree nosync nounwind readnone speculatable willreturn }

!llvm.module.flags = !{!0, !1}
!opencl.ocl.version = !{!2}
!llvm.ident = !{!3}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 7, !"PIC Level", i32 1}
!2 = !{i32 2, i32 0}
!3 = !{!"clang version 15.0.0 (http://10.15.3.7/dcutoolkit/driverruntime/llvm-project.git 340750feeda88c9c2ce8ad481b11d9aa7f033d39)"}
!4 = !{i16 1, i16 1025}
!5 = !{}
!6 = !{!7, !11, i64 12}
!7 = !{!"hsa_kernel_dispatch_packet_s", !8, i64 0, !8, i64 2, !8, i64 4, !8, i64 6, !8, i64 8, !8, i64 10, !11, i64 12, !11, i64 16, !11, i64 20, !11, i64 24, !11, i64 28, !12, i64 32, !13, i64 40, !12, i64 48, !14, i64 56}
!8 = !{!"short", !9, i64 0}
!9 = !{!"omnipotent char", !10, i64 0}
!10 = !{!"Simple C/C++ TBAA"}
!11 = !{!"int", !9, i64 0}
!12 = !{!"long", !9, i64 0}
!13 = !{!"any pointer", !9, i64 0}
!14 = !{!"hsa_signal_s", !12, i64 0}
!15 = !{i32 0, i32 1024}
!16 = !{!17, !17, i64 0}
!17 = !{!"float", !18, i64 0}
!18 = !{!"omnipotent char", !19, i64 0}
!19 = !{!"Simple C++ TBAA"}
!20 = distinct !{!20, !21}
!21 = !{!"llvm.loop.mustprogress"}
!22 = distinct !{!22, !21}
