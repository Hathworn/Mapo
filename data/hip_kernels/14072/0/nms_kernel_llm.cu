...
__global__ void nms_kernel(const int n_boxes, const float nms_overlap_thres, const float *dev_boxes, unsigned long long *dev_mask)
{
    ...
    {
        // Coalesced memory access for loading block_boxes
        const float *base_box = dev_boxes + (threadsPerBlock * col_start + threadIdx.x) * 5;
        block_boxes[threadIdx.x * 5 + 0] = base_box[0];
        block_boxes[threadIdx.x * 5 + 1] = base_box[1];
        block_boxes[threadIdx.x * 5 + 2] = base_box[2];
        block_boxes[threadIdx.x * 5 + 3] = base_box[3];
        block_boxes[threadIdx.x * 5 + 4] = base_box[4];
    }
    __syncthreads();
    ...
    if (threadIdx.x < row_size)
    {
        ...
        // Unroll loop for coalesced memory access in warp
