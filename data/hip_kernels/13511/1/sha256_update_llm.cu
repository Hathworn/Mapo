...
__device__ void sha256_transform(SHA256_CTX *ctx, uchar data[])
{
    ...
    uint m[64];
    uint a = ctx[q].state[0], b = ctx[q].state[1], c = ctx[q].state[2], d = ctx[q].state[3];
    uint e = ctx[q].state[4], f = ctx[q].state[5], g = ctx[q].state[6], h = ctx[q].state[7];
    ...
    #pragma unroll
    for (int i = 0; i < 64; ++i) {
        uint t1 = h + EP1(e) + CH(e,f,g) + k[i] + m[i];
