...
// Optimize memory access and loop
__global__ void update_linear_and_quadratic_terms_kernel(int32_t n, float old_num_frames, float prior_offset, float* cur_tot_weight, int32_t max_count, float* quadratic, float* linear) {
    ...
    // Restructure loop to avoid redundant calculations
    int32_t startIdx = blockIdx.x * blockDim.x + threadIdx.x;
    int32_t stride = blockDim.x * gridDim.x;

    for (int32_t i = startIdx; i < n; i += stride) {
        int32_t diag_idx = (i * (i + 3) / 2);
        quadratic[diag_idx] += prior_scale_change;
    }

    // Use shared memory or atomic operations if required for performance
