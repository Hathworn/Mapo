...
__global__ void power_spectrum_kernel(int row_length, const float *A_in, int32_t ldi, float *A_out, int32_t ldo, bool use_power) {
    ...
    for (int idx = thread_id; idx < half_length; idx += blockDim.x) {
        if (idx == 0) continue;  // Skip the special case

        float2 val = reinterpret_cast<const float2 *>(Ar)[idx];
        float ret = val.x * val.x + val.y * val.y;

        Aw[idx] = use_power ? ret : sqrtf(ret);  // Conditional operation
    }

    // Handle special case for thread 0
    if (threadIdx.x == 0) {
        float real = Ar[0];
        float im = Ar[row_length];

        Aw[0] = use_power ? real * real : fabs(real);  // Conditional assignment
        Aw[half_length] = use_power ? im * im : fabs(im);  // Conditional assignment
    }
}