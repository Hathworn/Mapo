...
__global__ void downscale(float *gradInput_data, const float *gradOutput_data, long no_elements, int scale_factor, int d1, int d2, int d3) {
    ...
    for (int i = 0; i < scale_factor; i++) {
        int ipidx_i = translate_idx_inv(ii, d1, d2, d3, scale_factor, i, 0);
        gradInput_data[ii] += gradOutput_data[ipidx_i];

        for (int j = 1; j < scale_factor; j++) {
            int ipidx_j = translate_idx_inv(ii, d1, d2, d3, scale_factor, i, j);
            gradInput_data[ii] += gradOutput_data[ipidx_j];
        }
    }
}