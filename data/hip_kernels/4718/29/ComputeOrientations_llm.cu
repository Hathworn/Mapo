...
__global__ void ComputeOrientations(float *g_Data, int *d_Ptrs, float *d_Orient, int maxPts, int w, int h)
{
    ...
    // Load patch data into shared memory
    for (int y = 0; y < 15; y++) {
        int memPos = 16 * y + x;
        int xi = xp + x;
        int yi = yp + y;
    ...
    for (int y = 1; y < 14; y++) {
        ...
        if (x >= 1 && x < 14) {
            ...
            float weightedGrad = grad * gauss[x] * gauss[y];
            atomicAdd(&hist[32 * (x - 1) + bin], weightedGrad); // Atomic add to avoid race conditions
        }
    }
    __syncthreads();

    // Reduce over histogram bins
    ...
    if (tx == 0) {
        for (int i = 0; i < 32; i++) {
            hist[i + 32] = 6 * hist[i] 
                         + 4 * (hist[(i + 1) & 31] + hist[(i + 31) & 31]) 
                         + hist[(i + 2) & 31] + hist[(i + 30) & 31];
        }
    }
    __syncthreads();
    ...
    // Finding maximum orientation and storing results
    if (tx == 0) {
        float maxval1 = 0.0f;
        float maxval2 = 0.0f;
        int i1 = -1;
