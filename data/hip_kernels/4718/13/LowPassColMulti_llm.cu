...
__global__ void LowPassColMulti(float *d_Result, float *d_Data, int width, int pitch, int height)
{
    ...
    if (colStart < width) {
        ...
        for (int y = totStart + ty; y <= totEnd; y += blockDim.y) {
            // Use max() and min() to handle boundary conditions more efficiently
            int yClamped = max(0, min(y, height - 1));
            sdata[0] = d_Data[colStart + yClamped * pitch];
            sdata += sStep;
            gPos += gStep;
        }
    }
    ...
    if (colStart < width) {
        ...
        for (int y = miny + ty; y <= maxy; y += blockDim.y) {
            // Restructure summation to enhance memory coalescing
            float result = 0;
            for (int k = 0; k <= 4; k++) {
                result += (sdata[k * CONVCOL_W] + sdata[(8 - k) * CONVCOL_W]) * kernel[k];
            }
            result += sdata[4 * CONVCOL_W] * kernel[4]; // Center element
