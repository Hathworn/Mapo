...
{
    // Optimize index calculation and branching
    int2 image_index_2d = make_int2(blockIdx.x * blockDim.x + threadIdx.x, blockIdx.y * blockDim.y + threadIdx.y);

    if (image_index_2d.x < num_pixels_x && image_index_2d.y < num_pixels_y)
    {
        int  image_index_1d = (num_pixels_x * image_index_2d.y) + image_index_2d.x; // Linear index
        ...
        float inv_y = 1.0f / y; // Precompute division
        float X_new = x * Y_new * inv_y;
        float Z_new = (1 - x - y) * Y_new * inv_y;

        // Store in registers before storing to global memory
