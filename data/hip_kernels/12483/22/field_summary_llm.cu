...
__global__ void field_summary( const int x_inner, const int y_inner, const int halo_depth, const double* volume, const double* density, const double* energy0, const double* u, double* vol_out, double* mass_out, double* ie_out, double* temp_out)
{
    ...
    // Declare shared memory for using only necessary threads
    ...
    // Initialize shared memory with all active threads
    vol_shared[lid] = 0.0;
    mass_shared[lid] = 0.0;
    ie_shared[lid] = 0.0;
    temp_shared[lid] = 0.0;
    ...
    for (int ii = BLOCK_SIZE / 2; ii > 0; ii >>= 1)
    {
        if (lid < ii)
        {
    ...
    // Output the results from the first thread of each block
    if (lid == 0)
    {
