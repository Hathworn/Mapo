...
__global__ void downscale(float *gradInput_data, const float *gradOutput_data, long no_elements, int scale_factor, int d1, int d2, int d3) {
    ...
    // Use local variables to reduce index recalculations
    int idx_x = ii % d3;
    int idx_z = (ii / d3) % d2;
    int idx_y = (ii / (d3 * d2)) % d1;
    int idx_w = (ii / (d3 * d2 * d1));

    // Unroll the loops over scale_factor
    for (int i = 0; i < scale_factor; i++) {
        for (int j = 0; j < scale_factor; j++) {
            int ipidx = ((idx_w * d1 + idx_y) * d2 + idx_z) * d3 + idx_x;
            ipidx += (j * d3 + i);
