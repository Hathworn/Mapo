...
__global__ void LowPassRowMulti(float *d_Result, float *d_Data, int width, int pitch, int height)
{
    ...
    // Load data with boundary conditions
    if (loadPos < 0)
        data[tx] = d_Data[yptr];
    ...
    // Apply convolution only if the output is within the image width
    if (xout < width && tx < CONVROW_W) {
        float sum = kernel[0] * (data[tx] + data[tx + 8]) +
                    kernel[1] * (data[tx + 1] + data[tx + 7]) +
                    kernel[2] * (data[tx + 2] + data[tx + 6]) +
                    kernel[3] * (data[tx + 3] + data[tx + 5]) +
                    kernel[4] * data[tx + 4];
