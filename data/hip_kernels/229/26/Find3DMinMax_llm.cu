...
__global__ void Find3DMinMax(int *d_Result, float *d_Data1, float *d_Data2, float *d_Data3, int width, int height) {
    ...
    for (int y = 0; y < 34; y++) {
        ...
        if (y > 1) {
            if (tx < memWid) {
                ...
                // Consolidate min/max computations
                ymin1[tx] = min1;
        ...
        if (y > 1) {
            if (tx < MINMAX_SIZE) {
                if (xWritePos <= xEndClamped) {
                    ...
                    // Simplify conditions
                    if (data2[memPos1 + 1] < minv || data2[memPos1 + 1] > maxv) output |= 0x80000000;
                }
            }
        }
        __syncthreads();
    ...
    if (tx < MINMAX_SIZE && xWritePos < width) {
        const int writeStart = blockIdx.y * width + xWritePos;
