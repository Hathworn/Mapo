...
// Optimize: Reduce conditional overhead by accessing shared memory with stride
if (t_ind < MED_BLOCK_SIZE / 3) {
    for (int i = 0; i < 3; ++i) {
        int sample_ind = d_start_inds[segment] +
            floorf(d_random_numbers[g_ind + i * MED_BLOCK_SIZE / 3] * (float)d_n_in[segment]);
        DATA[t_ind + i * MED_BLOCK_SIZE / 3] = d_in[sample_ind];
    }
}
