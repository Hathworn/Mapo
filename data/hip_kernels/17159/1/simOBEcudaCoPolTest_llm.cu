...
__device__ void stepOBE(double* dxdy, const double* rho, const double laser, const double w, const double w2, const double g12, const double g13, const double g23, const double g22) {
    // Optimized calculation with shared memory
    // Reduce redundant calculations
    double common_laser_rho0 = laser * rho[0];
    double common_laser_rho1 = laser * rho[1];
    double common_laser_rho3 = laser * rho[3];
    double common_w_rho0 = w * rho[0];

    //12R
    dxdy[0] = -common_laser_rho3 - w * rho[1] - g12 * rho[0];
    //12I
    dxdy[1] = common_laser_rho0 - common_laser_rho0 + common_w_rho0 + laser * (-rho[7] + rho[6] + rho[2]) - g12 * rho[1];
    //13R
    dxdy[2] = laser * rho[5] - common_laser_rho1 - w2 * rho[3] - g13 * rho[2];
    //13I
    dxdy[3] = common_laser_rho0 - laser * rho[4] + w2 * rho[2] - g13 * rho[3];
    //23R
    dxdy[4] = common_laser_rho3 - w * rho[5] - g23 * rho[4];
    //23I
    dxdy[5] = laser * (-rho[8] + rho[7] - rho[2]) + w * rho[4] - g23 * rho[5];
    //11R
    dxdy[6] = -2 * common_laser_rho1 + g22 * rho[7];
    //22R
    dxdy[7] = 2 * (common_laser_rho1 - laser * rho[5]) - g22 * rho[7];
    //33R
...
__global__ void simOBEcudaCoPolTest(double* AC, const double* Delaylist, const double w, const double FWHM, const double G1, const double G2, const double G3, const double t_min) {
    ...
    double k1[9], k2[9], k3[9], k4[9], tmp[9];
