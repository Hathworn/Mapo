...
__global__ void max_pooling_kernel(float *feature_map, float *probs, float *target, int feature_map_size, int feature_map_num, int pooling_rate, float *rnd_array, int rnd_num) {
    ...
    int rnd_index = (blockIdx.y * blockDim.y + threadIdx.y) * blockDim.x * blockIdx.x + threadIdx.x;
    rnd_index %= rnd_num;  // Calculate once and use % rnd_num
    ...
    // Coalesce memory accesses for readability
    ...
    for (int i = 0; i < pooling_rate; i++) {
        for (int j = 0; j < pooling_rate; j++) {
            expVal = shFm[threadIdx.y * pooling_rate + i][threadIdx.x * pooling_rate + j];
            if (expVal > 50) expVal = 50.0f;
            expVal = __expf(expVal);
            shFm[threadIdx.y * pooling_rate + i][threadIdx.x * pooling_rate + j] = expVal;
            sum += expVal;
        }
    }

    float recip_sum = 1.0f / (1.0f + sum);  // Avoid repeated division
    for (int i = 0; i < pooling_rate; i++) {
        for (int j = 0; j < pooling_rate; j++) {
            shFm[threadIdx.y * pooling_rate + i][threadIdx.x * pooling_rate + j] *= recip_sum; 
