...
__global__ void convolution_forward_kernel(float *input, float *filters, float *feature_map, float *hbias, int input_size, int channel_num, int feature_map_size, int filter_size, int filter_num, int lu_padding, float sigma) {
    // Use shared memory to preload data
    extern __shared__ float shared_memory[];
    float* shImg = shared_memory;
    float* shFilter = shared_memory + (32 + MAX_FILETER_SIZE - 1) * (32 + MAX_FILETER_SIZE - 1);
    ...
    for (int g = 0; g < channel_num; g++) {
        ...
        // Load filter into the shared memory
        if (threadIdx.x < filter_size && threadIdx.y < filter_size) {
            shFilter[threadIdx.y * filter_size + threadIdx.x] =
        ...
        // Preload the image data into shared memory
        float *img = input + imgIdx * input_size * input_size * channel_num
            + g * input_size * input_size;

        float *shImgLoad = &shImg[threadIdx.y * (32 + MAX_FILETER_SIZE - 1) + threadIdx.x];
        ...
        if(threadIdx.x < MAX_FILETER_SIZE - 1) {
            shImgLoad = &shImg[threadIdx.y * (32 + MAX_FILETER_SIZE - 1) + (threadIdx.x + 32)];
            if(ty < lu_padding || (tx + 32) >= (input_size + lu_padding)) {
                *shImgLoad = 0;
            } else {
                *shImgLoad = img[(ty-lu_padding) * input_size + (tx + 32-lu_padding)];
            }
        }
        
        if(threadIdx.y < MAX_FILETER_SIZE - 1) {
            shImgLoad = &shImg[(threadIdx.y + 32) * (32 + MAX_FILETER_SIZE - 1) + threadIdx.x];
            if(tx < lu_padding || (ty + 32) >= (input_size + lu_padding)) {
                *shImgLoad = 0;
            } else {
                *shImgLoad = img[(ty + 32-lu_padding) * input_size + (tx-lu_padding)];
            }
            
            if(threadIdx.x < MAX_FILETER_SIZE - 1) {
                shImgLoad = &shImg[(threadIdx.y + 32) * (32 + MAX_FILETER_SIZE - 1) + (threadIdx.x + 32)];
                if((ty + 32) >= (input_size + lu_padding) || (tx + 32) >= (input_size + lu_padding)) {
                    *shImgLoad = 0;
                } else {
                    *shImgLoad = img[(ty + 32-lu_padding) * input_size + (tx + 32-lu_padding)];
                }
            }
        }
        __syncthreads();

        float *imgPtr = &shImg[threadIdx.y * (32 + MAX_FILETER_SIZE - 1) + threadIdx.x];

        // Compute convolution
        for (int i = 0; i < filter_size; i++) {
            for (int j = 0; j < filter_size; j++) {
                local_target += imgPtr[j] * shFilter[i * filter_size + j];
            }
