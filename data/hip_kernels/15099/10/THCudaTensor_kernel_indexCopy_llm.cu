...
__global__ void THCudaTensor_kernel_indexCopy( float *res, float *src, long* res_stride, float *index, long res_nDim, int dim, long idx_size, long src_size, long size_dim )
{
    ...
    // Precompute thread limits and grid-stride loop size
    long flat_size = src_size / idx_size;
    long stride_d;

    for (int idx = thread_idx; idx < flat_size; idx += gridDim.x * blockDim.x)
    {
        long coeff = 0;
        for (int i=0; i<idx_size; i++)
        {
            ...
            // Simplified loop and branching
            for (int d=0; d<res_nDim; d++)
            {
                if (d < dim)
                {
                    stride_d = res_stride[d] / size_dim;
            ...
            // Use a direct addressing with precomputed indexes
