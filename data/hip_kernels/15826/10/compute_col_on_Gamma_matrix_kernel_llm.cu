...
__global__ void compute_col_on_Gamma_matrix_kernel(int col_index, int vertex_index, int* indices, double* exp_V, double* N_ptr, int LD_N, double* G_ptr, int LD_G, double* col_ptr, int incr) {
    // Use blockIdx.x directly to get the index
    int l = blockIdx.x; 
    ...
    if (j_index < vertex_index) {
        // Use ternary operator for concise delta calculation
        delta = (i_index == j_index) ? 1.0 : 0.0;
        col_ptr[l * incr] = (N_ptr[i_index + LD_N * j_index] * exp_Vj - delta) / (exp_Vj - 1.0);
    } else {
