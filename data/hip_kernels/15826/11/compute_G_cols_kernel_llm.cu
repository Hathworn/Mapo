...
__global__ void compute_G_cols_kernel(int N_i, int N_r, int N_c, int* p_ptr, double* exp_V_ptr, double* N_ptr, int N_ld, double* G_ptr, int G_ld, double* G_cols_ptr, int G_cols_ld) {
    ...
    int l_MIN = BLOCK_SIZE_y * blockIdx.y;
    ...
    if (I < N_r) {
        ...
        // Pre-compute 1/(exp_V[l] - 1) once for efficiency
        double inv_exp_diff[N_i];
        for (int l = l_MIN; l < l_MAX; ++l) {
            inv_exp_diff[l] = 1.0 / (exp_V_ptr[l] - 1.0);
        }
        ...
        for (int l = l_MIN; l < l_MAX; ++l) {
            if (p_ptr[l] >= N_c) {
                ...
                double alpha = exp_V_ptr[l] * inv_exp_diff[l];
                G_cols_ptr[I + l * G_cols_ld] = alpha * N_ptr[I + p_ptr[l] * N_ld];
            }
        }

        for (int l = l_MIN; l < l_MAX; ++l) {
            if (p_ptr[l] < N_c && I == p_ptr[l]) {
                G_cols_ptr[I + l * G_cols_ld] -= inv_exp_diff[l];
            }
        }
    }
}