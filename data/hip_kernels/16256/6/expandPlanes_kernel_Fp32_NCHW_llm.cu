...
__global__ void expandPlanes_kernel_Fp32_NCHW(float* output, const uint64_t* masks, const float* values, int n) {
    ...
    // Load inputs to shared memory with coalesced access.
    if (threadIdx.x < kNumShmemElements) {
        // Direct load to shared memory to reduce latency.
        shMasks[threadIdx.x] = masks[planeIndex + threadIdx.x];
        shVals[threadIdx.x] = values[planeIndex + threadIdx.x];
    }
    __syncthreads();

    uint64_t mask = shMasks[planeIndex]; // Direct indexing to save recalculation.
    
    int sqIndex = index & 0x3F;
    float op = shVals[planeIndex] * !!(mask & (1ull << sqIndex)); // Single line for condition and operation.
