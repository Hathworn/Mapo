...
__global__ void cunn_LookupTable_accGradParametersKernel( float *input, float *indices, float *gradOutput, float *gradWeight, float *count, float defaultScale, long numel, long stride, int paddingValue) {
    ...
    int idx = blockIdx.x * blockDim.y + threadIdx.y; // Compute correct index based on blockDim.y

    // Ensure warp-level parallelism and consecutive memory accesses
    const int SZ = 4;
    ...
    if (idx < numel
        ...
        do {
            ...
            // Load gradients and weights into shared memory or private variables for efficiency
            #pragma unroll
            ...
            // Update weight using preloaded data
            #pragma unroll
            for (int ii = 0; ii < SZ; ii++) {
