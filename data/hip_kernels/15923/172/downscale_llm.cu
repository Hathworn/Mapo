...
__global__ void downscale(float *gradInput_data, const float *gradOutput_data, long no_elements, int scale_factor, int d1, int d2, int d3) {
    ...
    const int total_elements = scale_factor * scale_factor;

    // Use registers for loop invariants for efficiency
    float sum = 0.0f;
    for (int offset = 0; offset < total_elements; ++offset) {
        const int i = offset / scale_factor;
        const int j = offset % scale_factor;
        int ipidx = translate_idx_inv(ii, d1, d2, d3, scale_factor, i, j);
        sum += gradOutput_data[ipidx];
    }
    gradInput_data[ii] = sum;  // Store sum directly
}