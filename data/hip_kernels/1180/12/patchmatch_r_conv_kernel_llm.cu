...
__global__ void patchmatch_r_conv_kernel(float *input, float *target, float *conv, int patch, int stride, int c1, int h1, int w1, int h2, int w2) {
    ...
    if (id < N) {
        ...
        // Optimize loop unrolling and minimize thread divergence
        #pragma unroll
        for (int dy = -kernel_radius; dy <= kernel_radius; dy += stride) {
            for (int dx = -kernel_radius; dx <= kernel_radius; dx += stride) {
                ...
                if (0 <= xx1 && xx1 < w1 && 0 <= yy1 && yy1 < h1 && 0 <= xx2 && xx2 < w2 && 0 <= yy2 && yy2 < h2) {
                    int _id1 = yy1 * w1 + xx1, _id2 = yy2 * w2 + xx2;
                    // Use local variables to reduce memory access
                    float local_conv_result = 0, local_norm_1 = 0, local_norm_2 = 0;
                    for (int c = 0; c < c1; c++) {
                        float term1 = input[c * size1 + _id1];
                        float term2 = target[c * size2 + _id2];
                        local_conv_result += term1 * term2;
                        local_norm_1 += term1 * term1;
                        local_norm_2 += term2 * term2;
                    }
                    conv_result += local_conv_result;
                    norm_1 += local_norm_1;
                    norm_2 += local_norm_2;
                }
            }
