...
__global__ void blend_kernel(float *A, float *BP, float *M, float *AP, float alpha, int c, int h, int w)
{
    ...
    if (_id < c * size) {
        ...
        // Pre-calculate indices for better memory access
        int index = dc * size + id;

        // Use ternary operator directly to improve readability and performance
        float weight = (M[id] < 0.05f) ? 0.f : alpha;
        
        // Optimize memory access using pre-calculated index
