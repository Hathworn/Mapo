...
__global__ void patchmatch2_conv_kernel(float *A, float *B, float *AP, float *BP, float *conv, int *prev_corrAB_upsampled, int patch, int s_rad, int c, int h, int w) {
    ...
    if (_id < size1 * s_n) {
        ...
        // Perform convolution within a kernel radius
        ...
        for (int dy = -kernel_radius; dy <= kernel_radius; dy++) {
            for (int dx = -kernel_radius; dx <= kernel_radius; dx++) {
                ...
                if (xx1 >= 0 && xx1 < w1 && yy1 >= 0 && yy1 < h1 && xx2 >= 0 && xx2 < w2 && yy2 >= 0 && yy2 < h2) {
                    int _id1 = yy1 * w1 + xx1;
                    int _id2 = yy2 * w2 + xx2;

                    // Compute convolution sum for valid coordinates
        ...
        // Normalize result by the count
        conv[_id] = cnt > 0 ? conv_result / cnt : 0.0f;
    }
}