...
__global__ void Correlation_backward_input1(int item, float *gradInput1, int nInputChannels, int inputHeight, int inputWidth, float *gradOutput, int nOutputChannels, int outputHeight, int outputWidth, float *rInput2, int pad_size, int kernel_size, int max_displacement, int stride1, int stride2)
{
    ...
    if (xmax < 0 || ymax < 0 || xmin >= outputWidth || ymin >= outputHeight || xmin > xmax || ymin > ymax) {
        // Exit if outside boundaries. gradInput1 assumed zero initialized.
        return;
    }
    ...
    extern __shared__ float prod_sum[]; // Use external shared memory for flexibility
    prod_sum[tch_off] = 0;
    ...
    if (tch_off == 0) {
        float reduce_sum = 0;
        for (int idx = 0; idx < blockDim.x; idx++) { // Use blockDim.x for loop limit
