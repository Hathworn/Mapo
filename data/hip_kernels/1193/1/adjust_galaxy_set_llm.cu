...
__global__ void adjust_galaxy_set(Galaxy *galaxy_set, int n)
{
    ...
    // Use thread block stride loop for efficiency
    for (int i = index; i < n; i += stride)
    {
        float declination = arcminutes_to_radians(galaxy_set[i].declination);
        galaxy_set[i].declination = declination;
        galaxy_set[i].declination_cos = __cosf(declination); // Use faster intrinsic function for cos
        galaxy_set[i].declination_sin = __sinf(declination); // Use faster intrinsic function for sin
