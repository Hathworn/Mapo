...
__global__ void KerComputeSpsTau(unsigned n, unsigned pini, float smag, float blin, const float4 *velrhop, const float2 *gradvelff, float2 *tauff)
{
    ...
    if (p < n) {
        ...
        const float2 rr0 = gradvelff[p1 * 3];
        const float2 rr1 = gradvelff[p1 * 3 + 1];
        const float2 rr2 = gradvelff[p1 * 3 + 2];
        ...
        const float prr = (grad_xy * grad_xy + grad_xz * grad_xz + grad_yz * grad_yz) + 2 * pow1;
        ...
        const float twovisc_sps = 2 * visc_sps;

        // Use inverse of rho from velrhop
