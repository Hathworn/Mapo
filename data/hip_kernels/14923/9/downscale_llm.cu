...
__global__ void downscale(float *gradInput_data, float *gradOutput_data, long no_elements, int scale_factor, int d1, int d2, int d3)
{
    ...
    int offset = scale_factor * scale_factor;
    int idx_counter = 0;
    int ipidx_array[offset];
    
    // Precompute input indices
    for (int i = 0; i < scale_factor; ++i) {
        for (int j = 0; j < scale_factor; ++j) {
            ipidx_array[idx_counter++] = translate_idx_inv(ii, d1, d2, d3, scale_factor, i, j);
        }
    }
    ...
    for (int idx = 0; idx < offset; ++idx) {
        sum += gradOutput_data[ipidx_array[idx]];
    }
