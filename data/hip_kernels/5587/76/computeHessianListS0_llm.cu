    ...
    if (col < 6) {
        ...
        for (int i = id; i < valid_points_num; i += stride) {
            ...
            for (int j = starting_voxel_id[i]; j < starting_voxel_id[i + 1]; j++) {
                ...
                // Pre-compute common sub-expressions for efficiency
                double cov_term0 = icov00[vid] * pg0 + icov01[vid] * pg1 + icov02[vid] * pg2;
                double cov_term1 = icov10[vid] * pg0 + icov11[vid] * pg1 + icov12[vid] * pg2;
                double cov_term2 = icov20[vid] * pg0 + icov21[vid] * pg1 + icov22[vid] * pg2;

                // Update hessian
                tmp_h[j] = diff_x * cov_term0 + diff_y * cov_term1 + diff_z * cov_term2;
            }
        }
    }
}