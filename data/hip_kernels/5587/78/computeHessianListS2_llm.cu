    ...
    if (row < 6 && col < 6) {
        ...
        for (int i = id; i < valid_points_num; i += stride) {
            ...
            for (int j = starting_voxel_id[i]; j < starting_voxel_id[i + 1]; j++) {
                ...
                // Skip unnecessary calculations for invalid values
                if (!(tmp_ex > 1 || tmp_ex < 0 || isnan(tmp_ex))) {
                    tmp_ex *= gauss_d1;

                    double dx_c = d_x - centroid_x[vid];
                    double dy_c = d_y - centroid_y[vid];
                    double dz_c = d_z - centroid_z[vid];

                    // Use fused multiply-add for better performance
                    final_hessian += fma(dx_c, icov00[vid] * ph0 + icov01[vid] * ph1 + icov02[vid] * ph2, 0.0) * tmp_ex;
                    final_hessian += fma(dy_c, icov10[vid] * ph0 + icov11[vid] * ph1 + icov12[vid] * ph2, 0.0) * tmp_ex;
                    final_hessian += fma(dz_c, icov20[vid] * ph0 + icov21[vid] * ph1 + icov22[vid] * ph2, 0.0) * tmp_ex;
                }
            }
