...
__global__ void ReplaceKernelA(const float* p_Input, float* p_Output, int p_Width, int p_Height, float hueRangeA, float hueRangeB, float hueRangeWithRollOffA, float hueRangeWithRollOffB, float satRangeA, float satRangeB, float satRolloff, float valRangeA, float valRangeB, float valRolloff, int OutputAlpha, int DisplayAlpha, float p_Black, float p_White) {
    ...
    if (x < p_Width && y < p_Height) {
        ...
        // Calculate saturation coefficient
        float s0 = satRangeA;
        float s1 = satRangeB;
        ...
        float coeff = fminf(fminf(hcoeff, scoeff), vcoeff);
        float A = OutputAlpha == 0 ? 1.0f : OutputAlpha == 1 ? hcoeff :
                  OutputAlpha == 2 ? scoeff : OutputAlpha == 3 ? vcoeff :
                  OutputAlpha == 4 ? fminf(hcoeff, scoeff) : OutputAlpha == 5 ?
                  fminf(hcoeff, vcoeff) : OutputAlpha == 6 ? fminf(scoeff, vcoeff) :
                  fminf(fminf(hcoeff, scoeff), vcoeff);

        if (DisplayAlpha == 0)
            A = coeff;
        if (p_Black > 0.0f)
            A = fmaxf(A - (p_Black * 4.0f) * (1.0f - A), 0.0f);
        if (p_White > 0.0f)
            A = fminf(A * (1.0f + p_White * 4.0f), 1.0f);
