#include "hip/hip_runtime.h"
#include "includes.h"

__global__ void MatrixMulSh(float *Md, float *Nd, float *Pd, const int WIDTH) {
    // Allocate shared memory for tiles
    __shared__ float Mds[TILE_WIDTH][TILE_WIDTH];
    __shared__ float Nds[TILE_WIDTH][TILE_WIDTH];

    // Calculate row and column indexes of Pd to compute
    unsigned int col = TILE_WIDTH * blockIdx.x + threadIdx.x;
    unsigned int row = TILE_WIDTH * blockIdx.y + threadIdx.y;

    // Initialize accumulator for Pd element
    float Pvalue = 0;

    // Loop over the Md and Nd tiles required to compute Pd element
    for (int m = 0; m < WIDTH / TILE_WIDTH; m++) {
        // Collaborative loading of Md and Nd tiles into shared memory
        Mds[threadIdx.y][threadIdx.x] = Md[row * WIDTH + (m * TILE_WIDTH + threadIdx.x)];
        Nds[threadIdx.y][threadIdx.x] = Nd[(m * TILE_WIDTH + threadIdx.y) * WIDTH + col];
        __syncthreads(); // Synchronize to ensure tiles are loaded

        // Multiply Md and Nd tiles to accumulate results
        for (int k = 0; k < TILE_WIDTH; k++)
            Pvalue += Mds[threadIdx.y][k] * Nds[k][threadIdx.x];
        __syncthreads(); // Synchronize before loading new tiles
    }

    // Write the result to the Pd matrix
    Pd[row * WIDTH + col] = Pvalue;
}