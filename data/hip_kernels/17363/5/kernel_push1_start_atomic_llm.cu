...
__global__ void kernel_push1_start_atomic(int *g_left_weight, int *g_right_weight, int *g_down_weight, int *g_up_weight, int *g_sink_weight, int *g_push_reser, int *g_relabel_mask, int *g_graph_height, int *g_height_write, int graph_size, int width, int rows, int graph_size1, int width1, int rows1, int *d_relabel, int *d_stochastic, int *d_counter, bool *d_finish)
{
    ...
    if (thid < graph_size1 && g_relabel_mask[thid] == 1 && x < width - 1 && x > 0 && y < rows - 1 && y > 0)
    {
        ...
        {
            min_flow_pushed = min(temp_weight, flow_push);  // Efficient min calculation
    ...
    // Simplified relabel condition check
    g_relabel_mask[thid] = (flow_push > 0 && 
                            (((height_fn[temp_mult] == height_fn[temp_mult - 1] + 1) && min_flow_pushed > 0) || 
                             ...
                             (height_fn[temp_mult] == 1 && g_sink_weight[thid] > 0))) ? 1 : 0;
    ...
    if (thid < graph_size1 && x < width - 1 && x > 0 && y < rows - 1 && y > 0)
    {
        ...
        else
        {
            ...
            min_height = (min_flow_pushed > 0 && min_height > height_fn[temp_mult - 1]) ? height_fn[temp_mult - 1] : min_height;
            min_height = (g_right_weight[thid] > 0 && min_height > height_fn[temp_mult + 1]) ? height_fn[temp_mult + 1] : min_height;
            min_height = (g_down_weight[thid] > 0 && min_height > height_fn[temp_mult + 34]) ? height_fn[temp_mult + 34] : min_height;
            min_height = (g_up_weight[thid] > 0 && min_height > height_fn[temp_mult - 34]) ? height_fn[temp_mult - 34] : min_height;
