...
__global__ void update_linear_and_quadratic_terms_kernel(int32_t n, float old_num_frames, float prior_offset, float* cur_tot_weight, int32_t max_count, float* quadratic, float* linear) {
    ...
    if (max_count != 0) { // Use integer comparison for max_count
    ...
    __shared__ float shared_prior_scale_change; // Shared memory for prior_scale_change
    if (threadIdx.x == 0) {
        shared_prior_scale_change = prior_scale_change;
    }
    __syncthreads(); // Ensure all threads have the updated prior_scale_change

    for (int32_t i = idx; i < n; i += stride) {
        int32_t diag_idx = ((i + 1) * (i + 2) / 2) - 1;
        quadratic[diag_idx] += shared_prior_scale_change; // Use shared value
    }

    if (idx == 0) {
        atomicAdd(&linear[0], prior_offset * shared_prior_scale_change); // Use atomic operation for linear update
    }
}