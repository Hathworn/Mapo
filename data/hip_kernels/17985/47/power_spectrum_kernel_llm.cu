...
__global__ void power_spectrum_kernel(int row_length, float *A_in, int32_t ldi, float *A_out, int32_t ldo, bool use_power) {
    ...
    // Parallelize the computation for half_length with a stride of blockDim.x
    for (int idx = thread_id; idx < half_length; idx += blockDim.x) {
        ...
        Aw[idx] = use_power ? ret : sqrtf(ret); // Use conditional operator for simplicity
    }

    // Handle special cases for the first thread in each block
    if (thread_id == 0) {
        float real = Ar[0];
