...
__global__ void LowPassRowMulti(float *d_Result, float *d_Data, int width, int pitch, int height)
{
    ...
    const int scale = blockIdx.x % (NUM_SCALES + 3); // Change to modulo for better clarity
    ...
    data[tx] = (loadPos < 0) ? d_Data[yptr] : 
               (loadPos >= width) ? d_Data[yptr + width - 1] :
               d_Data[yptr + loadPos];
    __syncthreads();

    if (xout < width && tx < CONVROW_W) {
        // Unroll loop manually to improve performance
        float sum = (data[tx + 0] + data[tx + 8]) * kernel[0] +
