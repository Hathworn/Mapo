...
__global__ void ComputeOrientations(float *g_Data, int *d_Ptrs, float *d_Orient, int maxPts, int w, int h)
{
    ...
    // Apply parabolic interpolation and find peaks
    if (tx >= 0 && tx <= 31) {
        float interp_hist = (tx == 0) ? 
            (6 * hist[0] + 4 * (hist[1] + hist[31]) + (hist[2] + hist[30]))
            : (tx == 1) ? 
            (6 * hist[1] + 4 * (hist[2] + hist[0]) + (hist[3] + hist[31]))
            : (tx == 30) ? 
            (6 * hist[30] + 4 * (hist[31] + hist[29]) + (hist[0] + hist[28]))
            : (tx == 31) ?
            (6 * hist[31] + 4 * (hist[0] + hist[30]) + (hist[1] + hist[29]))
            : (6 * hist[tx] + 4 * (hist[tx+1] + hist[tx-1]) + (hist[tx+2] + hist[tx-2]));

        hist[32 + tx] = interp_hist;
    }
    __syncthreads();

    // Non-max suppression and store the strongest orientation
    float v = hist[32 + tx];
