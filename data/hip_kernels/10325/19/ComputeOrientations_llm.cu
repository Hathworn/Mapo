...
__global__ void ComputeOrientations(float *g_Data, int *d_Ptrs, float *d_Orient, int maxPts, int w, int h)
{
    ...
    // Compute gradient magnitudes and populate histograms
    for (int y=1;y<14;y++) {
        int memPos = 16*y + x;
        if (x>=1 && x<14) {
            ...
            atomicAdd(&hist[32*(x-1)+bin], grad*gauss[x]*gauss[y]); // Use atomicAdd for parallel updates
        }
    }
    __syncthreads();

    // Reduce histogram values across threads in the block
    for (int offset = 8; offset >= 1; offset /= 2) {
        if (tx < 32 * offset) {
            hist[tx] += hist[tx + 32 * offset];
        }
        __syncthreads();
    }

    // Smooth histograms using the Guassian weights
    if (tx < 32) {
        if (tx == 0) {
    ...
    // Locate orientation peaks, using atomicMax might be helpful
    if (tx==0) {
        float maxval1 = 0.0;
        float maxval2 = 0.0;
        int i1 = -1;
