...
__global__ void downscale(float *gradInput_data, const float *gradOutput_data, long no_elements, int scale_factor, int d1, int d2, int d3) {
    ...
    // Load values into registers outside the inner loops
    int base_idx[scale_factor * scale_factor];
    for (int i = 0; i < scale_factor; i++) {
        for (int j = 0; j < scale_factor; j++) {
            base_idx[i * scale_factor + j] = translate_idx_inv(ii, d1, d2, d3, scale_factor, i, j);
        }
    }
    // Accumulate the output data using pre-computed indices
    for (int i = 0; i < scale_factor * scale_factor; i++) {
        gradInput_data[ii] += gradOutput_data[base_idx[i]];
    }
}