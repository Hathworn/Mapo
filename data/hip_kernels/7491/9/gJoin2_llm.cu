...
__global__ void gJoin2(float* out, size_t rowBatch, size_t cols, const float* in1, size_t inStride1, const float* in2, size_t inStride2) {
    ...
    int j = blockIdx.x + blockIdx.y * gridDim.x; // Improved block index calculation
    ...
    if (j < rows) {
        ...
        for (int i = threadIdx.x; i < cols; i += blockDim.x) { // Optimized loop initialization and condition
            if (curPos < inStride1)
                rowOut[i] = rowIn1[i];
            else
                rowOut[i] = rowIn2[i];
