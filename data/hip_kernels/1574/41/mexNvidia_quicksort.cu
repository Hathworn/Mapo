...
//  hipMalloc(&d_output,  nitems * sizeof(int));
//  hipMemset(d_output, 0, nitems *sizeof(int));

//  hipMalloc(&d_idx,  nitems * sizeof(int));
//  hipMemset(d_idx, 0, nitems *sizeof(int));

  //copy input to output; quicksort acts on that array
//  hipMemcpy( d_output, d_input, nitems*sizeof(int), hipMemcpyDeviceToDevice );
...
//  hipMemcpy(output, d_output, nitems * sizeof(int), hipMemcpyDeviceToHost);

//  plhs[1] = mxCreateNumericArray(2, dimst, mxUINT32_CLASS, mxREAL);
//  idx = (unsigned int*) mxGetData(plhs[1]); 
//  hipMemcpy(idx, d_idx, nitems * sizeof(int), hipMemcpyDeviceToHost);
