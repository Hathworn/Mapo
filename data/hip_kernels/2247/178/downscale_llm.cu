...
__global__ void downscale(float *gradInput_data, const float *gradOutput_data, long no_elements, int scale_factor, int d1, int d2, int d3) {
    long ii = blockIdx.x * blockDim.x + threadIdx.x + (blockDim.x * gridDim.x) * (blockIdx.y * blockDim.y + threadIdx.y);
    
    // Early exit for threads with no work
    if (ii >= no_elements) return;
    
    // Precompute scaled indices
    int base_ipidx = translate_idx_inv(ii, d1, d2, d3, scale_factor, 0, 0);
    
    // Efficient accumulation with loop unrolling
    float sum = 0.0f;
    for (int i = 0; i < scale_factor; i++) {
#pragma unroll // Request unrolling for efficiency
        for (int j = 0; j < scale_factor; j++) {
            int ipidx = base_ipidx + i * d3 * scale_factor + j;
