...
// Note: Assuming CUDA_SHARED_MEM_SIZE is a defined constant
__global__ void conv2genericrev(float *input, float *kernel, float *output, int input_n, int input_h, int input_w, 
                                int kernel_n, int kernel_h, int kernel_w, float alpha, int stride_h, int stride_w)
{
    // Calculate output dimensions
    int output_h = input_h - (kernel_h - 1) * stride_h;
    int output_w = input_w - (kernel_w - 1) * stride_w;

    // Define global thread indices and batch, kernel ids
    int kk = blockIdx.x;
    int ii = blockIdx.y;
    ...
    for(xx = 0; xx < output_w; xx++) {
        ...
        // Unroll loop for input and kernel dot product
        ...
        output_p[xx] = sum; // Store the result in shared memory
    }
    __syncthreads();

    // Reduction phase (only performed by the first row of threads)
    if (yy == 0) {
