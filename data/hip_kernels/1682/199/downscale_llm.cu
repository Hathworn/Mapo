...
__global__ void downscale(float *gradInput_data, const float *gradOutput_data, long no_elements, int scale_factor, int d1, int d2, int d3) {
    ...
    // Use registers for commonly accessed variables
    int scale_factor_squared = scale_factor * scale_factor;
    float grad_sum = 0.0f;

    // Unroll small loop to optimize performance
    for (int i = 0; i < scale_factor; ++i) {
        for (int j = 0; j < scale_factor; ++j) {
            int ipidx = translate_idx_inv(ii, d1, d2, d3, scale_factor, i, j);
            grad_sum += gradOutput_data[ipidx];
        }
    }

    // Reduce the number of global memory accesses
    gradInput_data[ii] = grad_sum / scale_factor_squared;
}