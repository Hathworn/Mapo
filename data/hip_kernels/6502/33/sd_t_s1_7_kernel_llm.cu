...
__global__ void sd_t_s1_7_kernel(int h1d, int h2d, int h3d, int p4d, int p6d, int p4ld_t2, int h1ld_t2, int h3ld_v2, int h2ld_v2, int p6ld_v2, int h3ld_t3, int h2ld_t3, int h1ld_t3, int p6ld_t3, int p4ld_t3, double *t3d, double *t2_d, double *v2_d, int p4, int total_x) {
    ...
    // Load t2_d to shared memory in parallel for each block
    ...
    for (int idx = thread_x; idx < total_x; idx += gridDim.x * blockDim.x) {
        int rest_x = idx;
        h3 = rest_x % h3d;
        rest_x /= h3d;
        h2 = rest_x % h2d;
        ...
        // Optimize loop order to improve access patterns
        for (h1 = 0; h1 < h1d; h1++) {
            double sum = 0.0;
            for (p4 = 0; p4 < p4d; p4++) {
                sum += t2_shm[h1 * p4d + p4] * v2_d[h3 * h3ld_v2 + h2 * h2ld_v2 + p6 * p6ld_v2];
            }
            t3d[h3 * h3ld_t3 + h2 * h2ld_t3 + h1 * h1ld_t3 + p6 * p6ld_t3 + rest_x * p4ld_t3] += sum;
        }
    }
    __syncthreads();
}