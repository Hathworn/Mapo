...
__global__ void sd_t_s1_6_kernel(int h1d, int h2d, int h3d, int p4d, int p5d, int p6d, int p4ld_t2, int h1ld_t2, int h3ld_v2, int h2ld_v2, int p6ld_v2, int p5ld_v2, int h3ld_t3, int h2ld_t3, int h1ld_t3, int p6ld_t3, int p5ld_t3, int p4ld_t3, double *t3d, double *t2_d, double *v2_d, int p4, int total_x) {
    int h1, h2, h3, p6, p5;
    extern __shared__ double t2_shm[]; // Use extern for dynamic shared memory

    // Load t2_d to shared memory
    ...
    for (int i = idx; i < total_x; i += gridDim.x * blockDim.x) {
        int rest_x = i;
        h3 = rest_x % h3d;
        rest_x /= h3d;
        h2 = rest_x % h2d;
        ...
        // Inside loop for shared data calculation
        for (h1 = 0; h1 < h1d; h1++) {
            for (p4 = 0; p4 < p4d; p4++) {
