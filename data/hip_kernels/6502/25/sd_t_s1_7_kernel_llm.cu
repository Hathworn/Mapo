...
__global__ void sd_t_s1_7_kernel(size_t h1d, size_t h2d, size_t h3d, size_t p4d, size_t p6d, size_t p4ld_t2, size_t h1ld_t2, size_t h3ld_v2, size_t h2ld_v2, size_t p6ld_v2, size_t h3ld_t3, size_t h2ld_t3, size_t h1ld_t3, size_t p6ld_t3, size_t p4ld_t3, double *t3d, double *t2_d, double *v2_d, size_t p4, size_t total_x) {
    ...
    // Load shared memory with a single kernel call
    ...
    // Move calculations into the loop to reduce redundant computations
    for (size_t i = thread_x; i < total_x; i += gridDim.x * blockDim.x) {
        ...
        for (h1 = 0; h1 < h1d; ++h1)
            for (p4 = 0; p4 < p4d; ++p4) {
                // Reduced repetitive addressing calculations
                double temp_t2 = t2_shm[h1 * p4d + p4];
                size_t v2_index = h3 * h3ld_v2 + h2 * h2ld_v2 + p6 * p6ld_v2;
                size_t t3d_index = h3 * h3ld_t3 + h2 * h2ld_t3 + h1 * h1ld_t3 + p6 * p6ld_t3 + p4 * p4ld_t3;
                t3d[t3d_index] += temp_t2 * v2_d[v2_index];
            }
    }
    __syncthreads();
}