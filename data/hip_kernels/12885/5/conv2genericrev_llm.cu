...
__global__ void conv2genericrev(float *input, float *kernel, float *output, int input_n, int input_h, int input_w, int kernel_n, int kernel_h, int kernel_w, float alpha, int stride_h, int stride_w)
{
    ...
    // Pre-calculate input start pointers
    float *input_start = input + (ii + batch * input_n) * input_h * input_w + yy * stride_h * input_w;
    float *kernel_start = kernel + (kk + batch * kernel_n) * kernel_w * kernel_h;

    for(xx = 0; xx < output_w; xx++) {
        // Align input pointer for current xx
        float *input_p = input_start + xx * stride_w;
        float *kernel_p = kernel_start;
        float sum = 0;
