...
__global__ void ComputeOrientations(float *g_Data, int *d_Ptrs, float *d_Orient, int maxPts, int w, int h)
{
    ...
    // Apply smoothing
    if (tx < 32) {
        __shared__ float smoothed_hist[32];
        float smoothed_v = 6.0f * hist[tx] + 4.0f * (hist[(tx+1) % 32] + hist[(tx+31) % 32])
                           + hist[(tx+2) % 32] + hist[(tx+30) % 32];
        smoothed_hist[tx] = smoothed_v;
        __syncthreads();
        float v = smoothed_hist[tx];
        hist[tx] = (v > smoothed_hist[(tx+1) % 32] && v >= smoothed_hist[(tx+31) % 32]) ? v : 0.0f;
    }
    __syncthreads();

    // Determine peak orientations
    if (tx == 0) {
        float maxval1 = 0.0f;
        float maxval2 = 0.0f;
        int i1 = -1;
        ...
        if (i2 >= 0) {
            float val1 = hist[32 + ((i2 + 1) % 32)];
            float val2 = hist[32 + ((i2 + 31) % 32)];
