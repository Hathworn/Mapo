...
__global__ void ComputeOrientations(float *g_Data, float *d_Sift, int maxPts, int w, int h) {
    ...
    int xp = static_cast<int>(d_Sift[bx + 0 * maxPts] - 6.5f);
    int yp = static_cast<int>(d_Sift[bx + 1 * maxPts] - 6.5f);
    int px = xp & 15;
    int x = tx - px;
    ...
    // Smoothing histogram
    if (tx >= 0 && tx <= 31) {
        hist[tx + 32] = 6 * hist[tx] + 4 * (hist[(tx + 1) % 32] + hist[(tx + 31) % 32]) +
