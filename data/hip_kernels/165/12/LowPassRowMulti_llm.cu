...
__global__ void LowPassRowMulti(float *d_Result, float *d_Data, int width, int pitch, int height)
{
    ...
    // Load boundary values using min and max to reduce branches
    data[tx] = d_Data[yptr + max(0, min(width - 1, loadPos))];
    __syncthreads();

    // Ensure the condition is limited to necessary threads
    if (xout < width && tx < CONVROW_W) {
        // Compute using unrolled loop for better performance
        d_Result[writePos] =
