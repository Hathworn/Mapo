...
__global__ void Find3DMinMax(int *d_Result, float *d_Data1, float *d_Data2, float *d_Data3, int width, int height)
{
    ...
    for (int y = 0; y < 34; y++) {
        ...
        if (y > 1) {
            if (tx < memWid) {
                // Use float4 and min/max function to reduce repetitive computation
                float4 data1_val = make_float4(data1[memPos0], data1[memPos1], data1[memPos2], 0);
                float4 data2_val = make_float4(data2[memPos0], data2[memPos1], data2[memPos2], 0);
                float4 data3_val = make_float4(data3[memPos0], data3[memPos1], data3[memPos2], 0);
                
                float min1 = fminf(fminf(data1_val.x, data1_val.y), data1_val.z);
                float min2 = fminf(fminf(data2_val.x, data2_val.y), data2_val.z);
                float min3 = fminf(fminf(data3_val.x, data3_val.y), data3_val.z);
                float max1 = fmaxf(fmaxf(data1_val.x, data1_val.y), data1_val.z);
                float max2 = fmaxf(fmaxf(data2_val.x, data2_val.y), data2_val.z);
                float max3 = fmaxf(fmaxf(data3_val.x, data3_val.y), data3_val.z);
