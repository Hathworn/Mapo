...
__global__ void FindMinMax(float *d_MinMax, float *d_Data, int width, int pitch, int height)
{
    ...
    if (x < width) {
        ...
        // Iterate vertically over block height
        for (int ty = 1; ty < 16; ty++) {
            p += pitch;
            val = d_Data[p];
            // Update shared minvals and maxvals
            if (val < minvals[tx]) minvals[tx] = val;
            if (val > maxvals[tx]) maxvals[tx] = val;
        }
    } else {
        // Initialize shared memory for out-of-bound threads
        minvals[tx] = d_Data[p-x];
        maxvals[tx] = d_Data[p-x];
    }
