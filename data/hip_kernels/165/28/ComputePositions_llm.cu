...
__global__ void ComputePositions(float *g_Data1, float *g_Data2, float *g_Data3, int *d_Ptrs, float *d_Sift, int numPts, int maxPts, int w, int h)
{
    ...
    // Load values into registers to reduce global memory access
    ...
    // Use FMA intrinsics to improve precision and performance
    float pdx = fma(idet, fma(idxx, dx, fma(idxy, dy, idxs * ds)), 0.0f);
    float pdy = fma(idet, fma(idxy, dx, fma(idyy, dy, idys * ds)), 0.0f);
    float pds = fma(idet, fma(idxs, dx, fma(idys, dy, idss * ds)), 0.0f);
