...
__global__ void ComputeOrientations(float *g_Data, int *d_Ptrs, float *d_Orient, int maxPts, int w, int h)
{
    ...
    if (tx < 15) {
        float i2sigma2 = -1.0f / (2.0f * 9.0f); // Precomputed constant
    ...
    // Gradient computation and histogram population
    for (int y = 1; y < 14; y++) {
        int memPos = 16 * y + x;
        if (x >= 1 && x < 14) {
    ...
    float v = hist[32 + tx];
    if (tx < 32) {
        hist[32 + tx] = 6 * v + 4 * (hist[(32 + tx + 1) % 32] + hist[(32 + tx - 1) % 32]) + 
                        hist[(32 + tx + 2) % 32] + hist[(32 + tx - 2) % 32];
    }
    __syncthreads();
    ...
    // Determine orientations based on peaks
    if (tx == 0) {
        float maxval1 = 0.0f;
        float maxval2 = 0.0f;
        int i1 = -1;
