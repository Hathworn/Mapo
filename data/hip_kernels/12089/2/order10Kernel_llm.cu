...
__global__ void order10Kernel(double* device_X_Total, double* device_X_Not, double *device_F_Not, double h, double *device_f) {
    ...
    // Use registers to store common terms to reduce memory accesses
    double f_tx = device_F_Not[tx];
    double fx0 = device_f[tx];
    double fx2 = device_f[tx + 2];
    double fx4 = device_f[tx + 4];
    double fx6 = device_f[tx + 6];

    // Combine addition operations to reduce instruction count
    double common_term = h * f_tx;

    device_X_Total[tx] = device_X_Not[tx] + common_term + h * (b11 * fx0 + b12 * fx2 + b13 * fx4 + b14 * fx6);
    __syncthreads(); // Synchronize threads before updating the next elements

    device_X_Total[tx + 2] = device_X_Not[tx] + common_term + h * (b21 * fx0 + b22 * fx2 + b23 * fx4 + b24 * fx6);
    __syncthreads();

    device_X_Total[tx + 4] = device_X_Not[tx] + common_term + h * (b31 * fx0 + b32 * fx2 + b33 * fx4 + b34 * fx6);
    __syncthreads();

    device_X_Total[tx + 6] = device_X_Not[tx] + common_term + h * (b41 * fx0 + b42 * fx2 + b43 * fx4 + b44 * fx6);
}