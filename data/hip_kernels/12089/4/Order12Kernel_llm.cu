...
__global__ void Order12Kernel(double*device_X_Total, double* device_X_Not, double* device_F_Not, double h, double*device_f) {
    ...
    // Perform computations using shared memory to avoid multiple global reads
    __shared__ double s_device_F_Not[10];
    __shared__ double s_device_f[10];

    if (tx < 10) {
        s_device_F_Not[tx] = device_F_Not[tx];
        s_device_f[tx] = device_f[tx];
    }
    __syncthreads();

    // Reduced the number of __syncthreads for better performance
    device_X_Total[tx] = device_X_Not[tx] + h * ((bh10 * s_device_F_Not[tx]) + (bh11 * s_device_f[tx]) + (bh12 * s_device_f[tx + 2]) + (bh13 * s_device_f[tx + 4]) + (bh14 * s_device_f[tx + 6]) + (bh15 * s_device_f[tx + 8]));
    device_X_Total[tx + 2] = device_X_Not[tx] + h * ((bh20 * s_device_F_Not[tx]) + (bh21 * s_device_f[tx]) + (bh22 * s_device_f[tx + 2]) + (bh23 * s_device_f[tx + 4]) + (bh24 * s_device_f[tx + 6]) + (bh25 * s_device_f[tx + 8]));
    device_X_Total[tx + 4] = device_X_Not[tx] + h * ((bh30 * s_device_F_Not[tx]) + (bh31 * s_device_f[tx]) + (bh32 * s_device_f[tx + 2]) + (bh33 * s_device_f[tx + 4]) + (bh34 * s_device_f[tx + 6]) + (bh35 * s_device_f[tx + 8]));
    device_X_Total[tx + 6] = device_X_Not[tx] + h * ((bh40 * s_device_F_Not[tx]) + (bh41 * s_device_f[tx]) + (bh42 * s_device_f[tx + 2]) + (bh43 * s_device_f[tx + 4]) + (bh44 * s_device_f[tx + 6]) + (bh45 * s_device_f[tx + 8]));
    device_X_Total[tx + 8] = device_X_Not[tx] + h * ((bh50 * s_device_F_Not[tx]) + (bh51 * s_device_f[tx]) + (bh52 * s_device_f[tx + 2]) + (bh53 * s_device_f[tx + 4]) + (bh54 * s_device_f[tx + 6]) + (bh55 * s_device_f[tx + 8]));
}