...
__global__ void Order10FkKernel(double* device_X_Total, double* device_X_Not, double* device_F_Not, double h, double* device_f)
{
    int tx = threadIdx.x;
    if (tx < MAXEQNS) {
        double temp0 = g10 * device_F_Not[tx] + g11 * device_f[tx] + g12 * device_f[tx+2] + g13 * device_f[tx+4] + g14 * device_f[tx+6] + g15 * device_f[tx+8];
        double temp1 = g20 * device_F_Not[tx] + g21 * device_f[tx] + g22 * device_f[tx+2] + g23 * device_f[tx+4] + g24 * device_f[tx+6] + g25 * device_f[tx+8];
        double temp2 = g30 * device_F_Not[tx] + g31 * device_f[tx] + g32 * device_f[tx+2] + g33 * device_f[tx+4] + g34 * device_f[tx+6] + g35 * device_f[tx+8];
        double temp3 = g40 * device_F_Not[tx] + g41 * device_f[tx] + g42 * device_f[tx+2] + g43 * device_f[tx+4] + g44 * device_f[tx+6] + g45 * device_f[tx+8];
        double temp4 = g50 * device_F_Not[tx] + g51 * device_f[tx] + g52 * device_f[tx+2] + g53 * device_f[tx+4] + g54 * device_f[tx+6] + g55 * device_f[tx+8];

        device_X_Total[tx]     = device_X_Not[tx] + h * temp0;
        device_X_Total[tx+2] = device_X_Not[tx] + h * temp1;
        device_X_Total[tx+4] = device_X_Not[tx] + h * temp2;
        device_X_Total[tx+6] = device_X_Not[tx] + h * temp3;
        device_X_Total[tx+8] = device_X_Not[tx] + h * temp4;    
    }
}