...
__global__ void downscale(float *gradInput_data, const float *gradOutput_data, long no_elements, int scale_factor, int d1, int d2, int d3) {
    ...
    __shared__ float buffer[blockDim.x][blockDim.y]; 
    float sum = 0.0f;  // Accumulator for scale_factor loop
    ...
    buffer[threadIdx.y][threadIdx.x] = sum;

    // Write back to global memory
    __syncthreads();  // Ensure all threads complete before writing back

    gradInput_data[ii] = buffer[threadIdx.y][threadIdx.x];
}
```
