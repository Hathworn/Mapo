...
__global__ void convolutionZ_63_Kernel(float *d_Dst, float *d_Src, int imageW, int imageH, int imageD, int outofbounds, float outofboundsvalue) {
    ...
    for (int i = DEPTH_HALO_STEPS; i < DEPTH_HALO_STEPS + DEPTH_RESULT_STEPS; i++) {
        int index = i * DEPTH_BLOCKDIM_Z * imageW * imageH;
        if (outofbounds == 0)
            s_Data[threadIdx.x][threadIdx.z + i * DEPTH_BLOCKDIM_Z] = (imageD - baseZ > i * DEPTH_BLOCKDIM_Z) ? d_Src[index] : 0;
        else if (outofbounds == 1)
            s_Data[threadIdx.x][threadIdx.z + i * DEPTH_BLOCKDIM_Z] = (imageD - baseZ > i * DEPTH_BLOCKDIM_Z) ? d_Src[index] : outofboundsvalue;
        else
            s_Data[threadIdx.x][threadIdx.z + i * DEPTH_BLOCKDIM_Z] = (imageD - baseZ > i * DEPTH_BLOCKDIM_Z) ? d_Src[index] : d_Src[lastPixelInLine];
    }
    ...
    for (int i = 0; i < DEPTH_HALO_STEPS; i++) {
        int index = i * DEPTH_BLOCKDIM_Z * imageW * imageH;
        if (outofbounds == 0)
            s_Data[threadIdx.x][threadIdx.z + i * DEPTH_BLOCKDIM_Z] = (baseZ >= -i * DEPTH_BLOCKDIM_Z) ? d_Src[index] : 0;
        else if (outofbounds == 1)
            s_Data[threadIdx.x][threadIdx.z + i * DEPTH_BLOCKDIM_Z] = (baseZ >= -i * DEPTH_BLOCKDIM_Z) ? d_Src[index] : outofboundsvalue;
        else
            s_Data[threadIdx.x][threadIdx.z + i * DEPTH_BLOCKDIM_Z] = (baseZ >= -i * DEPTH_BLOCKDIM_Z) ? d_Src[index] : d_Src[firstPixelInLine];
    }
