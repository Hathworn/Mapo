...
__global__ void DeformablePSROIPoolForwardKernel(const int count, const double *bottom_data, const double spatial_scale,
                                                 ...
                                                 double *top_data, double *top_count)
{
    CUDA_KERNEL_LOOP(index, count)
    {
        ...
        // Pre-calculate transformation offsets for efficiency
        ...
        double wstart = (double)(pw) * bin_size_w + roi_start_w + trans_x * roi_width;
        double hstart = (double)(ph) * bin_size_h + roi_start_h + trans_y * roi_height;
        ...
        // Pre-calculate offset data pointer
        ...
        for (int ih = 0; ih < sample_per_part; ih++)
        {
            for (int iw = 0; iw < sample_per_part; iw++)
            {
                ...
                if (w >= -0.5 && w < width - 0.5 && h >= -0.5 && h < height - 0.5) // Early continue condition check for efficiency
                {
