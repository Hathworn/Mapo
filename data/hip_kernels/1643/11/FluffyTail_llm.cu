...
__global__ void FluffyTail(const uint2 *source, uint2 *destination, const int *sourceIndexes, int *destinationIndexes)
{
    ...
    // Use block-level atomics if supported, else use shared memory watchdog
    __shared__ int sharedDestIdx;
    int myEdges = sourceIndexes[group];
  
    if (lid == 0) sharedDestIdx = atomicAdd(destinationIndexes, myEdges);
    __syncthreads();
  
    int globalDestIdx = sharedDestIdx + lid;

    // Ensure only the valid threads are accessing the destination
    if (lid < myEdges)
    {
        destination[globalDestIdx] = source[group * DUCK_B_EDGES / 4 + lid];
    }
}