...
__global__ void kernel_2dfd_last(float *g_u1, float *g_u2, const int nx, const int iStart, const int iEnd)
{
    ...
    {
        // fetch the front value for the next iteration
        yval[8] = g_u2[idx + iskip];

        // Optimize: coalesced data access and reduce bank conflicts
        if (threadIdx.x < NPAD)
        {
            tile[threadIdx.x]  = g_u2[idx - NPAD];
            tile[threadIdx.x + BDIMX + NPAD] = g_u2[idx + BDIMX]; // Adjust to avoid bank conflicts
        }
