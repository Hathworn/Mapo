...
__global__ void downscale(float *gradInput_data, const float *gradOutput_data, long no_elements, int scale_factor, int d1, int d2, int d3) {
    // Calculate linear thread index across the block and grid
    long ii = blockIdx.x * blockDim.x + threadIdx.x + 
              (blockIdx.y * blockDim.y + threadIdx.y) * (blockDim.x * gridDim.x);
    if (ii >= no_elements) return;
    
    int gradIdx;
    // Loop unrolling to minimize loop overhead
    for (int i = 0; i < scale_factor; ++i) {
        for (int j = 0; j < scale_factor; ++j) {
            gradIdx = translate_idx_inv(ii, d1, d2, d3, scale_factor, i, j);
            gradInput_data[ii] += gradOutput_data[gradIdx];
        }
    }
}