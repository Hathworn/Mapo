...
__global__ void ScaleUp(float *d_Result, float *d_Data, int width, int pitch, int height, int newpitch)
{
    ...
    // Load shared memory with boundary checks integrated
    int x = blockIdx.x * (SCALEUP_W / 2) + tx - 1;
    int y = blockIdx.y * (SCALEUP_H / 2) + ty - 1;
    if (tx < BW && ty < BH) {
    ...
    x = blockIdx.x * SCALEUP_W + tx;
    y = blockIdx.y * SCALEUP_H + ty;

    if (x < 2 * width && y < 2 * height) {
        int bx = (tx + 1) >> 1;  // Use bitwise shift for division by 2
        int by = (ty + 1) >> 1;
        int bp = by * BW + bx;
        ...
        // Bilinear interpolation directly to result
