...
__device__ float3 hsv_to_rgb_kernel(float3 hsv) {
    ...
    float r = v, g = v, b = v;  // Initialize to grey when no saturation
    float f, p, q, t;

    if (s != 0) {  // Avoid division when s is zero
        int index = static_cast<int>(h);  // Implicit floor
        f = h - index;
        p = v * (1 - s);
        q = v * (1 - s * f);
        t = v * (1 - s * (1 - f));
        
        // Lookup table approach to avoid conditionals
        switch (index) {
