...
__global__ void Find3DMinMax(int *d_Result, float *d_Data1, float *d_Data2, float *d_Data3, int width, int height)
{
    ...
    const int xStart = blockIdx.x * MINMAX_SIZE;  // Use standard multiplication
    ...
    for (int y = 0; y < 34; y++) {
        ...
        yp = max(yp, 0);
        yp = min(yp, height - 1);
        int readStart = yp * width;  // Use standard multiplication
    ...
    if (tx < MINMAX_SIZE && xWritePos < width) {
        int writeStart = blockIdx.y * width + xWritePos;  // Use standard multiplication
