...
__global__ void Find3DMinMax(int *d_Result, float *d_Data1, float *d_Data2, float *d_Data3, int width, int pitch, int height)
{
    ...
    for (int y=0; y<32+2; y++) {
        ...
        if (y > 1 && tx < memWid) {
            // Merge successive min and max calculations
        ...
        if (y > 1 && tx < MINMAX_W && xWritePos <= xEndClamped) {
            // Combine conditions using common min and max values
            float minv = fminf(ymin2[tx], fminf(ymin2[tx+2], fminf(ymin1[tx+1], fminf(ymin3[tx+1], fminf(data2[memPos0+1], data2[memPos2+1])))));
            minv = fminf(minv, d_Threshold[1]);
            float maxv = fmaxf(ymax2[tx], fmaxf(ymax2[tx+2], fmaxf(ymax1[tx+1], fmaxf(ymax3[tx+1], fmaxf(data2[memPos0+1], data2[memPos2+1])))));
