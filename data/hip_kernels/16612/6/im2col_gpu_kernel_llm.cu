...
__global__ void im2col_gpu_kernel(const int n, const float* data_im, const int height, const int width, const int ksize, const int pad, const int stride, const int height_col, const int width_col, float *data_col) {
    ...
    // Use a fixed-index loop to ensure all elements are processed
    while (index < n) {
        ...
        // Prefetch shared pointers
        for (int i = 0; i < ksize; ++i) {
            for (int j = 0; j < ksize; ++j) {
                int h = h_in + i;
                int w = w_in + j;
                // Use improved condition evaluation
        ...
        index += total_threads; // Increment by total_threads for next processing
    }
}