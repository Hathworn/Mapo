...
__device__ unsigned int shared_reduce(unsigned int p, volatile unsigned int * s) {
    // Store the value and synchronize threads within the warp
    s[threadIdx.x] = p;
    __syncthreads();

    // Use binary tree reduction pattern within the warp
...
__global__ void reduce(unsigned int * d_out_shared, const unsigned int * d_in) {
    ...
    if (t == 0) {
        *d_out_shared = sr; // Store the result from thread 0
    }
}